...
SOIL_internal_create_OGL_texture
    ...
    if (
            ((flags & SOIL_FLAG_POWER_OF_TWO) && (!SOIL_IS_POW2(iwidth) || !SOIL_IS_POW2(iheight))) ||
            /*	user asked for it and the texture is not power of 2	*/
    ...
    if (tex_id) {
        ...
        if (flags & SOIL_FLAG_COMPRESS_TO_DXT) {
            ...
            if (DXT_mode == SOIL_CAPABILITY_PRESENT) {
                ...
                if ((channels & 1) == 1) {
                    ...
                    internal_texture_format = sRGB_texture ? SOIL_GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT
                                                           : SOIL_RGBA_S3TC_DXT5;
                }
            }
        ...
        if (flags & SOIL_FLAG_MIPMAPS || flags & SOIL_FLAG_GL_MIPMAPS) {
            createMipmaps(NULL != img ? img : data, iwidth, iheight, channels, flags, opengl_texture_target,
                          internal_texture_format, original_texture_format, DXT_mode);
    ...
    switch (header->dwpfFlags & PVRTEX_PIXELTYPE) {
        ...
        case OGL_PVRTC2:
            if (is_PVRTC_supported) {
                ...
                PVR_format = header->dwAlphaBitMask == 0 ? SOIL_COMPRESSED_RGB_PVRTC_2BPPV1_IMG
        ...
        case OGL_PVRTC4:
            if (is_PVRTC_supported) {
                is_compressed_format_supported = is_compressed_format = 1;
                PVR_format = header->dwAlphaBitMask == 0 ? SOIL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG
...
#define _MAX(a, b) (( a <= b )? b : a)
    for (i = 0; i < num_surfs; i++) {
        ...
        for (mipmap_level = 0;
             mipmap_level <= mipmaps; width = _MAX(width / 2, (unsigned int) 1), height = _MAX(height / 2,
                                                                                               (unsigned int) 1), mipmap_level++) {
            ...
            if (is_compressed_format) {
                ...
                if ((header->dwpfFlags & PVRTEX_PIXELTYPE) == OGL_PVRTC2) {
                    compressed_image_size = (_MAX(width, PVRTC2_MIN_TEXWIDTH) * _MAX(height, PVRTC2_MIN_TEXHEIGHT) *
                                             header->dwBitCount + 7) / 8;
                } else {// PVRTC4 case
                    compressed_image_size = (_MAX(width, PVRTC4_MIN_TEXWIDTH) * _MAX(height, PVRTC4_MIN_TEXHEIGHT) *
                                             header->dwBitCount + 7) / 8;
                }
                ...
                if (is_compressed_format_supported) {
                    ...
                    if (loading_as_cubemap) {
                        soilGlCompressedTexImage2D(SOIL_TEXTURE_CUBE_MAP_POSITIVE_X + i, mipmap_level, PVR_format,
                                                   width, height, 0, compressed_image_size, cur_texture_ptr);
                    } else {
                        soilGlCompressedTexImage2D(opengl_texture_type, mipmap_level, PVR_format, width, height, 0,
                ...
                if (loading_as_cubemap) {
                    glTexImage2D(SOIL_TEXTURE_CUBE_MAP_POSITIVE_X + i, mipmap_level, PVR_type, width, height, 0,
                                 PVR_type, PVR_format, cur_texture_ptr);
                } else {
                    glTexImage2D(opengl_texture_type, mipmap_level, PVR_type, width, height, 0, PVR_type, PVR_format,
                                 cur_texture_ptr);
                }
            }
...
unsigned int SOIL_direct_load_ETC1_from_memory(
    ...
    soilGlCompressedTexImage2D(opengl_texture_type, 0, SOIL_GL_ETC1_RGB8_OES, width, height, 0, compressed_image_size,
                               texture_ptr);
