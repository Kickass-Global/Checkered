...
static int
jo_processDU(FILE *fp, int *bitBuf, int *bitCnt, float *CDU, float *fdtbl, int DC, const unsigned short HTDC[256][2],
    ...
    for (dataOff = 0; dataOff < 64; dataOff += 8) {
        jo_DCT(&CDU[dataOff], &CDU[dataOff + 1], &CDU[dataOff + 2], &CDU[dataOff + 3], &CDU[dataOff + 4],
...
int jo_write_jpg(const char *filename, const void *data, int width, int height, int comp, int quality) {
    ...
    const unsigned char head1[] = {0xFF, 0xC0, 0, 0x11, 8, (unsigned char) (height >> 8),
                                   ...
                                   (unsigned char) (width & 0xFF), 3, 1, 0x11, 0, 2, 0x11, 1, 3, 0x11, 1, 0xFF, 0xC4,
                                   0x01, 0xA2, 0};
