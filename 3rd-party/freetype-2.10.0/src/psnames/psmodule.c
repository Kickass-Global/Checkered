...
FT_CALLBACK_DEF(int) compare_uni_maps(
...
static const char ft_extra_glyph_names[] = {'D', 'e', 'l', 't', 'a', 0, 'O', 'm', 'e', 'g', 'a', 0, 'f', 'r', 'a', 'c',
                                            't', 'i', 'o', 'n', 0, 'h', 'y', 'p', 'h', 'e', 'n', 0, 'm', 'a', 'c', 'r',
                                            'o', 'n', 0, 'm', 'u', 0, 'p', 'e', 'r', 'i', 'o', 'd', 'c', 'e', 'n', 't',
                                            'e', 'r', 'e', 'd', 0, 's', 'p', 'a', 'c', 'e', 0, 'T', 'c', 'o', 'm', 'm',
                                            'a', 'a', 'c', 'c', 'e', 'n', 't', 0, 't', 'c', 'o', 'm', 'm', 'a', 'a',
                                            'c', 'c', 'e', 'n', 't', 0};

static const FT_Int ft_extra_glyph_name_offsets[EXTRA_GLYPH_LIST_SIZE] = {0, 6, 12, 21, 28, 35, 38, 53, 59, 72};


static void ps_check_extra_glyph_name(
    const char *gname, FT_UInt glyph, FT_UInt *extra_glyphs, FT_UInt *states
) {
    ...
    for (n = 0; n < EXTRA_GLYPH_LIST_SIZE; n++) {
        if (ft_strcmp(
            ft_extra_glyph_names + ft_extra_glyph_name_offsets[n], gname
        ) == 0) {
            if (states[n] == 0) {
...
static void ps_check_extra_glyph_unicode(
    FT_UInt32 uni_char, FT_UInt *states
) {
...
static FT_Error ps_unicodes_init(
    FT_Memory memory, PS_Unicodes table, FT_UInt num_glyphs, PS_GetGlyphNameFunc get_glyph_name,
    ...
    if (!FT_NEW_ARRAY(table->maps, num_glyphs + EXTRA_GLYPH_LIST_SIZE)) {
        ...
        for (n = 0; n < num_glyphs; n++) {
            ...
            if (gname) {
                ps_check_extra_glyph_name(
                    gname, n, extra_glyphs, extra_glyph_list_states
                );
                uni_char = ps_unicode_value(gname);

                if (BASE_GLYPH(uni_char) != 0) {
                    ps_check_extra_glyph_unicode(
                        uni_char, extra_glyph_list_states
                    );
        ...
        if (count == 0) {
            ...
            ft_qsort(
                table->maps, count, sizeof(PS_UniMap), compare_uni_maps
            );
        }
...
static FT_UInt ps_unicodes_char_index(
    PS_Unicodes table, FT_UInt32 unicode
) {
...
static FT_UInt32 ps_unicodes_char_next(
    PS_Unicodes table, FT_UInt32 *unicode
) {
...
static FT_Pointer psnames_get_service(
    FT_Module module, const char *service_id
) {
