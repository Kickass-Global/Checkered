...
#define FT_STRUCTURE  WinMZ_HeaderRec
    ...
    FT_FRAME_START(64), FT_FRAME_USHORT_LE (magic), FT_FRAME_SKIP_BYTES(29 * 2), FT_FRAME_ULONG_LE  (lfanew),
    FT_FRAME_END};
...
#define FT_STRUCTURE  WinNE_HeaderRec
    ...
    FT_FRAME_START(40), FT_FRAME_USHORT_LE (magic), FT_FRAME_SKIP_BYTES(34), FT_FRAME_USHORT_LE (resource_tab_offset),
    FT_FRAME_USHORT_LE (rname_tab_offset), FT_FRAME_END};
...
#define FT_STRUCTURE  WinPE32_HeaderRec
    ...
    FT_FRAME_USHORT_LE (number_of_sections), FT_FRAME_SKIP_BYTES(12), FT_FRAME_USHORT_LE (size_of_optional_header),
    FT_FRAME_SKIP_BYTES(2), FT_FRAME_USHORT_LE (magic32), /* 0x10B */
    FT_FRAME_SKIP_BYTES(110), FT_FRAME_ULONG_LE  (rsrc_virtual_address), FT_FRAME_ULONG_LE  (rsrc_size),
...
#define FT_STRUCTURE  WinPE32_SectionRec
    ...
    FT_FRAME_START(40), FT_FRAME_BYTES     (name, 8), FT_FRAME_SKIP_BYTES(4), FT_FRAME_ULONG_LE  (virtual_address),
    FT_FRAME_ULONG_LE  (size_of_raw_data), FT_FRAME_ULONG_LE  (pointer_to_raw_data), FT_FRAME_SKIP_BYTES(16),
    FT_FRAME_END};
...
#define FT_STRUCTURE  WinPE_RsrcDirRec
    ...
    FT_FRAME_START(16), FT_FRAME_ULONG_LE (characteristics), FT_FRAME_ULONG_LE (time_date_stamp),
    FT_FRAME_USHORT_LE(major_version), FT_FRAME_USHORT_LE(minor_version), FT_FRAME_USHORT_LE(number_of_named_entries),
    FT_FRAME_USHORT_LE(number_of_id_entries), FT_FRAME_END};
...
#define FT_STRUCTURE  WinPE_RsrcDataEntryRec
    ...
    FT_FRAME_START(16), FT_FRAME_ULONG_LE(offset_to_data), FT_FRAME_ULONG_LE(size), FT_FRAME_ULONG_LE(code_page),
...
#define FT_STRUCTURE  FT_WinFNT_HeaderRec
    ...
    FT_FRAME_START(148), FT_FRAME_USHORT_LE(version), FT_FRAME_ULONG_LE (file_size), FT_FRAME_BYTES    (copyright, 60),
    FT_FRAME_USHORT_LE(file_type), FT_FRAME_USHORT_LE(nominal_point_size), FT_FRAME_USHORT_LE(vertical_resolution),
    FT_FRAME_USHORT_LE(horizontal_resolution), FT_FRAME_USHORT_LE(ascent), FT_FRAME_USHORT_LE(internal_leading),
    FT_FRAME_USHORT_LE(external_leading), FT_FRAME_BYTE     (italic), FT_FRAME_BYTE     (underline),
    FT_FRAME_BYTE     (strike_out), FT_FRAME_USHORT_LE(weight), FT_FRAME_BYTE     (charset),
    FT_FRAME_USHORT_LE(pixel_width), FT_FRAME_USHORT_LE(pixel_height), FT_FRAME_BYTE     (pitch_and_family),
    FT_FRAME_USHORT_LE(avg_width), FT_FRAME_USHORT_LE(max_width), FT_FRAME_BYTE     (first_char),
    FT_FRAME_BYTE     (last_char), FT_FRAME_BYTE     (default_char), FT_FRAME_BYTE     (break_char),
    FT_FRAME_USHORT_LE(bytes_per_row), FT_FRAME_ULONG_LE (device_offset), FT_FRAME_ULONG_LE (face_name_offset),
    FT_FRAME_ULONG_LE (bits_pointer), FT_FRAME_ULONG_LE (bits_offset), FT_FRAME_BYTE     (reserved),
    FT_FRAME_ULONG_LE (flags), FT_FRAME_USHORT_LE(A_space), FT_FRAME_USHORT_LE(B_space), FT_FRAME_USHORT_LE(C_space),
    FT_FRAME_ULONG_LE (color_table_offset), FT_FRAME_BYTES    (reserved1, 16), FT_FRAME_END};
...
static FT_Error fnt_font_load(
    FNT_Font font, FT_Stream stream
) {
    ...
    if (mz_header.magic == WINFNT_MZ_MAGIC) {
        ...
        if (ne_header.magic == WINFNT_NE_MAGIC) {
            ...
            if (FT_STREAM_SEEK(res_offset) ||
                FT_FRAME_ENTER(ne_header.rname_tab_offset - ne_header.resource_tab_offset))
                goto Exit;
            ...
            FT_TRACE2(("magic %04lx, machine %02x, number_of_sections %u, "
                       ...
                       "rsrc_size %04lx\n", pe32_header.magic, pe32_header.machine, pe32_header.number_of_sections, pe32_header.size_of_optional_header, pe32_header.magic32, pe32_header.rsrc_virtual_address, pe32_header.rsrc_size));
            ...
            for (i = 0; i < pe32_header.number_of_sections; i++) {
                ...
                FT_TRACE2(
                    ("name %.8s, va %04lx, size %04lx, offset %04lx\n", pe32_section.name, pe32_section.virtual_address, pe32_section.size_of_raw_data, pe32_section.pointer_to_raw_data));
            ...
            for (i = 0; i < root_dir.number_of_named_entries + root_dir.number_of_id_entries; i++) {
                ...
                if (FT_STREAM_SEEK(pe32_section.pointer_to_raw_data + dir_entry1.offset) ||
                ...
                for (j = 0; j < name_dir.number_of_named_entries + name_dir.number_of_id_entries; j++) {
                    if (FT_STREAM_SEEK(name_dir_offset + 16 + j * 8) ||
                        FT_STREAM_READ_FIELDS(winpe_rsrc_dir_entry_fields, &dir_entry2))
                        goto Exit;
                    ...
                    if (FT_STREAM_SEEK(pe32_section.pointer_to_raw_data + dir_entry2.offset) ||
                    ...
                    for (k = 0; k < lang_dir.number_of_named_entries + lang_dir.number_of_id_entries; k++) {
                        if (FT_STREAM_SEEK(lang_dir_offset + 16 + k * 8) ||
                            FT_STREAM_READ_FIELDS(winpe_rsrc_dir_entry_fields, &dir_entry3))
                            goto Exit;
                        ...
                        if (dir_entry1.name == 8 /* RT_FONT */ ) {
                            if (FT_STREAM_SEEK(root_dir_offset + dir_entry3.offset) ||
                                FT_STREAM_READ_FIELDS(winpe_rsrc_data_entry_fields, &data_entry))
                                goto Exit;

                            FT_TRACE2(("found font #%lu, offset %04lx, "
                                       "size %04lx, cp %lu\n", dir_entry2.name, pe32_section.pointer_to_raw_data +
                                                                                data_entry.offset_to_data -
                                                                                pe32_section.virtual_address, data_entry.size, data_entry.code_page));

                            if (face_index == face->root.num_faces) {
                                if (FT_NEW(face->font))
                                    goto Exit;

                                face->font->offset = pe32_section.pointer_to_raw_data + data_entry.offset_to_data -
...
static FT_Error fnt_cmap_init(
    FNT_CMap cmap, FT_Pointer pointer
) {
...
static FT_UInt fnt_cmap_char_index(
    FNT_CMap cmap, FT_UInt32 char_code
) {
...
static FT_UInt32 fnt_cmap_char_next(
    FNT_CMap cmap, FT_UInt32 *pchar_code
) {
...
static const FT_CMap_ClassRec fnt_cmap_class_rec = {sizeof(FNT_CMapRec),
                                                    ...
                                                    (FT_CMap_InitFunc) fnt_cmap_init, (FT_CMap_DoneFunc) NULL,
...
static FT_Error FNT_Face_Init(
    FT_Stream stream, FT_Face fntface,        /* FNT_Face */
    ...
    /* with relevant information                   */
    {
...

        {
            ...
            error = FT_CMap_New(
                fnt_cmap_class, NULL, &charmap, NULL);
            if (error)
                goto Fail;
        }
...
static FT_Error FNT_Size_Select(
    FT_Size size, FT_ULong strike_index
) {
...
static FT_Error FNT_Size_Request(
    FT_Size size, FT_Size_Request req
) {
...
static FT_Error FNT_Load_Glyph(
    FT_GlyphSlot slot, FT_Size size, FT_UInt glyph_index, FT_Int32 load_flags
) {
    ...
    ft_synthesize_vertical_metrics(
        &slot->metrics, (FT_Pos) (bitmap->rows << 6));
...
static FT_Error winfnt_get_header(
    FT_Face face, FT_WinFNT_HeaderRec *aheader
) {
...
static const FT_Service_WinFntRec winfnt_service_rec = {winfnt_get_header       /* get_header */
};
...
static const FT_ServiceDescRec winfnt_services[] = {{FT_SERVICE_ID_FONT_FORMAT, FT_FONT_FORMAT_WINFNT},
                                                    {FT_SERVICE_ID_WINFNT, &winfnt_service_rec},
                                                    {NULL,                      NULL}};


static FT_Module_Interface winfnt_get_service(
    FT_Module module, const FT_String *service_id
) {
