...
static int ft_lzwstate_refill(FT_LzwState state) {
    ...
    count = FT_Stream_TryRead(
        state->source, state->buf_tab, state->num_bits
    );  /* WHY? */
...
static FT_Int32 ft_lzwstate_get_code(FT_LzwState state) {
    ...
    if (state->buf_clear || offset >= state->buf_size || state->free_ent >= state->free_bits) {
        if (state->free_ent >= state->free_bits) {
            ...
            state->free_bits =
                state->num_bits < state->max_bits ? (FT_UInt) ((1UL << num_bits) - 256) : state->max_free + 1;
        }
...
FT_LOCAL_DEF(void) ft_lzwstate_reset(FT_LzwState state) {
...
FT_LOCAL_DEF(void) ft_lzwstate_init(
    FT_LzwState state, FT_Stream source
) {
...
FT_LOCAL_DEF(void) ft_lzwstate_done(FT_LzwState state) {
...
FT_LOCAL_DEF(FT_ULong) ft_lzwstate_io(
    FT_LzwState state, FT_Byte *buffer, FT_ULong out_size
) {
    ...
    switch (state->phase) {
        case FT_LZW_PHASE_START: {
            ...
            state->free_bits =
                state->num_bits < state->max_bits ? (FT_UInt) ((1UL << state->num_bits) - 256) : state->max_free + 1;
