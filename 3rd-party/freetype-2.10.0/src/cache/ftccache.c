...
static void ftc_node_mru_unlink(
    FTC_Node node, FTC_Manager manager
) {
...
static void ftc_node_hash_unlink(
    FTC_Node node0, FTC_Cache cache
) {
...
static void ftc_node_hash_link(
    FTC_Node node, FTC_Cache cache
) {
...
static void ftc_cache_add(
    FTC_Cache cache, FT_Offset hash, FTC_Node node
) {
...
FT_LOCAL_DEF(FT_Error) FTC_Cache_NewNode(
    FTC_Cache cache, FT_Offset hash, FT_Pointer query, FTC_Node *anode
) {
...
FT_LOCAL_DEF(void) FTC_Cache_RemoveFaceID(
    FTC_Cache cache, FTC_FaceID face_id
) {
    ...
    for (i = 0; i < count; i++) {
        ...
        for (;;) {
            ...
            if (cache->clazz.node_remove_faceid(
                node, face_id, cache, &list_changed
            )) {
                *pnode = node->link;
                node->link = frees;
                frees = node;
