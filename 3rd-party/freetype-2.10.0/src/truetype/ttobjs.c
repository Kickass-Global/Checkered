 ...
 */
FT_LOCAL_DEF(void) tt_glyphzone_done(TT_GlyphZone zone) {
 ...
 */
FT_LOCAL_DEF(FT_Error) tt_glyphzone_new(
    FT_Memory memory, FT_UShort maxPoints, FT_Short maxContours, TT_GlyphZone zone
) {
    ...
    if (FT_NEW_ARRAY(zone->org, maxPoints) || FT_NEW_ARRAY(zone->cur, maxPoints) ||
        FT_NEW_ARRAY(zone->orus, maxPoints) || FT_NEW_ARRAY(zone->tags, maxPoints) ||
...
static FT_UInt32 tt_synth_sfnt_checksum(
...
static FT_ULong tt_get_sfnt_checksum(
    TT_Face face, FT_UShort i
) {
    ...
    if (face->goto_table(
        face, face->dir_tables[i].Tag, face->root.stream, NULL))
        return 0;

    return (FT_ULong) tt_synth_sfnt_checksum(
        face->root.stream, face->dir_tables[i].Length
    );
}
 ...
 *   FreeType error code.  0 means success.
 */
FT_LOCAL_DEF(FT_Error) tt_face_init(
    FT_Stream stream, FT_Face ttface,      /* TT_Face */
 ...
 */
FT_LOCAL_DEF(void) tt_face_done(FT_Face ttface)           /* TT_Face */
{
 ...
 *   FreeType error code.  0 means success.
 */
FT_LOCAL_DEF(FT_Error) tt_size_run_fpgm(
    TT_Size size, FT_Bool pedantic
) {
    ...
    TT_Set_CodeRange(
        exec, tt_coderange_font, face->font_program, (FT_Long) face->font_program_size
    );
 ...
 *   FreeType error code.  0 means success.
 */
FT_LOCAL_DEF(FT_Error) tt_size_run_prep(
    TT_Size size, FT_Bool pedantic
) {
    ...
    TT_Set_CodeRange(
        exec, tt_coderange_cvt, face->cvt_program, (FT_Long) face->cvt_program_size
    );
...
static FT_Error tt_size_init_bytecode(
    FT_Size ftsize, FT_Bool pedantic
) {
    ...
    if (FT_NEW_ARRAY(size->function_defs, size->max_function_defs) ||
        FT_NEW_ARRAY(size->instruction_defs, size->max_instruction_defs) || FT_NEW_ARRAY(size->cvt, size->cvt_size) ||
...
FT_LOCAL_DEF(FT_Error) tt_size_ready_bytecode(
    TT_Size size, FT_Bool pedantic
) {
 ...
 *   FreeType error code.  0 means success.
 */
FT_LOCAL_DEF(FT_Error) tt_size_init(FT_Size ttsize)           /* TT_Size */
{
 ...
 */
FT_LOCAL_DEF(void) tt_size_done(FT_Size ttsize)           /* TT_Size */
{
 ...
 */
FT_LOCAL_DEF(FT_Error) tt_size_reset(
    TT_Size size, FT_Bool only_height
) {
    ...
    *size_metrics = size->root.metrics;
    ...
    if (face->header.Flags & 8) {
        ...
        size_metrics->ascender = FT_PIX_ROUND(FT_MulFix(
            face->root.ascender, size_metrics->y_scale
        ));
        size_metrics->descender = FT_PIX_ROUND(FT_MulFix(
            face->root.descender, size_metrics->y_scale
        ));
        size_metrics->height = FT_PIX_ROUND(FT_MulFix(
            face->root.height, size_metrics->y_scale
        ));
    }
    ...
    if (face->header.Flags & 8) {
        ...
        size_metrics->x_scale = FT_DivFix(
            size_metrics->x_ppem << 6, face->root.units_per_EM
        );
        size_metrics->y_scale = FT_DivFix(
            size_metrics->y_ppem << 6, face->root.units_per_EM
        );

        size_metrics->max_advance = FT_PIX_ROUND(FT_MulFix(
            face->root.max_advance_width, size_metrics->x_scale
        ));
    }
    ...
    if (size_metrics->x_ppem >= size_metrics->y_ppem) {
        ...
        size->ttmetrics.y_ratio = FT_DivFix(
            size_metrics->y_ppem, size_metrics->x_ppem
        );
    } else {
        size->ttmetrics.scale = size_metrics->y_scale;
        size->ttmetrics.ppem = size_metrics->y_ppem;
        size->ttmetrics.x_ratio = FT_DivFix(
            size_metrics->x_ppem, size_metrics->y_ppem
        );
 ...
 *   FreeType error code.  0 means success.
 */
FT_LOCAL_DEF(FT_Error) tt_driver_init(FT_Module ttdriver)     /* TT_Driver */
{
 ...
 */
FT_LOCAL_DEF(void) tt_driver_done(FT_Module ttdriver)     /* TT_Driver */
{
