...
/**************************************************************************
   ...
   */
static FT_UShort *ft_var_readpackedpoints(
    FT_Stream stream, FT_ULong size, FT_UInt *point_cnt
) {
...
/**************************************************************************
   ...
   *   step).
   */
static FT_Fixed *ft_var_readpackeddeltas(
    FT_Stream stream, FT_ULong size, FT_UInt delta_cnt
) {
...
static FT_Error ft_var_load_item_variation_store(
    TT_Face face, FT_ULong offset, GX_ItemVarStore itemStore
) {
    ...
    for (i = 0; i < itemStore->dataCount; i++) {
        ...
        if (FT_READ_USHORT(varData->itemCount) || FT_READ_USHORT(shortDeltaCount) ||
...
static FT_Error ft_var_load_delta_set_index_mapping(
    TT_Face face, FT_ULong offset, GX_DeltaSetIdxMap map, GX_ItemVarStore itemStore
) {
...
/**************************************************************************
   ...
   */
static FT_Error ft_var_load_hvvar(
    ...
    error = ft_var_load_item_variation_store(
        face, table_offset + store_offset, &table->itemStore
    );
    if (error)
        goto Exit;
    ...
    if (widthMap_offset) {
        error = ft_var_load_delta_set_index_mapping(
            face, table_offset + widthMap_offset, &table->widthMap, &table->itemStore
        );
        if (error)
            goto Exit;
    }
...
static FT_Int ft_var_get_item_delta(
    ...
    for (master = 0; master < varData->regionIdxCount; master++) {
        ...
        for (j = 0; j < itemStore->axisCount; j++, axis++) {
            ...
            if (axis->startCoord > axis->peakCoord || axis->peakCoord > axis->endCoord)
                ...
                scalar = FT_MulDiv(
                    scalar, face->blend->normalizedcoords[j] - axis->startCoord, axis->peakCoord - axis->startCoord
                );
            else
                scalar = FT_MulDiv(
                    scalar, axis->endCoord - face->blend->normalizedcoords[j], axis->endCoord - axis->peakCoord
                );
        } /* per-axis loop */
...
/**************************************************************************
   ...
   */
static FT_Error tt_hvadvance_adjust(
    TT_Face face, FT_UInt gindex, FT_Int *avalue, FT_Bool vertical
) {
    ...
    delta = ft_var_get_item_delta(
        face, &table->itemStore, outerIndex, innerIndex
    );

    FT_TRACE5(
        ("%s value %d adjusted by %d unit%s (%s)\n", vertical ? "vertical height" : "horizontal width", *avalue, delta,
            delta == 1 ? "" : "s", vertical ? "VVAR" : "HVAR"));
...
FT_LOCAL_DEF(FT_Error) tt_hadvance_adjust(
    TT_Face face, FT_UInt gindex, FT_Int *avalue
) {
...
FT_LOCAL_DEF(FT_Error) tt_vadvance_adjust(
    TT_Face face, FT_UInt gindex, FT_Int *avalue
) {
...
static FT_Short *ft_var_get_value_pointer(
    TT_Face face, FT_ULong mvar_tag
) {
...
static void ft_var_load_mvar(TT_Face face) {
    ...
    error = ft_var_load_item_variation_store(
        face, table_offset + store_offset, &blend->mvar_table->itemStore
    );
    if (error)
        return;
    ...
    for (; value < limit; value++) {
        ...
        if (value->outerIndex >= itemStore->dataCount ||
            value->innerIndex >= itemStore->varData[value->outerIndex].itemCount) {
            error = FT_THROW(Invalid_Table);
            break;
        }
    }
...
/**************************************************************************
   ...
   *     The font face.
   */
FT_LOCAL_DEF(void) tt_apply_mvar(TT_Face face) {
    ...
    for (; value < limit; value++) {
        ...
        delta = ft_var_get_item_delta(
            face, &blend->mvar_table->itemStore, value->outerIndex, value->innerIndex
        );

        if (p) {
            FT_TRACE5(
                ("value %c%c%c%c (%d unit%s) adjusted by %d unit%s (MVAR)\n", (FT_Char) (value->tag >> 24), (FT_Char) (
                    value->tag >> 16), (FT_Char) (value->tag >> 8), (FT_Char) (value->tag), value->unmodified,
                    value->unmodified == 1 ? "" : "s", delta, delta == 1 ? "" : "s"));
    ...
    /* adjust all derived values */
    {
       ...
       */
        ...
        root->height = root->ascender - root->descender + current_line_gap + mvar_hlgp_delta;
        ...
        FT_List_Iterate(
            &root->sizes_list, tt_size_reset_iterator, NULL);
    }
}
...
#define FT_STRUCTURE  GX_GVar_Head
        ...
        FT_FRAME_START(20), FT_FRAME_LONG  (version), FT_FRAME_USHORT(axisCount), FT_FRAME_USHORT(globalCoordCount),
        FT_FRAME_ULONG (offsetToCoord), FT_FRAME_USHORT(glyphCount), FT_FRAME_USHORT(flags),
        FT_FRAME_ULONG (offsetToData), FT_FRAME_END};


    FT_TRACE2(("GVAR "));

    if (FT_SET_ERROR(face->goto_table(
        face, TTAG_gvar, stream, &table_len
    ))) {
        FT_TRACE2(("is missing\n"));
        goto Exit;
    }
...
/**************************************************************************
   ...
   */
static FT_Fixed ft_var_apply_tuple(
    GX_Blend blend, FT_UShort tupleIndex, FT_Fixed *tuple_coords, FT_Fixed *im_start_coords, FT_Fixed *im_end_coords
) {
    ...
    for (i = 0; i < blend->num_axis; i++) {
        ...
        if (!(tupleIndex & GX_TI_INTERMEDIATE_TUPLE)) {
            ...
            apply = FT_MulDiv(
                apply, blend->normalizedcoords[i], tuple_coords[i]
            );
        } else {
            /* intermediate tuple */
            ...
            FT_TRACE6(
                ("      intermediate tuple range ]%.5f;%.5f[ fits\n", im_start_coords[i] / 65536.0, im_end_coords[i] /
                                                                                                    65536.0));
            if (blend->normalizedcoords[i] < tuple_coords[i])
                apply = FT_MulDiv(
                    apply, blend->normalizedcoords[i] - im_start_coords[i], tuple_coords[i] - im_start_coords[i]
                );
            else
                apply = FT_MulDiv(
                    apply, im_end_coords[i] - blend->normalizedcoords[i], im_end_coords[i] - tuple_coords[i]
                );
        }
    }
...
static void ft_var_to_normalized(
    TT_Face face, FT_UInt num_coords, FT_Fixed *coords, FT_Fixed *normalized
) {
...
static void ft_var_to_design(
    TT_Face face, FT_UInt num_coords, FT_Fixed *coords, FT_Fixed *design
) {
    ...
    for (i = 0; i < nc; i++, a++) {
        if (design[i] < 0)
            design[i] = a->def + FT_MulFix(
                design[i], a->def - a->minimum
            );
        else if (design[i] > 0)
            design[i] = a->def + FT_MulFix(
                design[i], a->maximum - a->def
            );
        else
            design[i] = a->def;
    }
...
/**************************************************************************
   ...
   *   FreeType error code.  0 means success.
   */
FT_LOCAL_DEF(FT_Error) TT_Get_MM_Var(
    TT_Face face, FT_MM_Var **master
) {
...
#define FT_STRUCTURE  GX_FVar_Head
        ...
        FT_FRAME_START(16), FT_FRAME_LONG      (version), FT_FRAME_USHORT    (offsetToData), FT_FRAME_SKIP_SHORT,
        FT_FRAME_USHORT    (axisCount), FT_FRAME_USHORT    (axisSize), FT_FRAME_USHORT    (instanceCount),
        FT_FRAME_USHORT    (instanceSize), FT_FRAME_END};
...
#define FT_STRUCTURE  GX_FVar_Axis
        ...
        FT_FRAME_START(20), FT_FRAME_ULONG (axisTag), FT_FRAME_LONG  (minValue), FT_FRAME_LONG  (defaultValue),
        FT_FRAME_LONG  (maxValue), FT_FRAME_USHORT(flags), FT_FRAME_USHORT(nameID), FT_FRAME_END};
    ...
    if (need_init) {
        ...
        if (FT_SET_ERROR(face->goto_table(
            ...
            if (FT_SET_ERROR(face->goto_table(
                face, TTAG_CFF2, stream, &table_len
            ))) {
                FT_TRACE1(("\n"
        ...
        if (FT_SET_ERROR(face->goto_table(
            face, TTAG_fvar, stream, &table_len
        ))) {
...
#define ALIGN_SIZE(n) \
    ...
    if (need_init) {
        face->blend->mmvar_len =
            mmvar_size + axis_flags_size + axis_size + namedstyle_size + next_coords_size + next_name_size;
        ...
        if (num_instances != fvar_head.instanceCount) {
            ...
            found = sfnt->get_name_id(
                face, TT_NAME_ID_TYPOGRAPHIC_SUBFAMILY, &dummy1, &dummy2
            );
            if (found)
                strid = TT_NAME_ID_TYPOGRAPHIC_SUBFAMILY;
            else {
                found = sfnt->get_name_id(
                    face, TT_NAME_ID_FONT_SUBFAMILY, &dummy1, &dummy2
                );
                if (found)
            ...
            if (found) {
                found = sfnt->get_name_id(
                    face, TT_NAME_ID_PS_NAME, &dummy1, &dummy2
                );
                if (found) {
...
static FT_Error tt_set_mm_blend(
    TT_Face face, FT_UInt num_coords, FT_Fixed *coords, FT_Bool set_design_coords
) {
    ...
    enum {
        mcvt_retain, mcvt_modify, mcvt_load
    ...
    if (set_design_coords)
        ft_var_to_design(
            face, all_design_coords ? blend->num_axis : num_coords, blend->normalizedcoords, blend->coords
        );
...
/**************************************************************************
   ...
   *   FreeType error code.  0 means success.
   */
FT_LOCAL_DEF(FT_Error) TT_Set_MM_Blend(
    TT_Face face, FT_UInt num_coords, FT_Fixed *coords
) {
...
/**************************************************************************
   ...
   *   FreeType error code.  0 means success.
   */
FT_LOCAL_DEF(FT_Error) TT_Get_MM_Blend(
    TT_Face face, FT_UInt num_coords, FT_Fixed *coords
) {
...
/**************************************************************************
   ...
   *   FreeType error code.  0 means success.
   */
FT_LOCAL_DEF(FT_Error) TT_Set_Var_Design(
    TT_Face face, FT_UInt num_coords, FT_Fixed *coords
) {
...
/**************************************************************************
   ...
   *   FreeType error code.  0~means success.
   */
FT_LOCAL_DEF(FT_Error) TT_Get_Var_Design(
    TT_Face face, FT_UInt num_coords, FT_Fixed *coords
) {
...
/**************************************************************************
   ...
   *   FreeType error code.  0~means success.
   */
FT_LOCAL_DEF(FT_Error) TT_Set_Named_Instance(
    TT_Face face, FT_UInt instance_index
) {
    ...
    if (instance_index > 0 && mmvar->namedstyle) {
        ...
        error = sfnt->get_name(
            face, (FT_UShort) named_style->strid, &style_name
        );
        if (error)
            goto Exit;
        ...
        error = TT_Set_Var_Design(
            face, mmvar->num_axis, named_style->coords
        );
        if (error)
            goto Exit;
    } else
...
/**************************************************************************
   ...
   */
FT_LOCAL_DEF(FT_Error) tt_face_vary_cvt(
    TT_Face face, FT_Stream stream
) {
    ...
    if (FT_NEW_ARRAY(tuple_coords, blend->num_axis) || FT_NEW_ARRAY(im_start_coords, blend->num_axis) ||
    ...
    FT_TRACE5(("cvar: there %s %d tuple%s:\n", (tupleCount & GX_TC_TUPLE_COUNT_MASK) == 1 ? "is" : "are", tupleCount &
    ...
    for (i = 0; i < (tupleCount & GX_TC_TUPLE_COUNT_MASK); i++) {
        ...
        if (tupleIndex & GX_TI_EMBEDDED_TUPLE_COORD) {
            ...
            FT_MEM_COPY(tuple_coords, blend->tuplecoords + (tupleIndex & GX_TI_TUPLE_INDEX_MASK) * blend->num_axis,
        ...
        if (!points || !deltas ||
            (localpoints == ALL_POINTS && point_count != face->cvt_size)); /* failure, ignore it */
...
static void tt_delta_shift(
    int p1, int p2, int ref, FT_Vector *in_points, FT_Vector *out_points
) {
...
static void tt_interpolate_deltas(
    ...
    do {
        ...
        if (point <= end_point) {
            ...
            while (point <= end_point) {
                ...
                if (has_delta[point]) {
                    tt_delta_interpolate(
                        cur_delta + 1, point - 1, cur_delta, point, in_points, out_points
                    );
                    cur_delta = point;
                }
            ...
            if (cur_delta == first_delta)
                tt_delta_shift(
                    first_point, end_point, cur_delta, in_points, out_points
                );
            else {
                /* otherwise handle remaining points       */
                /* at the end and beginning of the contour */
                tt_delta_interpolate(
                    cur_delta + 1, end_point, cur_delta, first_delta, in_points, out_points
                );

                if (first_delta > 0)
                    tt_delta_interpolate(
                        first_point, first_delta - 1, cur_delta, first_delta, in_points, out_points
                    );
            }
        }
        contour++;
...
/**************************************************************************
   ...
   *   FreeType error code.  0 means success.
   */
FT_LOCAL_DEF(FT_Error) TT_Vary_Apply_Glyph_Deltas(
    ...
    if (tupleCount & GX_TC_TUPLES_SHARE_POINT_NUMBERS) {
        ...
        sharedpoints = ft_var_readpackedpoints(
            stream, blend->gvar_size, &spoint_count
        );
        offsetToData = FT_Stream_FTell(stream);
    ...
    for (i = 0; i < (tupleCount & GX_TC_TUPLE_COUNT_MASK); i++) {
        ...
        if (tupleIndex & GX_TI_PRIVATE_POINT_NUMBERS) {
            localpoints = ft_var_readpackedpoints(
                stream, blend->gvar_size, &point_count
            );
            points = localpoints;
        } else {
            points = sharedpoints;
        ...
        deltas_x = ft_var_readpackeddeltas(
            stream, blend->gvar_size, point_count == 0 ? n_points : point_count
        );
        deltas_y = ft_var_readpackeddeltas(
            stream, blend->gvar_size, point_count == 0 ? n_points : point_count
        );

        if (!points || !deltas_y || !deltas_x); /* failure, ignore it */
          ...
          FT_TRACE7(( "      none\n" ));
            ...
            tt_interpolate_deltas(
                outline, points_out, points_org, has_delta
            );
...
/**************************************************************************
   ...
   */
FT_LOCAL_DEF(FT_Error) tt_get_var_blend(
    TT_Face face, FT_UInt *num_coords, FT_Fixed **coords, FT_Fixed **normalizedcoords, FT_MM_Var **mm_var
) {
...
static void ft_var_done_item_variation_store(
    TT_Face face, GX_ItemVarStore itemStore
) {
...
/**************************************************************************
   ...
   */
FT_LOCAL_DEF(void) tt_done_blend(TT_Face face) {
    ...
    if (blend) {
        ...
        if (blend->vvar_table) {
            ft_var_done_item_variation_store(
                face, &blend->vvar_table->itemStore
            );
        ...
        if (blend->mvar_table) {
            ft_var_done_item_variation_store(
                face, &blend->mvar_table->itemStore
            );
