 ...
 */
FT_LOCAL_DEF(void) TT_Get_HMetrics(
    TT_Face face, FT_UInt idx, FT_Short *lsb, FT_UShort *aw
) {
 ...
 */
FT_LOCAL_DEF(void) TT_Get_VMetrics(
    TT_Face face, FT_UInt idx, FT_Pos yMax, FT_Short *tsb, FT_UShort *ah
) {
...
static FT_Error tt_get_metrics(
    TT_Loader loader, FT_UInt glyph_index
) {
    ...
    TT_Get_HMetrics(
        face, glyph_index, &left_bearing, &advance_width
    );
    TT_Get_VMetrics(
        face, glyph_index, loader->bbox.yMax, &top_bearing, &advance_height
    );
...
static void tt_get_metrics_incr_overrides(
    TT_Loader loader, FT_UInt glyph_index
) {
...
FT_CALLBACK_DEF(FT_Error) TT_Access_Glyph_Frame(
...
FT_CALLBACK_DEF(void) TT_Forget_Glyph_Frame(TT_Loader loader) {
...
FT_CALLBACK_DEF(FT_Error) TT_Load_Glyph_Header(TT_Loader loader) {
...
FT_CALLBACK_DEF(FT_Error) TT_Load_Simple_Glyph(TT_Loader load) {
    ...
    if (IS_HINTED(load->load_flags)) {
        ...
        error = Update_Max(
            load->exec->memory, &tmp, sizeof(FT_Byte), (void *) &load->exec->glyphIns, n_ins
        );
...
FT_CALLBACK_DEF(FT_Error) TT_Load_Composite_Glyph(TT_Loader loader) {
...
FT_LOCAL_DEF(void) TT_Init_Glyph_Loading(TT_Face face) {
...
static void tt_prepare_zone(
    TT_GlyphZone zone, FT_GlyphLoad load, FT_UInt start_point, FT_UInt start_contour
) {
 ...
 */
static FT_Error TT_Hint_Glyph(
    TT_Loader loader, FT_Bool is_composite
) {
    ...
    if (n_ins > 0) {
        ...
        TT_Set_CodeRange(
            loader->exec, tt_coderange_glyph, loader->exec->glyphIns, n_ins
        );
...
static FT_Error TT_Process_Simple_Glyph(TT_Loader loader) {
    ...
    if (FT_IS_NAMED_INSTANCE(FT_FACE(loader->face)) || FT_IS_VARIATION(FT_FACE(loader->face))) {
        ...
        error = TT_Vary_Apply_Glyph_Deltas(
            loader->face, loader->glyph_index, outline, (FT_UInt) n_points
        );
...
static FT_Error TT_Process_Composite_Component(
    TT_Loader loader, FT_SubGlyph subglyph, FT_UInt start_point, FT_UInt num_base_points
) {
    ...
    if (!(subglyph->flags & ARGS_ARE_XY_VALUES)) {
...
#endif
        {
             ...
             */
            FT_Fixed mac_xscale = FT_Hypot(
                subglyph->transform.xx, subglyph->transform.xy
            );
            FT_Fixed mac_yscale = FT_Hypot(
                subglyph->transform.yy, subglyph->transform.yx
            );
 ...
 */
static FT_Error TT_Process_Composite_Glyph(
    TT_Loader loader, FT_UInt start_point, FT_UInt start_contour
) {
...

    {
        ...
        if (n_ins > max_ins) {
            ...
            error = Update_Max(
                loader->exec->memory, &tmp, sizeof(FT_Byte), (void *) &loader->exec->glyphIns, n_ins
            );
    ...
    tt_prepare_zone(
        &loader->zone, &loader->gloader->base, start_point, start_contour
    );
...
static FT_ListNode ft_list_get_node_at(
    FT_List list, FT_UInt idx
) {
 ...
 *   TT_Loader object.
 */
static FT_Error load_truetype_glyph(
    TT_Loader loader, FT_UInt glyph_index, FT_UInt recurse_count, FT_Bool header_only
) {
    ...
    if (face->root.internal->incremental_interface) {
        ...
        FT_Stream_OpenMemory(
            &inc_stream, glyph_data.pointer, (FT_ULong) glyph_data.length
        );
        ...
        offset = tt_face_get_location(
            face, glyph_index, (FT_UInt *) &loader->byte_len
        );
    ...
    if (loader->byte_len > 0) {
        ...
        error = face->access_glyph_frame(
            loader, glyph_index, face->glyf_offset + offset, (FT_UInt) loader->byte_len
        );
        if (error)
            goto Exit;
    ...
    error = face->access_glyph_frame(
        loader, glyph_index, face->glyf_offset + offset + 10, (FT_UInt) loader->byte_len - 10
    );
    if (error)
        goto Exit;
    ...
    if (loader->n_contours > 0) {
        ...
        if (FT_List_Find(
            &loader->composites, FT_UINT_TO_POINTER(glyph_index))) {
        ...
        if (FT_IS_NAMED_INSTANCE(FT_FACE(face)) || FT_IS_VARIATION(FT_FACE(face))) {
            ...
            if (FT_NEW_ARRAY(points, outline.n_points) || FT_NEW_ARRAY(tags, outline.n_points) ||
...

        {
            ...
            for (n = 0; n < num_subglyphs; n++) {
                ...
                error = load_truetype_glyph(
                    loader, (FT_UInt) subglyph->index, recurse_count + 1, FALSE
                );
                if (error)
                    goto Exit;
            ...
            if (IS_HINTED(loader->load_flags) &&
                ...
                error = TT_Process_Composite_Glyph(
                    loader, start_point, start_contour
                );
                if (error)
                    goto Exit;
            }
...
static FT_Error compute_glyph_metrics(
    TT_Loader loader, FT_UInt glyph_index
) {
    ...
    if (
        ...
        widthp = tt_face_get_device_metrics(
            face, size->metrics->x_ppem, glyph_index
        );
    ...
    /* create some metrics manually                                   */
    {
        ...
        if (face->vertical_info && face->vertical.number_Of_VMetrics > 0) {
            top = (FT_Short) FT_DivFix(
                loader->pp3.y - bbox.yMax, y_scale
            );

            if (loader->pp3.y <= loader->pp4.y)
                advance = 0;
            else
                advance = (FT_UShort) FT_DivFix(
                    loader->pp3.y - loader->pp4.y, y_scale
                );
        } else {
            FT_Pos height;
...
static FT_Error load_sbit_image(
    TT_Size size, TT_GlyphSlot glyph, FT_UInt glyph_index, FT_Int32 load_flags
) {
    ...
    error = sfnt->load_sbit_image(
        face, size->strike_index, glyph_index, (FT_UInt) load_flags, stream, &glyph->bitmap, &sbit_metrics
    );
    if (!error) {
...
static FT_Error tt_loader_init(
    TT_Loader loader, TT_Size size, TT_GlyphSlot glyph, FT_Int32 load_flags, FT_Bool glyf_table_only
) {
    ...
    if (IS_HINTED(load_flags) && !glyf_table_only) {
        ...
        if (driver->interpreter_version == TT_INTERPRETER_VERSION_40) {
            ...
            grayscale_cleartype = FT_BOOL(
                subpixel_hinting_lean && !((load_flags & FT_LOAD_TARGET_LCD) || (load_flags & FT_LOAD_TARGET_LCD_V)));
...
static void tt_loader_done(TT_Loader loader) {
    ...
    FT_List_Finalize(
        &loader->composites, NULL, loader->face->root.memory, NULL);
}
 ...
 */
FT_LOCAL_DEF(FT_Error) TT_Load_Glyph(
    TT_Size size, TT_GlyphSlot glyph, FT_UInt glyph_index, FT_Int32 load_flags
) {
        ...
        if (FT_ERR_EQ(error, Missing_Bitmap)) {
            ...
            if (FT_IS_SCALABLE(glyph->face)) {
                ...
                if (!glyph->metrics.horiAdvance && glyph->linearHoriAdvance)
                    glyph->metrics.horiAdvance = FT_MulFix(
                        glyph->linearHoriAdvance, x_scale
                    );
                if (!glyph->metrics.vertAdvance && glyph->linearVertAdvance)
                    glyph->metrics.vertAdvance = FT_MulFix(
                        glyph->linearVertAdvance, y_scale
                    );
            }
