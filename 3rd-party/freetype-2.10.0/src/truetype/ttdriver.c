 ...
 *
 */
static FT_Error tt_property_set(
    FT_Module module,         /* TT_Driver */
...
static FT_Error tt_property_get(
    FT_Module module,         /* TT_Driver */
 ...
 */
static FT_Error tt_get_kerning(
...
static FT_Error tt_get_advances(
    FT_Face ttface, FT_UInt start, FT_UInt count, FT_Int32 flags, FT_Fixed *advances
) {
...
static FT_Error tt_size_select(
    ...
    if (FT_IS_SCALABLE(size->face)) {
        ...
        error = sfnt->load_strike_metrics(
            ttface, strike_index, size_metrics
        );
        if (error)
...
static FT_Error tt_size_request(
    ...
    if (FT_IS_SCALABLE(size->face)) {
        ...
        if (!error) {
            ...
            ttsize->point_size = FT_MulDiv(
                ttsize->ttmetrics.ppem, 64 * 72, resolution
            );
        }
#endif
    }
 ...
 *   FreeType error code.  0 means success.
 */
static FT_Error tt_glyph_load(
    FT_GlyphSlot ttslot,      /* TT_GlyphSlot */
...
FT_DEFINE_SERVICEDESCREC6(tt_services,
                          ...
                          FT_SERVICE_ID_FONT_FORMAT, FT_FONT_FORMAT_TRUETYPE, FT_SERVICE_ID_MULTI_MASTERS,
                          &tt_service_gx_multi_masters, FT_SERVICE_ID_METRICS_VARIATIONS,
                          &tt_service_metrics_variations, FT_SERVICE_ID_TRUETYPE_ENGINE, &tt_service_truetype_engine,
                          FT_SERVICE_ID_TT_GLYF, &tt_service_truetype_glyf, FT_SERVICE_ID_PROPERTIES,
...
FT_CALLBACK_DEF(FT_Module_Interface) tt_get_interface(
    FT_Module driver,    /* TT_Driver */
