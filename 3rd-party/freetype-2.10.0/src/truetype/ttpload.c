...
FT_LOCAL_DEF(FT_ULong) tt_face_get_location(
    TT_Face face, FT_UInt gindex, FT_UInt *asize
) {
    ...
    if (pos2 > face->glyf_len) {
        ...
        if (gindex == face->num_locations - 2) {
            FT_TRACE1(("tt_face_get_location:"
                       ...
                       "                     "
                       " truncating at the end of `glyf' table to %ld bytes\n", pos2 - pos1, gindex, face->glyf_len -
                                                                                                     pos1));
            pos2 = face->glyf_len;
        } else {
            FT_TRACE1(("tt_face_get_location:"
...
FT_LOCAL_DEF(void) tt_face_done_loca(TT_Face face) {
 ...
 *   FreeType error code.  0 means success.
 */
FT_LOCAL_DEF(FT_Error) tt_face_load_cvt(
    TT_Face face, FT_Stream stream
) {
 ...
 *   FreeType error code.  0 means success.
 */
FT_LOCAL_DEF(FT_Error) tt_face_load_fpgm(
    TT_Face face, FT_Stream stream
) {
 ...
 *   FreeType error code.  0 means success.
 */
FT_LOCAL_DEF(FT_Error) tt_face_load_prep(
    TT_Face face, FT_Stream stream
) {
...
FT_LOCAL_DEF(FT_Error) tt_face_load_hdmx(
    TT_Face face, FT_Stream stream
) {
...
FT_LOCAL_DEF(void) tt_face_free_hdmx(TT_Face face) {
 ...
 */
FT_LOCAL_DEF(FT_Byte*)tt_face_get_device_metrics(
    TT_Face face, FT_UInt ppem, FT_UInt gindex
) {
