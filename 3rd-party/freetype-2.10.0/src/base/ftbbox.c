 ...
 */
static void BBox_Conic_Check(
    FT_Pos y1, FT_Pos y2, FT_Pos y3, FT_Pos *min, FT_Pos *max
) {
 ...
 */
static int BBox_Conic_To(
    FT_Vector *control, FT_Vector *to, TBBox_Rec *user
) {
    ...
    if (CHECK_X(control, user->bbox))
        BBox_Conic_Check(
            user->last.x, control->x, to->x, &user->bbox.xMin, &user->bbox.xMax
        );

    if (CHECK_Y(control, user->bbox))
        BBox_Conic_Check(
            user->last.y, control->y, to->y, &user->bbox.yMin, &user->bbox.yMax
        );
 ...
 *     The address of the current maximum.
 */
static FT_Pos cubic_peak(
    FT_Pos q1, FT_Pos q2, FT_Pos q3, FT_Pos q4
) {
...
static void BBox_Cubic_Check(
    FT_Pos p1, FT_Pos p2, FT_Pos p3, FT_Pos p4, FT_Pos *min, FT_Pos *max
) {
    /* This function is only called when a control off-point is outside  */
 ...
 */
static int BBox_Cubic_To(
    FT_Vector *control1, FT_Vector *control2, FT_Vector *to, TBBox_Rec *user
) {
    ...
    if (CHECK_X(control1, user->bbox) || CHECK_X(control2, user->bbox))
        BBox_Cubic_Check(
            user->last.x, control1->x, control2->x, to->x, &user->bbox.xMin, &user->bbox.xMax
        );

    if (CHECK_Y(control1, user->bbox) || CHECK_Y(control2, user->bbox))
        BBox_Cubic_Check(
            user->last.y, control1->y, control2->y, to->y, &user->bbox.yMin, &user->bbox.yMax
        );
...
FT_EXPORT_DEF(FT_Error) FT_Outline_Get_BBox(
    FT_Outline *outline, FT_BBox *abbox
) {
