...
FT_EXPORT_DEF(FT_Error) FT_Bitmap_Copy(
    FT_Library library, const FT_Bitmap *source, FT_Bitmap *target
) {
...
static FT_Error ft_bitmap_assure_buffer(
    FT_Memory memory, FT_Bitmap *bitmap, FT_UInt xpixels, FT_UInt ypixels
) {
...
FT_EXPORT_DEF(FT_Error) FT_Bitmap_Embolden(
    FT_Library library, FT_Bitmap *bitmap, FT_Pos xStrength, FT_Pos yStrength
) {
    ...
    error = ft_bitmap_assure_buffer(
        library->memory, bitmap, (FT_UInt) xstr, (FT_UInt) ystr
    );
    if (error)
        return error;
...
static FT_Byte ft_gray_for_premultiplied_srgb_bgra(const FT_Byte *bgra) {
    ...
    l = (4732UL /* 0.0722 * 65536 */ * bgra[0] * bgra[0] + 46871UL /* 0.7152 * 65536 */ * bgra[1] * bgra[1] +
...
FT_EXPORT_DEF(FT_Error) FT_Bitmap_Convert(
    FT_Library library, const FT_Bitmap *source, FT_Bitmap *target, FT_Int alignment
) {
...
FT_EXPORT_DEF(FT_Error) FT_Bitmap_Blend(
    FT_Library library, const FT_Bitmap *source_, const FT_Vector source_offset_, FT_Bitmap *target,
...
FT_EXPORT_DEF(FT_Error) FT_Bitmap_Done(
    FT_Library library, FT_Bitmap *bitmap
) {
