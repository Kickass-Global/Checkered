...
typedef void Function_Sweep_Init(
    RAS_ARGS Short *min, Short *max
);

typedef void Function_Sweep_Span(
    RAS_ARGS Short y, FT_F26Dot6 x1, FT_F26Dot6 x2, PProfile left, PProfile right
);
 ...
 *  profile.
 */
static Bool New_Profile(
    RAS_ARGS TStates aState, Bool overshoot
) {
 ...
 */
static Bool Line_Up(
    RAS_ARGS Long x1, Long y1, Long x2, Long y2, Long miny, Long maxy
) {
 ...
 *   SUCCESS on success, FAILURE on render pool overflow.
 */
static Bool Bezier_Up(
    RAS_ARGS Int degree, TSplitter splitter, Long miny, Long maxy
) {
...
static Bool Line_To(
    ...
    switch (ras.state) {
        case Unknown_State:
            if (y > ras.lastY) {
                if (New_Profile(
                    RAS_VARS Ascending_State, IS_BOTTOM_OVERSHOOT(ras.lastY)))
                    return FAILURE;
            } else {
                if (y < ras.lastY)
                    if (New_Profile(
                        RAS_VARS Descending_State, IS_TOP_OVERSHOOT(ras.lastY)))
                        return FAILURE;
            }
            break;
        ...
        case Ascending_State:
            if (y < ras.lastY) {
                if (End_Profile(RAS_VARS IS_TOP_OVERSHOOT(ras.lastY)) || New_Profile(
                    RAS_VARS Descending_State, IS_TOP_OVERSHOOT(ras.lastY)))
                    return FAILURE;
            }
            break;
        ...
        case Descending_State:
            if (y > ras.lastY) {
                if (End_Profile(RAS_VARS IS_BOTTOM_OVERSHOOT(ras.lastY)) || New_Profile(
                    RAS_VARS Ascending_State, IS_BOTTOM_OVERSHOOT(ras.lastY)))
                    return FAILURE;
            }
            break;
    ...
    switch (ras.state) {
        case Ascending_State:
            if (Line_Up(
                RAS_VARS ras.lastX, ras.lastY, x, y, ras.minY, ras.maxY
            ))
                return FAILURE;
            break;
 ...
 *  profile.
 */
static Bool Conic_To(
    RAS_ARGS Long cx, Long cy, Long x, Long y
) {
 ...
 *  profile.
 */
static Bool Cubic_To(
    RAS_ARGS Long cx1, Long cy1, Long cx2, Long cy2, Long x, Long y
) {
 ...
 */
static Bool Decompose_Curve(
    RAS_ARGS UShort first, UShort last, Int flipped
) {
        ...
        switch (tag) {
            ...
            Do_Conic:
                if (point < limit) {
                    ...
                    if (Conic_To(
                        RAS_VARS v_control.x, v_control.y, v_middle.x, v_middle.y
                    ))
                        goto Fail;
                ...
                if (Conic_To(
                    RAS_VARS v_control.x, v_control.y, v_start.x, v_start.y
                ))
                    goto Fail;
...
static Bool Convert_Glyph(RAS_ARGS Int flipped) {
    ...
    for (i = 0; i < ras.outline.n_contours; i++) {
        ...
        if (Decompose_Curve(
            RAS_VARS (UShort) start, (UShort) ras.outline.contours[i], flipped
        ))
            return FAILURE;
 ...
 */
static void InsNew(
    PProfileList list, PProfile profile
) {
...
static void Vertical_Sweep_Init(
    RAS_ARGS Short *min, Short *max
) {
...
static void Vertical_Sweep_Span(
    RAS_ARGS Short y, FT_F26Dot6 x1, FT_F26Dot6 x2, PProfile left, PProfile right
) {
    ...
    FT_TRACE7(("  y=%d x=[%.12f;%.12f], drop-out=%d", y, x1 / (double) ras.precision, x2 /
                                                                                      (double) ras.precision, dropOutControl));
...
static void Vertical_Sweep_Drop(
    RAS_ARGS Short y, FT_F26Dot6 x1, FT_F26Dot6 x2, PProfile left, PProfile right
) {
...
static void Horizontal_Sweep_Init(
    RAS_ARGS Short *min, Short *max
) {
    /* nothing, really */
    FT_UNUSED_RASTER;
...
static void Horizontal_Sweep_Span(
    RAS_ARGS Short y, FT_F26Dot6 x1, FT_F26Dot6 x2, PProfile left, PProfile right
) {
...
static void Horizontal_Sweep_Drop(
    RAS_ARGS Short y, FT_F26Dot6 x1, FT_F26Dot6 x2, PProfile left, PProfile right
) {
...
#endif
            ras.Proc_Sweep_Drop(
                RAS_VARS y, P_Left->X, P_Right->X, P_Left, P_Right
            );
        }
...
static FT_Error Render_Glyph(RAS_ARG) {
    ...
    Set_High_Precision(
        RAS_VARS ras.outline.flags & FT_OUTLINE_HIGH_PRECISION
    );
...
static int ft_black_new(
    FT_Memory memory, black_PRaster *araster
) {
...
static void ft_black_reset(
    FT_Raster raster, PByte pool_base, ULong pool_size
) {
...
static int ft_black_set_mode(
    FT_Raster raster, ULong mode, void *args
) {
...
static int ft_black_render(
    FT_Raster raster, const FT_Raster_Params *params
) {
