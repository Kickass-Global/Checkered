 ...
 */
FT_CALLBACK_DEF(FT_Error) cff_get_kerning(
 ...
 *   FreeType error code.  0 means success.
 */
FT_CALLBACK_DEF(FT_Error) cff_glyph_load(
    FT_GlyphSlot cffslot,      /* CFF_GlyphSlot */
...
FT_CALLBACK_DEF(FT_Error) cff_get_advances(
    FT_Face face, FT_UInt start, FT_UInt count, FT_Int32 flags, FT_Fixed *advances
) {
    ...
    if (FT_IS_SFNT(face)) {
        ...
        if (flags & FT_LOAD_VERTICAL_LAYOUT) {
            ...
            for (nn = 0; nn < count; nn++) {
                ...
                ((SFNT_Service) ttface->sfnt)->get_metrics(
                    ttface, 1, start + nn, &dummy, &ah
                );
...
static FT_Error cff_get_glyph_name(
    CFF_Face face, FT_UInt glyph_index, FT_Pointer buffer, FT_UInt buffer_max
) {
    ...
    if (font->version_major == 2) {
        ...
        FT_Service_GlyphDict service = (FT_Service_GlyphDict) ft_module_get_service(
...
static FT_UInt cff_get_name_index(
    CFF_Face face, FT_String *glyph_name
) {
...
static FT_Error cff_ps_get_font_info(
    CFF_Face face, PS_FontInfoRec *afont_info
) {
    ...
    if (cff && !cff->font_info) {
        ...
        font_info->version = cff_index_get_sid_string(
            cff, dict->version
        );
        font_info->notice = cff_index_get_sid_string(
            cff, dict->notice
        );
        font_info->full_name = cff_index_get_sid_string(
            cff, dict->full_name
        );
        font_info->family_name = cff_index_get_sid_string(
            cff, dict->family_name
        );
        font_info->weight = cff_index_get_sid_string(
            cff, dict->weight
        );
...
static FT_Error cff_ps_get_font_extra(
    CFF_Face face, PS_FontExtraRec *afont_extra
) {
    ...
    if (cff && cff->font_extra == NULL) {
        ...
        if (embedded_postscript) {
            ...
            if ((start_fstype = ft_strstr(
                embedded_postscript, "/FSType"
            )) != NULL && (start_def = ft_strstr(
                start_fstype + sizeof("/FSType") - 1, "def"
            )) != NULL) {
                FT_String *s;
...
static const char *cff_get_ps_name(CFF_Face face) {
    ...
    if (FT_IS_SFNT(FT_FACE(face)) && sfnt) {
        FT_Library library = FT_FACE_LIBRARY(face);
        FT_Module sfnt_module = FT_Get_Module(library, "sfnt");
        FT_Service_PsFontName service = (FT_Service_PsFontName) ft_module_get_service(
 ...
 *
 */
static FT_Error cff_get_cmap_info(
 ...
 *
 */
static FT_Error cff_get_ros(
    CFF_Face face, const char **registry, const char **ordering, FT_Int *supplement
) {
    ...
    if (cff) {
        ...
        if (registry) {
            if (!cff->registry)
                cff->registry = cff_index_get_sid_string(
                    cff, dict->cid_registry
                );
        ...
        if (ordering) {
            if (!cff->ordering)
                cff->ordering = cff_index_get_sid_string(
                    cff, dict->cid_ordering
                );
...
static FT_Error cff_get_is_cid(
    CFF_Face face, FT_Bool *is_cid
) {
...
static FT_Error cff_get_cid_from_glyph_index(
    CFF_Face face, FT_UInt glyph_index, FT_UInt *cid
) {
...
static FT_Error cff_set_mm_blend(
    CFF_Face face, FT_UInt num_coords, FT_Fixed *coords
) {
...
static FT_Error cff_get_mm_blend(
    CFF_Face face, FT_UInt num_coords, FT_Fixed *coords
) {
...
static FT_Error cff_set_mm_weightvector(
    CFF_Face face, FT_UInt len, FT_Fixed *weightvector
) {
...
static FT_Error cff_get_mm_weightvector(
    CFF_Face face, FT_UInt *len, FT_Fixed *weightvector
) {
...
static FT_Error cff_get_mm_var(
    CFF_Face face, FT_MM_Var **master
) {
...
static FT_Error cff_set_var_design(
    CFF_Face face, FT_UInt num_coords, FT_Fixed *coords
) {
...
static FT_Error cff_get_var_design(
    CFF_Face face, FT_UInt num_coords, FT_Fixed *coords
) {
...
static FT_Error cff_set_instance(
    CFF_Face face, FT_UInt instance_index
) {
...
static FT_Error cff_hadvance_adjust(
    CFF_Face face, FT_UInt gindex, FT_Int *avalue
) {
...
FT_DEFINE_SERVICEDESCREC10(cff_services,
                           ...
                           &cff_service_metrics_variations, FT_SERVICE_ID_POSTSCRIPT_INFO, &cff_service_ps_info,
                           FT_SERVICE_ID_POSTSCRIPT_FONT_NAME, &cff_service_ps_name, FT_SERVICE_ID_GLYPH_DICT,
                           &cff_service_glyph_dict, FT_SERVICE_ID_TT_CMAP, &cff_service_get_cmap_info,
                           FT_SERVICE_ID_CID, &cff_service_cid_info, FT_SERVICE_ID_PROPERTIES, &cff_service_properties,
...
FT_CALLBACK_DEF(FT_Module_Interface) cff_get_interface(
    FT_Module driver,       /* CFF_Driver */
