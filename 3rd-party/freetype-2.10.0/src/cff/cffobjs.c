...
static PSH_Globals_Funcs cff_size_get_globals_funcs(CFF_Size size) {
    ...
    module = FT_Get_Module(
        size->root.face->driver->root.library, "pshinter"
    );
    return (module && pshinter && pshinter->get_globals_funcs) ? pshinter->get_globals_funcs(module) : 0;
}


FT_LOCAL_DEF(void) cff_size_done(FT_Size cffsize)        /* CFF_Size */
{
...
static void cff_make_private_dict(
    CFF_SubFont subfont, PS_Private priv
) {
...
FT_LOCAL_DEF(FT_Error) cff_size_init(FT_Size cffsize)         /* CFF_Size */
{
    ...
    if (funcs) {
        ...
        error = funcs->create(
            cffsize->face->memory, &priv, &internal->topfont
        );
        if (error)
            goto Exit;
        ...
        for (i = font->num_subfonts; i > 0; i--) {
            ...
            error = funcs->create(
                cffsize->face->memory, &priv, &internal->subfonts[i - 1]
            );
            if (error)
                goto Exit;
        }
...
FT_LOCAL_DEF(FT_Error) cff_size_select(
    ...
    if (funcs) {
        ...
        funcs->set_scale(
            internal->topfont, size->metrics.x_scale, size->metrics.y_scale, 0, 0
        );
        ...
        for (i = font->num_subfonts; i > 0; i--) {
            ...
            funcs->set_scale(
                internal->subfonts[i - 1], x_scale, y_scale, 0, 0
            );
        }
    }
...
FT_LOCAL_DEF(void) cff_slot_done(FT_GlyphSlot slot) {

    slot->internal->glyph_hints = NULL;
}


FT_LOCAL_DEF(FT_Error) cff_slot_init(FT_GlyphSlot slot) {
    ...
    if (pshinter) {
        ...
        module = FT_Get_Module(
            slot->face->driver->root.library, "pshinter"
        );
        if (module) {
...
static FT_String *cff_strcpy(
    FT_Memory memory, const FT_String *source
) {
...
static void remove_style(
    FT_String *family_name, const FT_String *style_name
) {
    ...
    if (family_name_length > style_name_length) {
        ...
        if (idx > style_name_length) {
            ...
            while (idx > 0 && (family_name[idx] == '-' || family_name[idx] == ' ' || family_name[idx] == '_' ||
                               family_name[idx] == '+'))
                idx--;
...
FT_LOCAL_DEF(FT_Error) cff_face_init(
    FT_Stream stream, FT_Face cffface,        /* CFF_Face */
    ...
    if (!error) {
        ...
        if (!error) {
            ...
            /* load font directory */
            error = sfnt->load_face(
                stream, face, face_index, num_params, params
            );
            if (error)
                goto Exit;
        } else {
    ...
    /* now load and parse the CFF table in the file */
    {
        ...
        face->extra.data = cff;
        error = cff_font_load(
            library, stream, face_index, cff, face, pure_cff, cff2
        );
        if (error)
            goto Exit;
            ...
            if (sub->has_font_matrix) {
                ...
                if (top->has_font_matrix) {
                    ...
                    FT_Matrix_Multiply_Scaled(
                        &top->font_matrix, &sub->font_matrix, scaling
                    );
                    FT_Vector_Transform_Scaled(
                        &sub->font_offset, &top->font_matrix, scaling
                    );

                    sub->units_per_em = (FT_ULong) FT_MulDiv((FT_Long) sub->units_per_em, (FT_Long) top->units_per_em,
                                                             scaling
                    );
                }
            } else {
        ...
        if (pure_cff) {
            ...
            if (!cffface->family_name) {
                cffface->family_name = cff_index_get_name(
                    cff, (FT_UInt) (face_index & 0xFFFF));
                if (cffface->family_name)
                    remove_subset_prefix(cffface->family_name);
            }

            if (cffface->family_name) {
                char *full = cff_index_get_sid_string(
                    cff, dict->full_name
                );
                char *fullp = full;
            ...
            {
                char *weight = cff_index_get_sid_string(
                    cff, dict->weight
                );
        ...
        /* will use it, whatever be in the CFF part of the file.          */
        {
            ...
            error = FT_CMap_New(
                &cff_cmap_unicode_class_rec, NULL, &cmaprec, NULL);
...
FT_LOCAL_DEF(void) cff_face_done(FT_Face cffface)         /* CFF_Face */
{
...
FT_LOCAL_DEF(FT_Error) cff_driver_init(FT_Module module)        /* CFF_Driver */
{
...
FT_LOCAL_DEF(void) cff_driver_done(FT_Module module)        /* CFF_Driver */
{
