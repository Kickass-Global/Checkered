...
FT_LOCAL_DEF(FT_Error) cff_parser_init(
    CFF_Parser parser, FT_UInt code, void *object, FT_Library library, FT_UInt stackSize, FT_UShort num_designs,
    FT_UShort num_axes
) {
...
FT_LOCAL_DEF(void) cff_parser_done(CFF_Parser parser) {
...
/* read an integer */
static FT_Long cff_parse_integer(
...
static const FT_Long power_tens[] = {1L, 10L, 100L, 1000L, 10000L, 100000L, 1000000L, 10000000L, 100000000L,
                                     1000000000L};

/* maximum values allowed for multiplying      */
/* with the corresponding `power_tens' element */
static const FT_Long power_ten_limits[] = {FT_LONG_MAX / 1L, FT_LONG_MAX / 10L, FT_LONG_MAX / 100L, FT_LONG_MAX / 1000L,
                                           FT_LONG_MAX / 10000L, FT_LONG_MAX / 100000L, FT_LONG_MAX / 1000000L,
                                           FT_LONG_MAX / 10000000L, FT_LONG_MAX / 100000000L,
                                           FT_LONG_MAX / 1000000000L,};


/* read a real */
static FT_Fixed cff_parse_real(
    FT_Byte *start, FT_Byte *limit, FT_Long power_ten, FT_Long *scaling
) {
...
FT_LOCAL_DEF(FT_Long) cff_parse_num(
    CFF_Parser parser, FT_Byte **d
) {
...
#else
        return (FT_Short) (
            ((((FT_UInt32) *(d[0] + 1) << 16) | ((FT_UInt32) *(d[0] + 2) << 8) | (FT_UInt32) *(d[0] + 3)) + 0x80U)
                >> 8);
#endif
    } else
...
static FT_Fixed do_fixed(
    CFF_Parser parser, FT_Byte **d, FT_Long scaling
) {
...
/* read a floating point number, either integer or real */
static FT_Fixed cff_parse_fixed(
    CFF_Parser parser, FT_Byte **d
) {
...
static FT_Fixed cff_parse_fixed_scaled(
    CFF_Parser parser, FT_Byte **d, FT_Long scaling
) {
...
static FT_Fixed cff_parse_fixed_dynamic(
    CFF_Parser parser, FT_Byte **d, FT_Long *scaling
) {
...
static FT_Error cff_parse_font_matrix(CFF_Parser parser) {
    ...
    if (parser->top >= parser->stack + 6) {
        ...
        *upm = (FT_ULong) power_tens[-max_scaling];
        ...
        FT_TRACE4((" [%f %f %f %f %f %f]\n", (double) matrix->xx / *upm / 65536, (double) matrix->xy / *upm / 65536,
            (double) matrix->yx / *upm / 65536, (double) matrix->yy / *upm / 65536, (double) offset->x / *upm / 65536,
...
static FT_Error cff_parse_blend(CFF_Parser parser) {
    ...
    if (cff_blend_check_vector(
        blend, priv->vsindex, subFont->lenNDV, subFont->NDV
    )) {
        error = cff_blend_build_vector(
            blend, priv->vsindex, subFont->lenNDV, subFont->NDV
        );
        if (error)
            goto Exit;
...
FT_LOCAL_DEF(FT_Error) cff_parser_run(
    CFF_Parser parser, FT_Byte *start, FT_Byte *limit
) {
