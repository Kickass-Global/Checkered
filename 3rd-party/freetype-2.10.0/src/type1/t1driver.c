...
static FT_Error t1_get_glyph_name(
    T1_Face face, FT_UInt glyph_index, FT_Pointer buffer, FT_UInt buffer_max
) {
...
static FT_UInt t1_get_name_index(
    T1_Face face, FT_String *glyph_name
) {
...
static FT_Error t1_ps_get_font_info(
    FT_Face face, PS_FontInfoRec *afont_info
) {
...
static FT_Error t1_ps_get_font_extra(
    FT_Face face, PS_FontExtraRec *afont_extra
) {
...
static FT_Error t1_ps_get_font_private(
    FT_Face face, PS_PrivateRec *afont_private
) {
    ...
    switch (key) {
        ...
        case PS_DICT_CHAR_STRING:
            if (idx < (FT_UInt) type1->num_glyphs) {
                ...
                if (value && value_len >= retval) {
                    ft_memcpy(
                        value, (void *) (type1->charstrings[idx]), retval - 1
                    );
                    ((FT_Char *) value)[retval - 1] = (FT_Char) '\0';
                }
            }
        ...
        case PS_DICT_ENCODING_ENTRY:
            if (type1->encoding_type == T1_ENCODING_TYPE_ARRAY && idx < (FT_UInt) type1->encoding.num_chars) {
                ...
                if (value && value_len >= retval) {
                    ft_memcpy(
                        value, (void *) (type1->encoding.char_name[idx]), retval - 1
                    );
                    ((FT_Char *) value)[retval - 1] = (FT_Char) '\0';
                }
            }
        ...
        case PS_DICT_FAMILY_NAME:
            if (type1->font_info.family_name) {
                ...
                if (value && value_len >= retval)
                    ft_memcpy(
                        value, (void *) (type1->font_info.family_name), retval
                    );
            }
            break;
...
#ifndef T1_CONFIG_OPTION_NO_AFM
static const FT_Service_KerningRec t1_service_kerning = {T1_Get_Track_Kerning,       /* get_track */
};
#endif
...
FT_CALLBACK_DEF(FT_Module_Interface) Get_Interface(
    FT_Module module, const FT_String *t1_interface
) {
