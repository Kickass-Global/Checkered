...
FT_LOCAL_DEF(void) T1_Size_Done(FT_Size t1size)          /* T1_Size */
{
...
FT_LOCAL_DEF(FT_Error) T1_Size_Init(FT_Size t1size)      /* T1_Size */
{
    ...
    if (funcs) {
        ...
        error = funcs->create(
            size->root.face->memory, &face->type1.private_dict, &globals
        );
        if (!error)
...
FT_LOCAL_DEF(FT_Error) T1_Size_Request(
    ...
    if (funcs)
        funcs->set_scale((PSH_Globals) t1size->internal->module_data, size->root.metrics.x_scale,
                         size->root.metrics.y_scale, 0, 0
        );
...
FT_LOCAL_DEF(void) T1_GlyphSlot_Done(FT_GlyphSlot slot) {

    slot->internal->glyph_hints = NULL;
}


FT_LOCAL_DEF(FT_Error) T1_GlyphSlot_Init(FT_GlyphSlot slot) {
 ...
 */
FT_LOCAL_DEF(void) T1_Face_Done(FT_Face t1face)         /* T1_Face */
{
 ...
 */
FT_LOCAL_DEF(FT_Error) T1_Face_Init(
    FT_Stream stream, FT_Face t1face,          /* T1_Face */
    ...
    face->pshinter = FT_Get_Module_Interface(
    ...
    /* set up root face fields */
    {
        ...
        root->face_flags |=
            FT_FACE_FLAG_SCALABLE | FT_FACE_FLAG_HORIZONTAL | FT_FACE_FLAG_GLYPH_NAMES | FT_FACE_FLAG_HINTER;
 ...
 *   FreeType error code.  0 means success.
 */
FT_LOCAL_DEF(FT_Error) T1_Driver_Init(FT_Module module) {
 ...
 */
FT_LOCAL_DEF(void) T1_Driver_Done(FT_Module driver) {
