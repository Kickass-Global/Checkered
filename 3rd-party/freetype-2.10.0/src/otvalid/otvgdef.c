...
static void otv_CaretValue_validate(
    FT_Bytes table, OTV_Validator otvalid
) {
...
static void otv_MarkGlyphSets_validate(
    FT_Bytes table, OTV_Validator otvalid
) {
...
FT_LOCAL_DEF(void) otv_GDEF_validate(
    FT_Bytes table, FT_Bytes gsub, FT_Bytes gpos, FT_UInt glyph_count, FT_Validator ftvalid
) {
    ...
    switch (version) {
        case 0:
            ...
            need_MarkAttachClassDef = FT_BOOL(
                otv_GSUBGPOS_have_MarkAttachmentType_flag(gsub) || otv_GSUBGPOS_have_MarkAttachmentType_flag(gpos));
