...
FT_LOCAL_DEF(FT_ULong) cid_get_offset(
    FT_Byte **start, FT_Byte offsize
) {
...
static FT_Error cid_load_keyword(
    CID_Face face, CID_Loader *loader, const T1_Field keyword
) {
...
FT_CALLBACK_DEF(void) cid_parse_font_matrix(
    CID_Face face, CID_Parser *parser
) {
    ...
    if (parser->num_dict >= 0 && parser->num_dict < face->cid.num_dicts) {
        ...
        FT_TRACE4((" [%f %f %f %f %f %f]\n", (double) temp[0] / 65536 / 1000, (double) temp[1] / 65536 / 1000,
            (double) temp[2] / 65536 / 1000, (double) temp[3] / 65536 / 1000, (double) temp[4] / 65536 / 1000,
...
FT_CALLBACK_DEF(void) parse_fd_array(
    CID_Face face, CID_Parser *parser
) {
...
FT_CALLBACK_DEF(void) parse_expansion_factor(
    CID_Face face, CID_Parser *parser
) {
...
FT_CALLBACK_DEF(void) parse_font_name(
    CID_Face face, CID_Parser *parser
) {
...
static FT_Error cid_parse_dict(
    CID_Face face, CID_Loader *loader, FT_Byte *base, FT_ULong size
) {
                    ...
                    for (;;) {
                        ...
                        if (cur[0] == name[0] && len == ft_strlen((const char *) name)) {
                            ...
                            if (n >= len) {
                                ...
                                parser->root.error = cid_load_keyword(
                                    face, loader, keyword
                                );
...
static void cid_init_loader(
    CID_Loader *loader, CID_Face face
) {
...
static FT_Error cid_hex_to_binary(
    FT_Byte *data, FT_ULong data_len, FT_ULong offset, CID_Face face
) {
...
FT_LOCAL_DEF(FT_Error) cid_face_open(
    CID_Face face, FT_Int face_index
) {
    ...
    error = cid_parser_new(
        parser, face->root.stream, face->root.memory, (PSAux_Service) face->psaux
    );
    if (error)
        goto Exit;

    error = cid_parse_dict(
        face, &loader, parser->postscript, parser->postscript_len
    );
    if (error)
        goto Exit;
    ...
    if (parser->binary_length) {
        ...
        if (FT_ALLOC(face->binary_data, parser->binary_length) || FT_SET_ERROR(cid_hex_to_binary(
            face->binary_data, parser->binary_length, parser->data_offset, face
        )))
            goto Exit;

        FT_Stream_OpenMemory(
            face->cid_stream, face->binary_data, parser->binary_length
        );
        cid->data_offset = 0;
    } else {
    ...
    for (n = 0; n < cid->num_dicts; n++) {
        ...
        if ((FT_Int) dict->num_subrs < 0 ||
            (dict->sd_bytes && dict->num_subrs > (binary_length - dict->subrmap_offset) / (FT_UInt) dict->sd_bytes)) {
