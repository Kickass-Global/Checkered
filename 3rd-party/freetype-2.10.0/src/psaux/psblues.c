...
FT_LOCAL_DEF(void) cf2_blues_init(
    CF2_Blues blues, CF2_Font font
) {
    ...
    cf2_getBlueMetrics(
        decoder, &blues->blueScale, &blues->blueShift, &blues->blueFuzz
    );
    ...
    if (cf2_getLanguageGroup(decoder) == 1 && (numBlueValues == 0 ||
        ...
        blues->emBoxBottomEdge.dsCoord = cf2_fixedRound(FT_MulFix(
            blues->emBoxBottomEdge.csCoord, blues->scale
        )) - CF2_MIN_COUNTER;
        ...
        blues->emBoxTopEdge.dsCoord = cf2_fixedRound(FT_MulFix(
            blues->emBoxTopEdge.csCoord, blues->scale
        )) + CF2_MIN_COUNTER;
    ...
    for (i = 0; i < blues->count; i++) {
        if (blues->zone[i].bottomZone)
            blues->zone[i].dsFlatEdge = cf2_fixedRound(FT_MulFix(
                blues->zone[i].csFlatEdge, blues->scale
            ) - blues->boost);
        else
            blues->zone[i].dsFlatEdge = cf2_fixedRound(FT_MulFix(
                blues->zone[i].csFlatEdge, blues->scale
 ...
 *
 */
FT_LOCAL_DEF(FT_Bool) cf2_blues_capture(
    const CF2_Blues blues, CF2_Hint bottomHintEdge, CF2_Hint topHintEdge
) {
    /* TODO: validate? */
    ...
    for (i = 0; i < blues->count; i++) {
        if (blues->zone[i].bottomZone && cf2_hint_isBottom(bottomHintEdge)) {
            if (SUB_INT32(blues->zone[i].csBottomEdge, csFuzz) <= bottomHintEdge->csCoord &&
                ...
                if (blues->suppressOvershoot)
                    ...
                    dsNew = FT_MIN(cf2_fixedRound(bottomHintEdge->dsCoord),
        ...
        if (!blues->zone[i].bottomZone && cf2_hint_isTop(topHintEdge)) {
            if (SUB_INT32(blues->zone[i].csBottomEdge, csFuzz) <= topHintEdge->csCoord &&
                topHintEdge->csCoord <= ADD_INT32(blues->zone[i].csTopEdge, csFuzz)) {
