...
enum {
    AFM_STREAM_STATUS_NORMAL, AFM_STREAM_STATUS_EOC, AFM_STREAM_STATUS_EOL, AFM_STREAM_STATUS_EOF
};
...
static const char *const afm_key_table[N_AFM_TOKENS] = {"Ascender", "AxisLabel", "AxisType", "B", "BlendAxisTypes",
                                                        "BlendDesignMap", "BlendDesignPositions", "C", "CC", "CH",
                                                        "CapHeight", "CharWidth", "CharacterSet", "Characters",
                                                        "Descender", "EncodingScheme", "EndAxis", "EndCharMetrics",
                                                        "EndComposites", "EndDirection", "EndFontMetrics",
                                                        "EndKernData", "EndKernPairs", "EndTrackKern", "EscChar",
                                                        "FamilyName", "FontBBox", "FontName", "FullName", "IsBaseFont",
                                                        "IsCIDFont", "IsFixedPitch", "IsFixedV", "ItalicAngle", "KP",
                                                        "KPH", "KPX", "KPY", "L", "MappingScheme", "MetricsSets", "N",
                                                        "Notice", "PCC", "StartAxis", "StartCharMetrics",
                                                        "StartComposites", "StartDirection", "StartFontMetrics",
                                                        "StartKernData", "StartKernPairs", "StartKernPairs0",
                                                        "StartKernPairs1", "StartTrackKern", "StdHW", "StdVW",
                                                        "TrackKern", "UnderlinePosition", "UnderlineThickness", "VV",
                                                        "VVector", "Version", "W", "W0", "W0X", "W0Y", "W1", "W1X",
                                                        "W1Y", "WX", "WY", "Weight", "WeightVector", "XHeight"};
...
FT_LOCAL_DEF(FT_Int) afm_parser_read_vals(
    AFM_Parser parser, AFM_Value vals, FT_Int n
) {
...
FT_LOCAL_DEF(char*)afm_parser_next_key(
    AFM_Parser parser, FT_Bool line, FT_Offset *len
) {
...
static AFM_Token afm_tokenize(
    const char *key, FT_Offset len
) {
...
FT_LOCAL_DEF(FT_Error) afm_parser_init(
...
static FT_Error afm_parser_read_int(
    AFM_Parser parser, FT_Int *aint
) {
...
FT_CALLBACK_DEF(int) afm_compare_kern_pairs(
    ...
    while ((key = afm_parser_next_key(parser, 1, &len)) != 0) {
        ...
        switch (token) {
            ...
            case AFM_TOKEN_ENDFONTMETRICS:
                ...
                ft_qsort(
                    fi->KernPairs, fi->NumKernPair, sizeof(AFM_KernPairRec), afm_compare_kern_pairs
                );
                return FT_Err_Ok;
...
static FT_Error afm_parser_skip_section(
    AFM_Parser parser, FT_Int n, AFM_Token end_section
) {
...
FT_LOCAL_DEF(FT_Error) afm_parser_parse(AFM_Parser parser) {
    ...
    while ((key = afm_parser_next_key(parser, 1, &len)) != 0) {
        ...
        switch (afm_tokenize(key, len)) {
            ...
            case AFM_TOKEN_STARTCHARMETRICS: {
                ...
                error = afm_parser_skip_section(
                    parser, n, AFM_TOKEN_ENDCHARMETRICS
                );
                if (error)
                    return error;
            }
