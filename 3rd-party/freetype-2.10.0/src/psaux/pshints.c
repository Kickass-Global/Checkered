...
static CF2_Int cf2_getWindingMomentum(
    CF2_Fixed x1, CF2_Fixed y1, CF2_Fixed x2, CF2_Fixed y2
) {
...
static void cf2_hint_init(
    CF2_Hint hint, const CF2_ArrStack stemHintArray, size_t indexStemHint, const CF2_Font font, CF2_Fixed hintOrigin,
    CF2_Fixed scale, FT_Bool bottom
) {
...
FT_LOCAL_DEF(FT_Bool) cf2_hint_isValid(const CF2_Hint hint) {
...
FT_LOCAL_DEF(FT_Bool) cf2_hint_isTop(const CF2_Hint hint) {

    return FT_BOOL(hint->flags & (CF2_PairTop | CF2_GhostTop));
}


FT_LOCAL_DEF(FT_Bool) cf2_hint_isBottom(const CF2_Hint hint) {
...
FT_LOCAL_DEF(void) cf2_hint_lock(CF2_Hint hint) {

    hint->flags |= CF2_Locked;
}


FT_LOCAL_DEF(void) cf2_hintmap_init(
    CF2_HintMap hintmap, CF2_Font font, CF2_HintMap initialMap, CF2_ArrStack hintMoves, CF2_Fixed scale
) {
...
static CF2_Fixed cf2_hintmap_map(
    CF2_HintMap hintmap, CF2_Fixed csCoord
) {
    ...
    if (hintmap->count == 0 || !hintmap->hinted) {
        ...
        if (i == 0 && csCoord < hintmap->edge[0].csCoord) {
            ...
            return ADD_INT32(FT_MulFix(
                SUB_INT32(
                    csCoord, hintmap->edge[0].csCoord
                ), hintmap->scale
            ...
            return ADD_INT32(FT_MulFix(
                SUB_INT32(
                    csCoord, hintmap->edge[i].csCoord
...
static void cf2_hintmap_adjustHints(CF2_HintMap hintmap) {
    ...
    for (i = 0; i < hintmap->count; i++) {
        ...
        if (!cf2_hint_isLocked(&hintmap->edge[i])) {
            ...
            if (j >= hintmap->count - 1 ||
                hintmap->edge[j + 1].dsCoord >= ADD_INT32(hintmap->edge[j].dsCoord, moveUp + upMinCounter)) {
                /* there is room to move up; is there also room to move down? */
                if (i == 0 ||
                    hintmap->edge[i - 1].dsCoord <= ADD_INT32(hintmap->edge[i].dsCoord, moveDown - downMinCounter)) {
        ...
        if (i > 0) {
            if (hintmap->edge[i].csCoord != hintmap->edge[i - 1].csCoord)
                hintmap->edge[i - 1].scale = FT_DivFix(
                    SUB_INT32(hintmap->edge[i].dsCoord, hintmap->edge[i - 1].dsCoord),
                    SUB_INT32(hintmap->edge[i].csCoord, hintmap->edge[i - 1].csCoord));
        }

        if (isPair) {
            if (hintmap->edge[j].csCoord != hintmap->edge[j - 1].csCoord)
                hintmap->edge[j - 1].scale = FT_DivFix(
                    SUB_INT32(hintmap->edge[j].dsCoord, hintmap->edge[j - 1].dsCoord),
                    SUB_INT32(hintmap->edge[j].csCoord, hintmap->edge[j - 1].csCoord));
...
static void cf2_hintmap_insertHint(
    CF2_HintMap hintmap, CF2_Hint bottomHintEdge, CF2_Hint topHintEdge
) {
    ...
    if (cf2_hintmap_isValid(hintmap->initialHintMap) && !cf2_hint_isLocked(firstHintEdge)) {
        if (isPair) {
            ...
            CF2_Fixed midpoint = cf2_hintmap_map(
                hintmap->initialHintMap, ADD_INT32(secondHintEdge->csCoord, firstHintEdge->csCoord) / 2
            );
            CF2_Fixed halfWidth = FT_MulFix(SUB_INT32(secondHintEdge->csCoord, firstHintEdge->csCoord) / 2,
                                            hintmap->scale
            );
            ...
            firstHintEdge->dsCoord = cf2_hintmap_map(
                hintmap->initialHintMap, firstHintEdge->csCoord
            );
    }
    ...
    /* make room to insert */
    {
        ...
        if (isPair) {
            ...
            FT_TRACE7(("  Inserting hint %.2f (%.2f)\n", secondHintEdge->csCoord / 65536.0, secondHintEdge->dsCoord /
                                                                                            65536.0));
 ...
 *
 */
FT_LOCAL_DEF(void) cf2_hintmap_build(
    CF2_HintMap hintmap, CF2_ArrStack hStemHintArray, CF2_ArrStack vStemHintArray, CF2_HintMask hintMask,
    ...
    if (!initialMap && !cf2_hintmap_isValid(hintmap->initialHintMap)) {
        ...
        cf2_hintmap_build(
            hintmap->initialHintMap, hStemHintArray, vStemHintArray, &tempHintMask, hintOrigin, TRUE
        );
    }
    ...
    if (!cf2_hintmask_isValid(hintMask)) {
        ...
        cf2_hintmask_setAll(
            hintMask, cf2_arrstack_size(hStemHintArray) + cf2_arrstack_size(vStemHintArray));
        if (!cf2_hintmask_isValid(hintMask)) {
            if (font->isT1) {
    ...
    if (font->blues.doEmBoxHints) {
        ...
        cf2_hintmap_insertHint(
            hintmap, &font->blues.emBoxBottomEdge, &dummy
        );
        /* ghost top */
        cf2_hintmap_insertHint(
    ...
    for (i = 0, maskByte = 0x80; i < bitCount; i++) {
        if (maskByte & *maskPtr) {
            ...
            cf2_hint_init(
                &bottomHintEdge, hStemHintArray, i, font, hintOrigin, hintmap->scale, TRUE /* bottom */ );
            cf2_hint_init(
                &topHintEdge, hStemHintArray, i, font, hintOrigin, hintmap->scale, FALSE /* top */ );

            if (cf2_hint_isLocked(&bottomHintEdge) || cf2_hint_isLocked(&topHintEdge) || cf2_blues_capture(
                &font->blues, &bottomHintEdge, &topHintEdge
            )) {
...
FT_LOCAL_DEF(void) cf2_glyphpath_init(
    CF2_GlyphPath glyphpath, CF2_Font font, CF2_OutlineCallbacks callbacks, CF2_Fixed scaleY,
    /* CF2_Fixed  hShift, */
    CF2_ArrStack hStemHintArray, CF2_ArrStack vStemHintArray, CF2_HintMask hintMask, CF2_Fixed hintOriginY,
    const CF2_Blues blues, const FT_Vector *fractionalTranslation
) {
    ...
    cf2_arrstack_init(
        &glyphpath->hintMoves, font->memory, &font->error, sizeof(CF2_HintMoveRec));

    cf2_hintmap_init(
        &glyphpath->initialHintMap, font, &glyphpath->initialHintMap, &glyphpath->hintMoves, scaleY
    );
    cf2_hintmap_init(
        &glyphpath->firstHintMap, font, &glyphpath->initialHintMap, &glyphpath->hintMoves, scaleY
    );
    cf2_hintmap_init(
        &glyphpath->hintMap, font, &glyphpath->initialHintMap, &glyphpath->hintMoves, scaleY
    );
...
FT_LOCAL_DEF(void) cf2_glyphpath_finalize(CF2_GlyphPath glyphpath) {
 ...
 */
static void cf2_glyphpath_hintPoint(
    CF2_GlyphPath glyphpath, CF2_HintMap hintmap, FT_Vector *ppt, CF2_Fixed x, CF2_Fixed y
) {
    ...
    ppt->x = ADD_INT32(FT_MulFix(glyphpath->font->outerTransform.a, pt.x), ADD_INT32(
        FT_MulFix(glyphpath->font->outerTransform.c, pt.y), glyphpath->fractionalTranslation.x
    ));
    ppt->y = ADD_INT32(FT_MulFix(glyphpath->font->outerTransform.b, pt.x), ADD_INT32(
        FT_MulFix(glyphpath->font->outerTransform.d, pt.y), glyphpath->fractionalTranslation.y
    ));
}
...
static FT_Bool cf2_glyphpath_computeIntersection(
    CF2_GlyphPath glyphpath, const FT_Vector *u1, const FT_Vector *u2, const FT_Vector *v1, const FT_Vector *v2,
...
#define CF2_CS_SCALE(x)         \
    ...
    if (u1->x == u2->x && cf2_fixedAbs(SUB_INT32(
        intersection->x, u1->x
    )) < glyphpath->snapThreshold)
        intersection->x = u1->x;
    if (u1->y == u2->y && cf2_fixedAbs(SUB_INT32(
        intersection->y, u1->y
    )) < glyphpath->snapThreshold)
        intersection->y = u1->y;

    if (v1->x == v2->x && cf2_fixedAbs(SUB_INT32(
        intersection->x, v1->x
    )) < glyphpath->snapThreshold)
        intersection->x = v1->x;
    if (v1->y == v2->y && cf2_fixedAbs(SUB_INT32(
        intersection->y, v1->y
    )) < glyphpath->snapThreshold)
        intersection->y = v1->y;
 ...
 *
 */
static void cf2_glyphpath_pushPrevElem(
    CF2_GlyphPath glyphpath, CF2_HintMap hintmap, FT_Vector *nextP0, FT_Vector nextP1, FT_Bool close
) {
    ...
    switch (glyphpath->prevElemOp) {
        case CF2_PathOpLineTo:
            ...
            if (close) {
                ...
                cf2_glyphpath_hintPoint(
                    glyphpath, &glyphpath->firstHintMap, &params.pt1, glyphpath->prevElemP1.x, glyphpath->prevElemP1.y
                );
            } else {
                cf2_glyphpath_hintPoint(
                    glyphpath, hintmap, &params.pt1, glyphpath->prevElemP1.x, glyphpath->prevElemP1.y
                );
            }
        ...
        case CF2_PathOpCubeTo:
            ...
            cf2_glyphpath_hintPoint(
                glyphpath, hintmap, &params.pt2, glyphpath->prevElemP2.x, glyphpath->prevElemP2.y
            );
            cf2_glyphpath_hintPoint(
                glyphpath, hintmap, &params.pt3, glyphpath->prevElemP3.x, glyphpath->prevElemP3.y
            );
    ...
    if (!useIntersection || close) {
        ...
        if (close) {
            ...
            /* hint zone                                                      */
            cf2_glyphpath_hintPoint(
                glyphpath, &glyphpath->firstHintMap, &params.pt1, nextP0->x, nextP0->y
            );
        } else {
            cf2_glyphpath_hintPoint(
                glyphpath, hintmap, &params.pt1, nextP0->x, nextP0->y
            );
        }
...
/* element                                                            */
static void cf2_glyphpath_pushMove(
    CF2_GlyphPath glyphpath, FT_Vector start
) {
    ...
    if (!cf2_hintmap_isValid(&glyphpath->hintMap)) {
        ...
        cf2_glyphpath_moveTo(
            glyphpath, glyphpath->start.x, glyphpath->start.y
        );
    }

    cf2_glyphpath_hintPoint(
        glyphpath, &glyphpath->hintMap, &params.pt1, start.x, start.y
    );
 ...
 */
static void cf2_glyphpath_computeOffset(
    CF2_GlyphPath glyphpath, CF2_Fixed x1, CF2_Fixed y1, CF2_Fixed x2, CF2_Fixed y2, CF2_Fixed *x, CF2_Fixed *y
) {
    ...
    *x = *y = 0;
    ...
    glyphpath->callbacks->windingMomentum = ADD_INT32(glyphpath->callbacks->windingMomentum,
 ...
 *
 */
FT_LOCAL_DEF(void) cf2_glyphpath_moveTo(
    CF2_GlyphPath glyphpath, CF2_Fixed x, CF2_Fixed y
) {
    ...
    if (!cf2_hintmap_isValid(&glyphpath->hintMap) || cf2_hintmask_isNew(glyphpath->hintMask))
        cf2_hintmap_build(
            &glyphpath->hintMap, glyphpath->hStemHintArray, glyphpath->vStemHintArray, glyphpath->hintMask,
...
FT_LOCAL_DEF(void) cf2_glyphpath_lineTo(
    CF2_GlyphPath glyphpath, CF2_Fixed x, CF2_Fixed y
) {
    ...
    cf2_glyphpath_computeOffset(
        glyphpath, glyphpath->currentCS.x, glyphpath->currentCS.y, x, y, &xOffset, &yOffset
    );
    ...
    if (glyphpath->elemIsQueued) {
        ...
        cf2_glyphpath_pushPrevElem(
            glyphpath, &glyphpath->hintMap, &P0, P1, FALSE
        );
    }
...
FT_LOCAL_DEF(void) cf2_glyphpath_curveTo(
    CF2_GlyphPath glyphpath, CF2_Fixed x1, CF2_Fixed y1, CF2_Fixed x2, CF2_Fixed y2, CF2_Fixed x3, CF2_Fixed y3
) {
    ...
    cf2_glyphpath_computeOffset(
        glyphpath, glyphpath->currentCS.x, glyphpath->currentCS.y, x1, y1, &xOffset1, &yOffset1
    );
    cf2_glyphpath_computeOffset(
        glyphpath, x2, y2, x3, y3, &xOffset3, &yOffset3
    );
...
FT_LOCAL_DEF(void) cf2_glyphpath_closeOpenPath(CF2_GlyphPath glyphpath) {
    ...
    if (glyphpath->pathIsOpen) {
        ...
        if (glyphpath->elemIsQueued)
            cf2_glyphpath_pushPrevElem(
                glyphpath, &glyphpath->hintMap, &glyphpath->offsetStart0, glyphpath->offsetStart1, TRUE
            );
