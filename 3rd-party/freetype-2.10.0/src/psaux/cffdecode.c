 ...
 *   The bias value.
 */
static FT_Int cff_compute_bias(
    FT_Int in_charstring_type, FT_UInt num_subrs
) {
...
FT_LOCAL_DEF(FT_Int) cff_lookup_glyph_by_stdcharcode(
    CFF_Font cff, FT_Int charcode
) {
 ...
 *     The hinting mode.
 */
FT_LOCAL_DEF(void) cff_decoder_init(
    CFF_Decoder *decoder, TT_Face face, CFF_Size size, CFF_GlyphSlot slot, FT_Bool hinting, FT_Render_Mode hint_mode,
...
FT_LOCAL_DEF(FT_Error) cff_decoder_prepare(
    ...
    /* manage CID fonts */
    if (cff->num_subfonts) {
        FT_Byte fd_index = cffload->fd_select_get(
