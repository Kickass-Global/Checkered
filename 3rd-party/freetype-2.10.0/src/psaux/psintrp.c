...
FT_LOCAL_DEF(void) cf2_hintmask_init(
    CF2_HintMask hintmask, FT_Error *error
) {
...
FT_LOCAL_DEF(FT_Bool) cf2_hintmask_isValid(const CF2_HintMask hintmask) {

    return hintmask->isValid;
}


FT_LOCAL_DEF(FT_Bool) cf2_hintmask_isNew(const CF2_HintMask hintmask) {

    return hintmask->isNew;
}


FT_LOCAL_DEF(void) cf2_hintmask_setNew(
    CF2_HintMask hintmask, FT_Bool val
) {
...
FT_LOCAL_DEF(FT_Byte*)cf2_hintmask_getMaskPtr(CF2_HintMask hintmask) {

    return hintmask->mask;
}


static size_t cf2_hintmask_setCounts(
    CF2_HintMask hintmask, size_t bitCount
) {
...
/* pointer                                                           */
static void cf2_hintmask_read(
    CF2_HintMask hintmask, CF2_Buffer charstring, size_t bitCount
) {
...
FT_LOCAL_DEF(void) cf2_hintmask_setAll(
    CF2_HintMask hintmask, size_t bitCount
) {
...
static void cf2_doStems(
    const CF2_Font font, CF2_Stack opStack, CF2_ArrStack stemHintArray, CF2_Fixed *width, FT_Bool *haveWidth,
...
static void cf2_doFlex(
    CF2_Stack opStack, CF2_Fixed *curX, CF2_Fixed *curY, CF2_GlyphPath glyphPath, const FT_Bool *readFromStack,
    ...
    for (i = 0; i < top; i++) {
        vals[i + 2] = vals[i];
        if (readFromStack[i])
            vals[i + 2] = ADD_INT32(vals[i + 2], cf2_stack_getReal(
                opStack, idx++
            ));
    }
    ...
    for (j = 0; j < 2; j++)
        cf2_glyphpath_curveTo(
            glyphPath, vals[j * 6 + 2], vals[j * 6 + 3], vals[j * 6 + 4], vals[j * 6 + 5], vals[j * 6 + 6],
            vals[j * 6 + 7]
        );
...
static void cf2_doBlend(
    const CFF_Blend blend, CF2_Stack opStack, CF2_UInt numBlends
) {
    ...
    for (i = 0; i < numBlends; i++) {
        ...
        for (j = 1; j < blend->lenBV; j++)
            sum = ADD_INT32(sum, FT_MulFix(
                *weight++, cf2_stack_getReal(
                    opStack, delta++
                )));
...
/*
   ...
   *
   */
FT_LOCAL_DEF(void) cf2_interpT2CharString(
    CF2_Font font, CF2_Buffer buf, CF2_OutlineCallbacks callbacks, const FT_Vector *translation, FT_Bool doingSeac,
...
#define PS_STORAGE_SIZE 3
    ...
    cf2_glyphpath_init(
        &glyphPath, font, callbacks, scaleY,
        /* hShift, */
    ...
    while (1) {
        ...
        if (font->isT1) {
            if (!initial_map_ready &&
                !(op1 == cf2_cmdHSTEM || op1 == cf2_cmdVSTEM || op1 == cf2_cmdHSBW || op1 == cf2_cmdCALLSUBR ||
                  op1 == cf2_cmdRETURN || op1 == cf2_cmdESC || op1 == cf2_cmdENDCHAR || op1 >= 32 /* Numbers */ )) {
            ...
            if (result_cnt > 0 &&
                !(op1 == cf2_cmdCALLSUBR || op1 == cf2_cmdRETURN || op1 == cf2_cmdESC || op1 >= 32 /* Numbers */ )) {
        ...
        switch (op1) {
            ...
            case cf2_cmdHSTEM:
                ...
                cf2_doStems(
                    font, opStack, &hStemHintArray, width, &haveWidth, font->isT1 ? decoder->builder.left_bearing->y : 0
                );
            ...
            case cf2_cmdVSTEM:
                ...
                /* add left-sidebearing correction in Type 1 mode */
                cf2_doStems(
                    font, opStack, &vStemHintArray, width, &haveWidth, font->isT1 ? decoder->builder.left_bearing->x : 0
                );
            ...
            case cf2_cmdRLINETO: {
                ...
                for (idx = 0; idx < count; idx += 2) {
                    curX = ADD_INT32(curX, cf2_stack_getReal(
                        opStack, idx + 0
                    ));
                    curY = ADD_INT32(curY, cf2_stack_getReal(
                        opStack, idx + 1
                    ));
            ...
            case cf2_cmdCALLSUBR: {
                ...
                switch (op1) {
                    case cf2_cmdCALLGSUBR:
                        ...
                        if (cf2_initGlobalRegionBuffer(
                            decoder, subrNum, charstring
                        )) {
                            lastError = FT_THROW(Invalid_Glyph_Format);
            ...
            case cf2_cmdESC: {
                ...
                switch (op2) {
                    case cf2_escHFLEX: {
                        static const FT_Bool readFromStack[12] = {TRUE /* dx1 */, FALSE /* dy1 */, TRUE /* dx2 */,
                        ...
                        cf2_doFlex(
                            opStack, &curX, &curY, &glyphPath, readFromStack, FALSE /* doConditionalLastRead */ );
                    }
                        continue;

                    case cf2_escFLEX: {
                        static const FT_Bool readFromStack[12] = {TRUE /* dx1 */, TRUE /* dy1 */, TRUE /* dx2 */,
                   ...
                   */
                                {
                                    if (!font->isT1)
                                        ...
                                        cf2_stack_setReal(
                                            opStack, 2, SUB_INT32(SUB_INT32(v1, v0), cf2_stack_getReal(opStack, 1)));
                                        cf2_stack_setReal(
                                            opStack, 4, SUB_INT32(SUB_INT32(v2, v1), cf2_stack_getReal(opStack, 3)));

                                        /* add left-sidebearing correction */
                                        cf2_doStems(
                                            font, opStack, isV ? &vStemHintArray : &hStemHintArray, width, &haveWidth,
                                ...
                                case cf2_escSEAC:
                                    if (!font->isT1)
                                        ...
                                        if (decoder->builder.no_recurse) {
                                            ...
                                            subg->flags =
                                                FT_SUBGLYPH_FLAG_ARGS_ARE_XY_VALUES | FT_SUBGLYPH_FLAG_USE_MY_METRICS;
                                            subg->arg1 = 0;
                                            subg->arg2 = 0;
                                            subg++;
                                        ...
                                        error2 = cf2_getT1SeacComponent(
                                            decoder, (FT_UInt) bchar_index, &component
                                        );
                                        if (error2) {
                                            lastError = error2; /* pass FreeType error through */
                                            goto exit;
                                        ...
                                        cf2_interpT2CharString(
                                            font, &component, callbacks, translation, TRUE, 0, 0, &dummyWidth
                                        );
                                        ...
                                        error2 = cf2_getT1SeacComponent(
                                            decoder, (FT_UInt) achar_index, &component
                                        );
                                        if (error2) {
                                            lastError = error2; /* pass FreeType error through */
                                            goto exit;
                                        ...
                                        cf2_interpT2CharString(
                                            font, &component, callbacks, translation, TRUE, adx - asb, ady, &dummyWidth
                                        );
                                        cf2_freeT1SeacComponent(decoder, &component);
                     ...
                     */
                                        ...
                                        switch (subr_no) {
                                            case 0:                     /* end flex feature */
                                                ...
                                                if (initial_map_ready &&
                                                    (!decoder->flex_state || decoder->num_flex_vectors != 7)) {
                                                    FT_ERROR(("cf2_interpT2CharString (Type 1 mode):"
                                                              " unexpected flex end\n"));
                                                    lastError = FT_THROW(Invalid_Glyph_Format);
                                                    goto exit;
                                            ...
                                            case 2:                     /* add flex vectors */
                                            {
                                                ...
                                                if (idx > 0 && idx < 7) {
                                                    ...
                                                    if (idx == 3 || idx == 6)
                                                        cf2_glyphpath_curveTo(
                                                            &glyphPath, flexStore[0], flexStore[1], flexStore[2],
                                                            flexStore[3], flexStore[4], flexStore[5]
                                                        );
                                                }
                                            }
                                                break;
                                            ...
                                            case 18:                    /* multiple masters */
                                            {
                                                ...
                                                for (nn = 0; nn < num_points; nn++) {
                                                    CF2_Fixed tmp = cf2_stack_getReal(
                                                        opStack, values
                                                    );


                                                    for (mm = 1; mm < blend->num_designs; mm++)
                                                        tmp = ADD_INT32(tmp, FT_MulFix(
                                                            cf2_stack_getReal(
                                                                opStack, delta++
                                                ...
                                                cf2_stack_pop(
                                                    opStack, (CF2_UInt) arg_cnt - num_points
                                                );
                                                ...
                                                /*     WeightVector                         */
                                            {
                                                ...
                                                ft_memcpy(
                                                    &decoder->buildchar[idx], blend->weight_vector,
                      ...
                      break;
                                                ...
                                                /*     onto stack                             */
                                            {
                                                ...
                                                decoder->current_subfont->random = cff_random(
            ...
            case cf2_cmdENDCHAR:
                ...
                if (!font->isCFF2 && !font->isT1 && cf2_stack_count(opStack) > 1) {
                    ...
                    cf2_interpT2CharString(
                        font, &component, callbacks, translation, TRUE, curX, curY, &dummyWidth
                    );
            ...
            case cf2_cmdHINTMASK:
                ...
                cf2_doStems(
                    font, opStack, &vStemHintArray, width, &haveWidth, 0
                );
                ...
                if (op1 == cf2_cmdHINTMASK) {
                    ...
                    cf2_hintmask_read(
                        &hintMask, charstring, cf2_arrstack_size(&hStemHintArray) + cf2_arrstack_size(&vStemHintArray));
                } else {
                    /*
           ...
           */
                    ...
                    cf2_hintmap_init(
                        &counterHintMap, font, &glyphPath.initialHintMap, &glyphPath.hintMoves, scaleY
                    );
                    cf2_hintmask_init(&counterMask, error);

                    cf2_hintmask_read(
                        &counterMask, charstring,
                        cf2_arrstack_size(&hStemHintArray) + cf2_arrstack_size(&vStemHintArray));
                    cf2_hintmap_build(
                        &counterHintMap, &hStemHintArray, &vStemHintArray, &counterMask, 0, FALSE
                    );
                }
                break;
