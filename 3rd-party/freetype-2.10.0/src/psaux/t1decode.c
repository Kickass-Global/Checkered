...
static const FT_Int t1_args_count[op_max] = {0, /* none */
                                             0, /* endchar */
                                             2, /* hsbw */
                                             5, /* seac */
                                             4, /* sbw */
 ...
 *   glyph wasn't found.
 */
FT_LOCAL_DEF(FT_Int) t1_lookup_glyph_by_stdcharcode_ps(
    PS_Decoder *decoder, FT_Int charcode
) {
 ...
 *   FreeType error code.  0 means success.
 */
FT_LOCAL_DEF(FT_Error) t1_decoder_parse_metrics(
    ...
    while (ip < limit) {
        ...
        switch (*ip++) {
            ...
            case 255:    /* four bytes integer */
                ...
                value = (FT_Int32) (((FT_UInt32) ip[0] << 24) | ((FT_UInt32) ip[1] << 16) | ((FT_UInt32) ip[2] << 8) |
                                    (FT_UInt32) ip[3]);
                ip += 4;
...
FT_LOCAL_DEF(FT_Error) t1_decoder_init(
    T1_Decoder decoder, FT_Face face, FT_Size size, FT_GlyphSlot slot, FT_Byte **glyph_names, PS_Blend blend,
    FT_Bool hinting, FT_Render_Mode hint_mode, T1_Decoder_Callback parse_callback
) {
...
/* finalize T1 decoder */
FT_LOCAL_DEF(void) t1_decoder_done(T1_Decoder decoder) {
