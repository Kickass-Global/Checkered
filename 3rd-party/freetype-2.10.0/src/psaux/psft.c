 ...
 *
 */
static FT_Error cf2_checkTransform(
    const CF2_Matrix *transform, CF2_Int unitsPerEm
) {
...
static void cf2_setGlyphWidth(
    CF2_Outline outline, CF2_Fixed width
) {
...
static void cf2_builder_moveTo(
    CF2_OutlineCallbacks callbacks, const CF2_CallbackParams params
) {
...
static void cf2_builder_lineTo(
    CF2_OutlineCallbacks callbacks, const CF2_CallbackParams params
) {
    ...
    if (!builder->path_begun) {
        ...
        /* `path_begun'                                               */
        error = ps_builder_start_point(
            builder, params->pt0.x, params->pt0.y
        );
        if (error) {
            if (!*callbacks->error)
    ...
    error = ps_builder_add_point1(
        builder, params->pt1.x, params->pt1.y
    );
    if (error) {
        if (!*callbacks->error)
            *callbacks->error = error;
...
static void cf2_builder_cubeTo(
    CF2_OutlineCallbacks callbacks, const CF2_CallbackParams params
) {
    ...
    ps_builder_add_point(
        builder, params->pt1.x, params->pt1.y, 0
    );
    ps_builder_add_point(
        builder, params->pt2.x, params->pt2.y, 0
    );
    ps_builder_add_point(
        builder, params->pt3.x, params->pt3.y, 1
    );
}


static void cf2_outline_init(
    CF2_Outline outline, FT_Memory memory, FT_Error *error
) {
...
static void cf2_getScaleAndHintFlag(
    PS_Decoder *decoder, CF2_Fixed *x_scale, CF2_Fixed *y_scale, FT_Bool *hinted, FT_Bool *scaled
) {
...
FT_LOCAL_DEF(FT_Error) cf2_decoder_parse_charstrings(
...

    {
        ...
        cf2_getScaleAndHintFlag(
            decoder, &transform.a, &transform.d, &hinted, &scaled
        );
...
FT_LOCAL_DEF(CFF_SubFont) cf2_getSubfont(PS_Decoder *decoder) {
...
FT_LOCAL_DEF(CFF_VStore) cf2_getVStore(PS_Decoder *decoder) {
...
FT_LOCAL_DEF(FT_UInt) cf2_getMaxstack(PS_Decoder *decoder) {
...
FT_LOCAL_DEF(FT_Error) cf2_getNormalizedVector(
    PS_Decoder *decoder, CF2_UInt *len, FT_Fixed **vec
) {
...
FT_LOCAL_DEF(CF2_Fixed) cf2_getPpemY(PS_Decoder *decoder) {
...
FT_LOCAL_DEF(CF2_Fixed) cf2_getStdVW(PS_Decoder *decoder) {
...
FT_LOCAL_DEF(CF2_Fixed) cf2_getStdHW(PS_Decoder *decoder) {
...
FT_LOCAL_DEF(void) cf2_getBlueMetrics(
    PS_Decoder *decoder, CF2_Fixed *blueScale, CF2_Fixed *blueShift, CF2_Fixed *blueFuzz
) {

    FT_ASSERT(decoder && decoder->current_subfont);

    *blueScale = FT_DivFix(
        decoder->current_subfont->private_dict.blue_scale, cf2_intToFixed(1000));
...
FT_LOCAL_DEF(void) cf2_getBlueValues(
    PS_Decoder *decoder, size_t *count, FT_Pos **data
) {
...
FT_LOCAL_DEF(void) cf2_getOtherBlues(
    PS_Decoder *decoder, size_t *count, FT_Pos **data
) {
...
FT_LOCAL_DEF(void) cf2_getFamilyBlues(
    PS_Decoder *decoder, size_t *count, FT_Pos **data
) {
...
FT_LOCAL_DEF(void) cf2_getFamilyOtherBlues(
    PS_Decoder *decoder, size_t *count, FT_Pos **data
) {
...
FT_LOCAL_DEF(CF2_Int) cf2_getLanguageGroup(PS_Decoder *decoder) {
...
/* return 0 on success                                   */
FT_LOCAL_DEF(CF2_Int) cf2_initGlobalRegionBuffer(
    PS_Decoder *decoder, CF2_Int subrNum, CF2_Buffer buf
) {
...
FT_LOCAL_DEF(FT_Error) cf2_getSeacComponent(
    PS_Decoder *decoder, CF2_Int code, CF2_Buffer buf
) {
...
FT_LOCAL_DEF(void) cf2_freeSeacComponent(
    PS_Decoder *decoder, CF2_Buffer buf
) {

    FT_ASSERT(decoder);
    FT_ASSERT(!decoder->builder.is_t1);

    decoder->free_glyph_callback((TT_Face) decoder->builder.face, (FT_Byte **) &buf->start,
                                 (FT_ULong) (buf->end - buf->start));
}


FT_LOCAL_DEF(FT_Error) cf2_getT1SeacComponent(
    PS_Decoder *decoder, FT_UInt glyph_index, CF2_Buffer buf
) {
    ...
    /* callback function.                                     */
    if (inc)
        error = inc->funcs->get_glyph_data(
            inc->object, glyph_index, &glyph_data
        );
    else
#endif
...
FT_LOCAL_DEF(void) cf2_freeT1SeacComponent(
    PS_Decoder *decoder, CF2_Buffer buf
) {
...
FT_LOCAL_DEF(CF2_Int) cf2_initLocalRegionBuffer(
    PS_Decoder *decoder, CF2_Int subrNum, CF2_Buffer buf
) {
...
FT_LOCAL_DEF(CF2_Fixed) cf2_getDefaultWidthX(PS_Decoder *decoder) {
...
FT_LOCAL_DEF(CF2_Fixed) cf2_getNominalWidthX(PS_Decoder *decoder) {
...
FT_LOCAL_DEF(void) cf2_outline_reset(CF2_Outline outline) {
...
FT_LOCAL_DEF(void) cf2_outline_close(CF2_Outline outline) {
