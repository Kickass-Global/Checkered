...
static voidpf ft_gzip_alloc(
    FT_Memory memory, uInt items, uInt size
) {
...
static void ft_gzip_free(
    FT_Memory memory, voidpf address
) {
...
static FT_Error ft_gzip_file_init(
    FT_GZipFile zip, FT_Stream stream, FT_Stream source
) {
...
static FT_Error ft_gzip_file_fill_input(FT_GZipFile zip) {
    ...
    if (stream->read) {
        size = stream->read(
            stream, stream->pos, zip->input, FT_GZIP_BUFFER_SIZE
        );
        if (size == 0) {
...
static FT_Error ft_gzip_file_skip_output(
    FT_GZipFile zip, FT_ULong count
) {
...
static FT_ULong ft_gzip_file_io(
    FT_GZipFile zip, FT_ULong pos, FT_Byte *buffer, FT_ULong count
) {
...
static unsigned long ft_gzip_stream_io(
...
FT_EXPORT_DEF(FT_Error) FT_Stream_OpenGzip(
    FT_Stream stream, FT_Stream source
) {
...
FT_EXPORT_DEF(FT_Error) FT_Gzip_Uncompress(
    FT_Memory memory, FT_Byte *output, FT_ULong *output_len, const FT_Byte *input, FT_ULong input_len
) {
