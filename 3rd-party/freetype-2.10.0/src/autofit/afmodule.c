...
static FT_Error af_property_get_face_globals(
    FT_Face face, AF_FaceGlobals *aglobals, AF_Module module
) {
...
static FT_Error af_property_set(
    FT_Module ft_module, const char *property_name, const void *value, FT_Bool value_is_string
) {
    ...
    if (!ft_strcmp(property_name, "fallback-script")) {
        ...
        if (x1 < 0 || x2 < 0 || x3 < 0 || x4 < 0 || y1 < 0 || y2 < 0 || y3 < 0 || y4 < 0 || x1 > x2 || x2 > x3 ||
            x3 > x4 || y1 > 500 || y2 > 500 || y3 > 500 || y4 > 500)
            return FT_THROW(Invalid_Argument);
...
static FT_Error af_property_get(
    FT_Module ft_module, const char *property_name, void *value
) {
...
FT_CALLBACK_DEF(FT_Module_Interface) af_get_interface(
...
FT_CALLBACK_DEF(FT_Error) af_autofitter_init(FT_Module ft_module)      /* AF_Module */
{
...
FT_CALLBACK_DEF(void) af_autofitter_done(FT_Module ft_module)      /* AF_Module */
{
...
FT_CALLBACK_DEF(FT_Error) af_autofitter_load_glyph(
    AF_Module module, FT_GlyphSlot slot, FT_Size size, FT_UInt glyph_index, FT_Int32 load_flags
) {
    ...
    error = af_loader_load_glyph(
        loader, module, slot->face, glyph_index, load_flags
    );
