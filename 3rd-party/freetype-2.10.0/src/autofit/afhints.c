...
FT_LOCAL_DEF(FT_Error) af_axis_hints_new_segment(
    AF_AxisHints axis, FT_Memory memory, AF_Segment *asegment
) {
    ...
    if (axis->num_segments < AF_SEGMENTS_EMBEDDED) {
        ...
        if (axis->segments == axis->embedded.segments) {
            if (FT_NEW_ARRAY(axis->segments, new_max))
                goto Exit;
            ft_memcpy(
                axis->segments, axis->embedded.segments, sizeof(axis->embedded.segments));
        } else {
...
FT_LOCAL(FT_Error) af_axis_hints_new_edge(
    AF_AxisHints axis, FT_Int fpos, AF_Direction dir, FT_Bool top_to_bottom_hinting, FT_Memory memory, AF_Edge *anedge
) {
    ...
    if (axis->num_edges < AF_EDGES_EMBEDDED) {
        ...
        if (axis->edges == axis->embedded.edges) {
            if (FT_NEW_ARRAY(axis->edges, new_max))
                goto Exit;
            ft_memcpy(
                axis->edges, axis->embedded.edges, sizeof(axis->embedded.edges));
        } else {
...
FT_LOCAL_DEF(AF_Direction) af_direction_compute(
    FT_Pos dx, FT_Pos dy
) {
...
FT_LOCAL_DEF(void) af_glyph_hints_init(
    AF_GlyphHints hints, FT_Memory memory
) {
...
FT_LOCAL_DEF(void) af_glyph_hints_done(AF_GlyphHints hints) {
...
FT_LOCAL_DEF(void) af_glyph_hints_rescale(
    AF_GlyphHints hints, AF_StyleMetrics metrics
) {
...
FT_LOCAL_DEF(FT_Error) af_glyph_hints_reload(
    AF_GlyphHints hints, FT_Outline *outline
) {
                ...
                if (point->flags & AF_FLAG_CONTROL) {
...

                    {
                        ...
                        if (ft_corner_is_flat(
                            point->fx - prev_v->fx, point->fy - prev_v->fy, next_u->fx - point->fx,
                            next_u->fy - point->fy
                        )) {
...
FT_LOCAL_DEF(void) af_glyph_hints_save(
    AF_GlyphHints hints, FT_Outline *outline
) {
...
FT_LOCAL_DEF(void) af_glyph_hints_align_edge_points(
...
FT_LOCAL_DEF(void) af_glyph_hints_align_strong_points(
    AF_GlyphHints hints, AF_Dimension dim
) {
        ...
        for (point = points; point < point_limit; point++) {
...

            {
                ...
                /* point is not on an edge */
                {
                    ...
                    if (before->scale == 0)
                        before->scale = FT_DivFix(
                            after->pos - before->pos, after->fpos - before->fpos
                        );

                    u = before->pos + FT_MulFix(
                        fu - before->fpos, before->scale
                    );
                }
            }
...
static void af_iup_shift(
    AF_Point p1, AF_Point p2, AF_Point ref
) {
...
static void af_iup_interp(
    AF_Point p1, AF_Point p2, AF_Point ref1, AF_Point ref2
) {
...
FT_LOCAL_DEF(void) af_glyph_hints_align_weak_points(
    AF_GlyphHints hints, AF_Dimension dim
) {
    ...
    for (; contour < contour_limit; contour++) {
        ...
        for (;;) {
            ...
            af_iup_interp(
                last_touched + 1, point - 1, last_touched, point
            );
        }
        ...
        else /* interpolate the last part */
        {
            if (last_touched < end_point)
                af_iup_interp(
                    last_touched + 1, end_point, last_touched, first_touched
                );

            if (first_touched > points)
                af_iup_interp(
                    first_point, first_touched - 1, last_touched, first_touched
                );
        }
...
FT_LOCAL_DEF(void) af_glyph_hints_scale_dim(
    AF_GlyphHints hints, AF_Dimension dim, FT_Fixed scale, FT_Pos delta
) {
