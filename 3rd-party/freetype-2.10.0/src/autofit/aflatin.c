...
FT_LOCAL_DEF(void) af_latin_metrics_init_widths(
    AF_LatinMetrics metrics, FT_Face face
) {
...

    {
       ...
       */
        ...
        GET_UTF8_CHAR( ch, p_old );
            ...
            glyph_index = af_shaper_get_elem(
                &metrics->root, shaper_buf, 0, NULL, NULL);
            if (glyph_index)
                break;
        }
        ...
        for (dim = 0; dim < AF_DIMENSION_MAX; dim++) {
            ...
            error = af_latin_hints_compute_segments(
                hints, (AF_Dimension) dim
            );
            if (error)
                goto Exit;
         ...
         */
            ...
            af_sort_and_quantize_widths(
                &num_widths, axis->widths, dummy->units_per_em / 100
            );
...
static void af_latin_sort_blue(
    FT_UInt count, AF_LatinBlue *table
) {
...
static void af_latin_metrics_init_blues(
    AF_LatinMetrics metrics, FT_Face face
) {
    ...
    for (; bs->string != AF_BLUE_STRING_MAX; bs++) {
        ...
        GET_UTF8_CHAR( ch, p_old );
            ...
            for (i = 0; i < num_idx; i++) {
                ...
                glyph_index = af_shaper_get_elem(
                    &metrics->root, shaper_buf, i, NULL, &y_offset
                );
                if (glyph_index == 0) {
                    FT_TRACE5(("  U+%04lX unavailable\n", ch));
                    continue;
            ...
            if (*a > *b) {
                *a = *b;
                FT_TRACE5(("blue zone overlap:"
                           " adjusting %s %d to %ld\n", a_is_top ? "overshoot" : "reference", blue_sorted[i] -
                                                                                              axis->blues, *a));
            }
        }
    }
...
FT_LOCAL_DEF(void) af_latin_metrics_check_digits(
    AF_LatinMetrics metrics, FT_Face face
) {
    ...
    while (*p) {
        ...
        glyph_index = af_shaper_get_elem(
            &metrics->root, shaper_buf, 0, &advance, NULL);
        if (!glyph_index)
            continue;
...
FT_LOCAL_DEF(FT_Error) af_latin_metrics_init(
    AF_LatinMetrics metrics, FT_Face face
) {
...
static void af_latin_metrics_scale_dim(
    AF_LatinMetrics metrics, AF_Scaler scaler, AF_Dimension dim
) {
                ...
                if (dim == AF_DIMENSION_VERT) {
                    ...
                    if (dist == 0) {
                        FT_TRACE5(("af_latin_metrics_scale_dim:"
                                   ...
                                   "\n", af_style_names[metrics->root.style_class->style], scale / 65536.0, new_scale /
                                                                                                            65536.0,
    ...
    FT_TRACE5(("%s widths (style `%s')\n", dim == AF_DIMENSION_HORZ ? "horizontal"
                                                                    : "vertical", af_style_names[metrics->root.style_class->style]));
...
FT_LOCAL_DEF(void) af_latin_metrics_scale(
    AF_LatinMetrics metrics, AF_Scaler scaler
) {
...
FT_LOCAL_DEF(void) af_latin_get_standard_widths(
    AF_LatinMetrics metrics, FT_Pos *stdHW, FT_Pos *stdVW
) {
...
FT_LOCAL_DEF(FT_Error) af_latin_hints_compute_segments(
    AF_GlyphHints hints, AF_Dimension dim
) {
...
FT_LOCAL_DEF(void) af_latin_hints_link_segments(
...
FT_LOCAL_DEF(FT_Error) af_latin_hints_compute_edges(
    AF_GlyphHints hints, AF_Dimension dim
) {
    ...
    edge_distance_threshold = FT_MulFix(
        laxis->edge_distance_threshold, scale
    );
    if (edge_distance_threshold > 64 / 4)
        edge_distance_threshold = 64 / 4;

    edge_distance_threshold = FT_DivFix(
        edge_distance_threshold, scale
    );
    ...
    for (seg = segments; seg < segment_limit; seg++) {
        ...
        if (!found) {
            ...
            error = af_axis_hints_new_edge(
                axis, seg->pos, (AF_Direction) seg->dir, top_to_bottom_hinting, memory, &edge
            );
            if (error)
                goto Exit;
...
FT_LOCAL_DEF(FT_Error) af_latin_hints_detect_features(
    AF_GlyphHints hints, FT_UInt width_count, AF_WidthRec *widths, AF_Dimension dim
) {
...
static void af_latin_hints_compute_blue_edges(
    AF_GlyphHints hints, AF_LatinMetrics metrics
) {
...
static FT_Error af_latin_hints_init(
    AF_GlyphHints hints, AF_LatinMetrics metrics
) {
...
static FT_Pos af_latin_snap_width(
    AF_Width widths, FT_UInt count, FT_Pos width
) {
...
static FT_Pos af_latin_compute_stem_width(
    AF_GlyphHints hints, AF_Dimension dim, FT_Pos width, FT_Pos base_delta, FT_UInt base_flags, FT_UInt stem_flags
) {
...
static void af_latin_align_linked_edge(
    AF_GlyphHints hints, AF_Dimension dim, AF_Edge base_edge, AF_Edge stem_edge
) {
...
static void af_latin_align_serif_edge(
    AF_GlyphHints hints, AF_Edge base, AF_Edge serif
) {
...
static void af_latin_hint_edges(
    AF_GlyphHints hints, AF_Dimension dim
) {
    ...
    FT_TRACE5(("latin %s edge hinting (style `%s')\n", dim == AF_DIMENSION_VERT ? "horizontal"
                                                                                : "vertical", af_style_names[hints->metrics->style_class->style]));
    ...
    for (edge = edges; edge < edge_limit; edge++) {
        ...
        if (!anchor) {
            ...
            FT_TRACE5(("  ANCHOR: edge %d (opos=%.2f) and %d (opos=%.2f)"
                       " snapped to %.2f and %.2f\n", edge - edges, edge->opos / 64.0, edge2 - edges, edge2->opos /
                                                                                                      64.0, edge->pos /
                                                                                                            64.0,
                edge2->pos / 64.0));
            ...
            if (delta < 64 + 16) {
                ...
                if (before >= edges && before < edge && after < edge_limit && after > edge) {
                    if (after->opos == before->opos)
                        ...
                        edge->pos = before->pos + FT_MulDiv(
                            edge->opos - before->opos, after->pos - before->pos, after->opos - before->opos
                        );

                    FT_TRACE5(("  SERIF_LINK1: edge %d (opos=%.2f) snapped to %.2f"
                               " from %d (opos=%.2f)\n", edge - edges, edge->opos / 64.0, edge->pos / 64.0, before -
                                                                                                            edges,
                        before->opos / 64.0));
                } else {
...
static FT_Error af_latin_hints_apply(
    FT_UInt glyph_index, AF_GlyphHints hints, FT_Outline *outline, AF_LatinMetrics metrics
) {
    ...
    if (AF_HINTS_DO_HORIZONTAL(hints)) {
        ...
        error = af_latin_hints_detect_features(
            hints, axis->width_count, axis->widths, AF_DIMENSION_HORZ
        );
        if (error)
            goto Exit;
    }
    ...
    if (AF_HINTS_DO_VERTICAL(hints)) {
        ...
        error = af_latin_hints_detect_features(
            hints, axis->width_count, axis->widths, AF_DIMENSION_VERT
        );
        if (error)
            goto Exit;
    ...
    for (dim = 0; dim < AF_DIMENSION_MAX; dim++) {
#ifdef AF_CONFIG_OPTION_USE_WARPER
        if (dim == AF_DIMENSION_HORZ && metrics->root.scaler.render_mode == FT_RENDER_MODE_NORMAL &&
            ...
            af_warper_compute(
                &warper, hints, (AF_Dimension) dim, &scale, &delta
            );
            af_glyph_hints_scale_dim(
                hints, (AF_Dimension) dim, scale, delta
            );
            continue;
        }
