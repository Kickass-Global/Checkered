...
FT_LOCAL_ARRAY(AF_WritingSystemClass) af_writing_system_classes[];
...
FT_LOCAL_ARRAY(AF_ScriptClass) af_script_classes[];
...
FT_LOCAL_ARRAY(AF_StyleClass) af_style_classes[];
...
FT_LOCAL(FT_Error) af_face_globals_new(
    FT_Face face, AF_FaceGlobals *aglobals, AF_Module module
);

FT_LOCAL(FT_Error) af_face_globals_get_metrics(
    AF_FaceGlobals globals, FT_UInt gindex, FT_UInt options, AF_StyleMetrics *ametrics
);

FT_LOCAL(void) af_face_globals_free(AF_FaceGlobals globals);
