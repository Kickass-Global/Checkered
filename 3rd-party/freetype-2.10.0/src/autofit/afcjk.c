...
FT_LOCAL_DEF(void) af_cjk_metrics_init_widths(
    AF_CJKMetrics metrics, FT_Face face
) {
...
static void af_cjk_metrics_init_blues(
    AF_CJKMetrics metrics, FT_Face face
) {
...
FT_LOCAL_DEF(void) af_cjk_metrics_check_digits(
    AF_CJKMetrics metrics, FT_Face face
) {
...
FT_LOCAL_DEF(FT_Error) af_cjk_metrics_init(
    AF_CJKMetrics metrics, FT_Face face
) {
...
static void af_cjk_metrics_scale_dim(
    AF_CJKMetrics metrics, AF_Scaler scaler, AF_Dimension dim
) {
    ...
    for (nn = 0; nn < axis->blue_count; nn++) {
        ...
        if (dist <= 48 && dist >= -48) {
            ...
            FT_TRACE5((">> active cjk blue zone %c%d[%ld/%ld]:\n"
                       "     ref:   cur=%.2f fit=%.2f\n"
                       "     shoot: cur=%.2f fit=%.2f\n", (dim == AF_DIMENSION_HORZ) ? 'H'
                                                                                     : 'V', nn, blue->ref.org, blue->shoot.org,
                blue->ref.cur / 64.0, blue->ref.fit / 64.0, blue->shoot.cur / 64.0, blue->shoot.fit / 64.0));
...
FT_LOCAL_DEF(void) af_cjk_metrics_scale(
    AF_CJKMetrics metrics, AF_Scaler scaler
) {
...
FT_LOCAL_DEF(void) af_cjk_get_standard_widths(
    AF_CJKMetrics metrics, FT_Pos *stdHW, FT_Pos *stdVW
) {
...
static FT_Error af_cjk_hints_compute_segments(
...
static FT_Error af_cjk_hints_compute_edges(
    AF_GlyphHints hints, AF_Dimension dim
) {
...
static FT_Error af_cjk_hints_detect_features(
    AF_GlyphHints hints, AF_Dimension dim
) {
...
static void af_cjk_hints_compute_blue_edges(
    AF_GlyphHints hints, AF_CJKMetrics metrics, AF_Dimension dim
) {
...
FT_LOCAL_DEF(FT_Error) af_cjk_hints_init(
    AF_GlyphHints hints, AF_CJKMetrics metrics
) {
...
static FT_Pos af_cjk_snap_width(
...
static FT_Pos af_cjk_compute_stem_width(
    AF_GlyphHints hints, AF_Dimension dim, FT_Pos width, FT_UInt base_flags, FT_UInt stem_flags
) {
...
static void af_cjk_align_linked_edge(
    ...
    FT_TRACE5(("  CJKLINK: edge %d @%d (opos=%.2f) linked to %.2f,"
               " dist was %.2f, now %.2f\n", stem_edge - hints->axis[dim].edges, stem_edge->fpos, stem_edge->opos /
                                                                                                  64.0, stem_edge->pos /
                                                                                                        64.0, dist /
                                                                                                              64.0,
        fitted_width / 64.0));
...
static FT_Pos af_hint_normal_stem(
    AF_GlyphHints hints, AF_Edge edge, AF_Edge edge2, FT_Pos anchor, AF_Dimension dim
) {
    ...
    cur_len = af_cjk_compute_stem_width(
    ...
    for (edge = edges; edge < edge_limit; edge++) {
        ...
        if (dim != AF_DIMENSION_VERT && !anchor) {
            ...
            delta = af_hint_normal_stem(
                hints, edge, edge2, 0, AF_DIMENSION_HORZ
            );
        } else
            ...
            if (before < edges)
                ...
                if (after->fpos == before->fpos)
                    ...
                    edge->pos = before->pos + FT_MulDiv(
                        edge->fpos - before->fpos, after->pos - before->pos, after->fpos - before->fpos
                    );
            }
        }
    }
...
static void af_cjk_align_edge_points(
    AF_GlyphHints hints, AF_Dimension dim
) {
    ...
    snapping = FT_BOOL((dim == AF_DIMENSION_HORZ && AF_LATIN_HINTS_DO_HORZ_SNAP(hints)) ||
...
FT_LOCAL_DEF(FT_Error) af_cjk_hints_apply(
    FT_UInt glyph_index, AF_GlyphHints hints, FT_Outline *outline, AF_CJKMetrics metrics
) {
