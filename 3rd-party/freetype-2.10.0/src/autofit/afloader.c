...
FT_LOCAL_DEF(void) af_loader_init(
    AF_Loader loader, AF_GlyphHints hints
) {
...
FT_LOCAL_DEF(FT_Error) af_loader_reset(
    AF_Loader loader, AF_Module module, FT_Face face
) {
...
FT_LOCAL_DEF(void) af_loader_done(AF_Loader loader) {
...
static FT_Error af_loader_embolden_glyph_in_slot(
    ...
    if (writing_system_class->style_metrics_getstdw)
        writing_system_class->style_metrics_getstdw(
            style_metrics, &stdHW, &stdVW
        );
    else {
    ...
    if (size_changed || (stdVW > 0 && stdVW != globals->standard_vertical_width)) {
        ...
        darken_by_font_units_x = af_intToFixed(af_loader_compute_darkening(
            loader, face, stdVW
        ));
        darken_x = FT_DivFix(
            FT_MulFix(
                darken_by_font_units_x, size_metrics->x_scale
            ), em_ratio
        );
    ...
    if (size_changed || (stdHW > 0 && stdHW != globals->standard_horizontal_width)) {
        ...
        darken_by_font_units_y = af_intToFixed(af_loader_compute_darkening(
            loader, face, stdHW
        ));
        darken_y = FT_DivFix(
            FT_MulFix(
                darken_by_font_units_y, size_metrics->y_scale
            ), em_ratio
        );
    ...
    FT_Outline_EmboldenXY(
        &slot->outline, globals->darken_x, globals->darken_y
    );
...
FT_LOCAL_DEF(FT_Error) af_loader_load_glyph(
    AF_Loader loader, AF_Module module, FT_Face face, FT_UInt glyph_index, FT_Int32 load_flags
) {
     ...
     */
    error = af_face_globals_get_metrics(
        loader->globals, glyph_index, style_options, &style_metrics
    );
    if (error)
        goto Exit;
    ...
    if (writing_system_class->style_hints_init) {
        error = writing_system_class->style_hints_init(
            hints, style_metrics
        );
        if (error)
            goto Exit;
        ...
        case FT_GLYPH_FORMAT_OUTLINE:
            ...
            if (loader->transformed)
                FT_Outline_Translate(
                    &slot->outline, loader->trans_delta.x, loader->trans_delta.y
                );
            ...
            loader->pp2.x = FT_MulFix(
                slot->metrics.horiAdvance, hints->x_scale
            ) + hints->x_delta;
    ...
    Hint_Metrics:
    {
        ...
        if (scaler.render_mode != FT_RENDER_MODE_LIGHT && (FT_IS_FIXED_WIDTH(slot->face) ||
                                                           (af_face_globals_is_digit(loader->globals, glyph_index) &&
                                                            style_metrics->digits_have_same_width))) {
            slot->metrics.horiAdvance = FT_MulFix(
                slot->metrics.horiAdvance, style_metrics->scaler.x_scale
            );
        ...
        slot->metrics.vertAdvance = FT_MulFix(
            slot->metrics.vertAdvance, style_metrics->scaler.y_scale
        );
 ...
 */
FT_LOCAL_DEF(FT_Int32) af_loader_compute_darkening(
    AF_Loader loader, FT_Face face, FT_Pos standard_width
) {
