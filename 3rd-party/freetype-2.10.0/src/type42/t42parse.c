...
static void t42_parse_encoding(
    T42_Face face, T42_Loader loader
);
...
#define T1CODE        T1_FIELD_LOCATION_FONT_INFO
    ...
    T1_FIELD_STRING("version", version, 0) T1_FIELD_STRING("Notice", notice, 0)
    T1_FIELD_STRING("FullName", full_name, 0) T1_FIELD_STRING("FamilyName", family_name, 0)
    T1_FIELD_STRING("Weight", weight, 0) T1_FIELD_NUM   ("ItalicAngle", italic_angle, 0)
    T1_FIELD_BOOL  ("isFixedPitch", is_fixed_pitch, 0) T1_FIELD_NUM   ("UnderlinePosition", underline_position, 0)
...
#define T1CODE        T1_FIELD_LOCATION_FONT_DICT
    ...
    T1_FIELD_KEY  ("FontName", font_name, 0) T1_FIELD_NUM  ("PaintType", paint_type, 0)
    T1_FIELD_NUM  ("FontType", font_type, 0) T1_FIELD_FIXED("StrokeWidth", stroke_width, 0)
...
FT_LOCAL_DEF(FT_Error) t42_parser_init(
    T42_Parser parser, FT_Stream stream, FT_Memory memory, PSAux_Service psaux
) {
...
FT_LOCAL_DEF(void) t42_parser_done(T42_Parser parser) {
...
typedef enum T42_Load_Status_ {
    BEFORE_START, BEFORE_TABLE_DIR, OTHER_TABLES
...
static void t42_parse_charstrings(
    ...
    error = psaux->ps_table_funcs->init(
        code_table, loader->num_glyphs, memory
    );
    if (error)
        goto Fail;

    error = psaux->ps_table_funcs->init(
        name_table, loader->num_glyphs, memory
    );
    if (error)
        goto Fail;
...
static FT_Error t42_load_keyword(
    T42_Face face, T42_Loader loader, T1_Field field
) {
...
FT_LOCAL_DEF(FT_Error) t42_parse_dict(
    T42_Face face, T42_Loader loader, FT_Byte *base, FT_Long size
) {
            ...
            if (len > 0 && len < 22 && parser->root.cursor < limit) {
                ...
                for (i = 0; i < n_keywords; i++) {
                    ...
                    if (cur[0] == name[0] && len == ft_strlen((const char *) name) && ft_memcmp(cur, name, len) == 0) {
                        ...
                        parser->root.error = t42_load_keyword(
                            face, loader, keyword
                        );
                        if (parser->root.error)
...
FT_LOCAL_DEF(void) t42_loader_init(
    T42_Loader loader, T42_Face face
) {
...
FT_LOCAL_DEF(void) t42_loader_done(T42_Loader loader) {
