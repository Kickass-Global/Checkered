...
static FT_Error T42_Open_Face(T42_Face face) {
    ...
    error = t42_parser_init(
        parser, face->root.stream, memory, psaux
    );
    if (error)
        goto Exit;
...
FT_LOCAL_DEF(FT_Error) T42_Face_Init(
    FT_Stream stream, FT_Face t42face,       /* T42_Face */
    ...
    face->psaux = FT_Get_Module_Interface(
    ...
    /* Load the TTF font embedded in the T42 font */
    {
        ...
        args.driver = FT_Get_Module(
            FT_FACE_LIBRARY(face), "truetype"
        );
        ...
        error = FT_Open_Face(
            FT_FACE_LIBRARY(face), &args, 0, &face->ttf_face
        );
    }
...
FT_LOCAL_DEF(void) T42_Face_Done(FT_Face t42face) {
 ...
 */
FT_LOCAL_DEF(FT_Error) T42_Driver_Init(FT_Module module)        /* T42_Driver */
{
...
FT_LOCAL_DEF(void) T42_Driver_Done(FT_Module module) {

    FT_UNUSED(module);
}


FT_LOCAL_DEF(FT_Error) T42_Size_Init(FT_Size size)         /* T42_Size */
{
...
FT_LOCAL_DEF(FT_Error) T42_Size_Request(
...
FT_LOCAL_DEF(FT_Error) T42_Size_Select(
    FT_Size t42size,         /* T42_Size */
...
FT_LOCAL_DEF(void) T42_Size_Done(FT_Size t42size)             /* T42_Size */
{
...
FT_LOCAL_DEF(FT_Error) T42_GlyphSlot_Init(FT_GlyphSlot t42slot)        /* T42_GlyphSlot */
{
...
FT_LOCAL_DEF(void) T42_GlyphSlot_Done(FT_GlyphSlot t42slot)       /* T42_GlyphSlot */
{
...
FT_LOCAL_DEF(FT_Error) T42_GlyphSlot_Load(
    FT_GlyphSlot glyph, FT_Size size, FT_UInt glyph_index, FT_Int32 load_flags
) {
    ...
    error = ttclazz->load_glyph(
        t42slot->ttslot, t42size->ttsize, glyph_index, load_flags | FT_LOAD_NO_BITMAP);
