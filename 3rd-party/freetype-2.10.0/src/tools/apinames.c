...
static void names_add(
    ...
    if (num_names >= max_names) {
        ...
        the_names = (NameRec *) realloc(
            the_names, sizeof(the_names[0]) * max_names
        );
        if (!the_names)
...
static void names_sort(void) {
    ...
    qsort(
        the_names, (size_t) num_names, sizeof(the_names[0]), name_compare
    );
}


static void names_dump(
    FILE *out, OutputFormat format, const char *dll_name
) {
