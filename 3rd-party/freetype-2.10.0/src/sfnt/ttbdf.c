...
FT_LOCAL_DEF(void) tt_face_free_bdf_props(TT_Face face) {
...
static FT_Error tt_face_load_bdf_props(
...
FT_LOCAL_DEF(FT_Error) tt_face_find_bdf_prop(
    TT_Face face, const char *property_name, BDF_PropertyRec *aprop
) {
    ...
    for (; count > 0; count--) {
        ...
        if ((type & 0x10) != 0) {
            ...
            if (name_offset < bdf->strings_size && property_len < bdf->strings_size - name_offset && ft_strncmp(
                property_name, (const char *) bdf->strings + name_offset, bdf->strings_size - name_offset
            ) == 0) {
                switch (type & 0x0F) {
                    case 0x00:  /* string */
                    case 0x01:  /* atoms */
