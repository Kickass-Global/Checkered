...
static FT_String *tt_name_ascii_from_utf16(
    TT_Name entry, FT_Memory memory
) {
...
static FT_String *tt_name_ascii_from_other(
    TT_Name entry, FT_Memory memory
) {
...
FT_LOCAL_DEF(FT_Error) tt_face_get_name(
    TT_Face face, FT_UShort nameid, FT_String **name
) {
    ...
    if (rec && convert) {
        if (!rec->string) {
            ...
            if (FT_QNEW_ARRAY (rec->string, rec->stringLength) || FT_STREAM_SEEK(rec->stringOffset) ||
...
static FT_Encoding sfnt_find_encoding(
...
FT_CALLBACK_DEF(int) compare_offsets(
...
static FT_Error woff_open_font(
    FT_Stream stream, TT_Face face
) {
...
#define FT_STRUCTURE  WOFF_HeaderRec
        ...
        FT_FRAME_START(44), FT_FRAME_ULONG (signature), FT_FRAME_ULONG (flavor), FT_FRAME_ULONG (length),
        FT_FRAME_USHORT(num_tables), FT_FRAME_USHORT(reserved), FT_FRAME_ULONG (totalSfntSize),
        FT_FRAME_USHORT(majorVersion), FT_FRAME_USHORT(minorVersion), FT_FRAME_ULONG (metaOffset),
        FT_FRAME_ULONG (metaLength), FT_FRAME_ULONG (metaOrigLength), FT_FRAME_ULONG (privOffset),
        FT_FRAME_ULONG (privLength), FT_FRAME_END};
    ...
    if (woff.length != stream->size || woff.num_tables == 0 || 44 + woff.num_tables * 20UL >= woff.length ||
        12 + woff.num_tables * 16UL >= woff.totalSfntSize || (woff.totalSfntSize & 3) != 0 ||
        (woff.metaOffset == 0 && (woff.metaLength != 0 || woff.metaOrigLength != 0)) ||
    ...
    for (nn = 0; nn < woff.num_tables; nn++) {
        ...
        FT_TRACE2(("  %c%c%c%c  %08lx  %08lx  %08lx  %08lx\n", (FT_Char) (table->Tag >> 24), (FT_Char) (table->Tag
            >> 16), (FT_Char) (table->Tag
            >> 8), (FT_Char) (table->Tag), table->Offset, table->CompLength, table->OrigLength, table->CheckSum));
    ...
    ft_qsort(
        indices, woff.num_tables, sizeof(WOFF_Table), compare_offsets
    );
    ...
    for (nn = 0; nn < woff.num_tables; nn++) {
        ...
        if (table->Offset != woff_offset || table->CompLength > woff.length ||
            table->Offset > woff.length - table->CompLength || table->OrigLength > woff.totalSfntSize ||
            sfnt_offset > woff.totalSfntSize - table->OrigLength || table->CompLength > table->OrigLength) {
    ...
    for (nn = 0; nn < woff.num_tables; nn++) {
        ...
        if (table->CompLength == table->OrigLength) {
            ...
            ft_memcpy(
                sfnt + table->OrigOffset, stream->cursor, table->OrigLength
            );
        } else {
            ...
            error = FT_Gzip_Uncompress(
                memory, sfnt + table->OrigOffset, &output_len, stream->cursor, table->CompLength
            );
            if (error)
                goto Exit;
    ...
    FT_Stream_Free(
        face->root.stream, (face->root.face_flags & FT_FACE_FLAG_EXTERNAL_STREAM) != 0
    );
...
static FT_Error sfnt_open_font(
    FT_Stream stream, TT_Face face
) {
...
#define FT_STRUCTURE  TTC_HeaderRec
    ...
    if (tag != 0x00010000UL && tag != TTAG_ttcf && tag != TTAG_OTTO && tag != TTAG_true && tag != TTAG_typ1 &&
        tag != TTAG_0xA5kbd && tag != TTAG_0xA5lst && tag != 0x00020000UL) {
...
FT_LOCAL_DEF(FT_Error) sfnt_init_face(
    FT_Stream stream, TT_Face face, FT_Int face_instance_index, FT_Int num_params, FT_Parameter *params
) {
    ...
    if (!face->mm) {
        ...
        face->mm = ft_module_get_service(
            tt_module, FT_SERVICE_ID_MULTI_MASTERS, 0
        );
    }
    ...
    if (!face->var) {
        ...
        face->var = ft_module_get_service(
            tt_module, FT_SERVICE_ID_METRICS_VARIATIONS, 0
        );
    }
#endif
...
#ifdef TT_CONFIG_OPTION_GX_VAR_SUPPORT
    {
        ...
        if (face->goto_table(face, TTAG_fvar, stream, &fvar_len) || fvar_len < 20 || FT_READ_ULONG(version) ||
            FT_READ_USHORT(offset) || FT_STREAM_SKIP(2) /* reserved */                     ||
            FT_READ_USHORT(num_axes) || FT_READ_USHORT(axis_size) || FT_READ_USHORT(num_instances) ||
        ...
        if ((face->variation_support & TT_FACE_FLAG_VAR_FVAR) &&
            !(FT_ALLOC(default_values, num_axes * 4) || FT_ALLOC(instance_values, num_axes * 4))) {
        ...
        if (face->goto_table(face, TTAG_glyf, stream, 0) && face->goto_table(face, TTAG_CFF2, stream, 0) &&
...
FT_LOCAL_DEF(FT_Error) sfnt_load_face(
    FT_Stream stream, TT_Face face, FT_Int face_instance_index, FT_Int num_params, FT_Parameter *params
) {
    ...
    has_outline = FT_BOOL(face->root.internal->incremental_interface || tt_face_lookup_table(face, TTAG_glyf) ||
                          tt_face_lookup_table(face, TTAG_CFF) || tt_face_lookup_table(face, TTAG_CFF2));
#else
    ...
    /* now set up root fields */
    {
         ...
         * Compute face flags.
         */
        if (face->sbit_table_type == TT_SBIT_TABLE_TYPE_CBLC || face->sbit_table_type == TT_SBIT_TABLE_TYPE_SBIX ||
            face->colr)
        ...
        /* set the encoding fields */
        {
            ...
            for (m = 0; m < root->num_charmaps; m++) {
                ...
                charmap->encoding = sfnt_find_encoding(
        ...
        if (FT_IS_SCALABLE(root)) {
            ...
            root->max_advance_height = (FT_Short) (face->vertical_info ? face->vertical.advance_Height_Max
                                                                       : root->height);
...
FT_LOCAL_DEF(void) sfnt_done_face(TT_Face face) {
