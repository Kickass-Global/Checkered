...
FT_LOCAL_DEF(FT_Error) tt_face_load_sbit(
...
FT_LOCAL_DEF(void) tt_face_free_sbit(TT_Face face) {
...
FT_LOCAL_DEF(FT_Error) tt_face_set_sbit_strike(
    TT_Face face, FT_Size_Request req, FT_ULong *astrike_index
) {
...
FT_LOCAL_DEF(FT_Error) tt_face_load_strike_metrics(
    TT_Face face, FT_ULong strike_index, FT_Size_Metrics *metrics
) {
    ...
    switch ((FT_UInt) face->sbit_table_type) {
        ...
        case TT_SBIT_TABLE_TYPE_CBLC: {
            ...
            metrics->x_scale = FT_MulDiv(
                metrics->x_ppem, 64 * 0x10000, face->header.Units_Per_EM
            );
            metrics->y_scale = FT_MulDiv(
                metrics->y_ppem, 64 * 0x10000, face->header.Units_Per_EM
            );
        ...
        case TT_SBIT_TABLE_TYPE_SBIX: {
            ...
            metrics->height = FT_MulDiv(
                hori->Ascender - hori->Descender + hori->Line_Gap, ppem_ * 64, upem
            );
...
static FT_Error tt_sbit_decoder_init(
    TT_SBitDecoder decoder, TT_Face face, FT_ULong strike_index, TT_SBit_MetricsRec *metrics
) {
...
static FT_Error tt_sbit_decoder_alloc_bitmap(
    TT_SBitDecoder decoder, FT_Bool metrics_only
) {
...
static FT_Error tt_sbit_decoder_load_metrics(
...
typedef FT_Error  (*TT_SBitDecoder_LoadFunc)(
    TT_SBitDecoder decoder, FT_Byte *p, FT_Byte *plimit, FT_Int x_pos, FT_Int y_pos, FT_UInt recurse_count
);


static FT_Error tt_sbit_decoder_load_byte_aligned(
    TT_SBitDecoder decoder, FT_Byte *p, FT_Byte *limit, FT_Int x_pos, FT_Int y_pos, FT_UInt recurse_count
) {
...
static FT_Error tt_sbit_decoder_load_bit_aligned(
    TT_SBitDecoder decoder, FT_Byte *p, FT_Byte *limit, FT_Int x_pos, FT_Int y_pos, FT_UInt recurse_count
) {
...
static FT_Error tt_sbit_decoder_load_compound(
    TT_SBitDecoder decoder, FT_Byte *p, FT_Byte *limit, FT_Int x_pos, FT_Int y_pos, FT_UInt recurse_count
) {
    ...
    for (nn = 0; nn < num_components; nn++) {
        ...
        error = tt_sbit_decoder_load_image(
            decoder, gindex, x_pos + dx, y_pos + dy, recurse_count + 1,
...
static FT_Error tt_sbit_decoder_load_bitmap(
    TT_SBitDecoder decoder, FT_UInt glyph_format, FT_ULong glyph_start, FT_ULong glyph_size, FT_Int x_pos, FT_Int y_pos,
...
static FT_Error tt_sbit_decoder_load_image(
    ...
    return tt_sbit_decoder_load_bitmap(
        decoder, image_format, image_start, image_end, x_pos, y_pos, recurse_count, metrics_only
    );
...
static FT_Error tt_face_load_sbix_image(
    TT_Face face, FT_ULong strike_index, FT_UInt glyph_index, FT_Stream stream, FT_Bitmap *map,
    TT_SBit_MetricsRec *metrics, FT_Bool metrics_only
) {
...
FT_LOCAL(FT_Error) tt_face_load_sbit_image(
    TT_Face face, FT_ULong strike_index, FT_UInt glyph_index, FT_UInt load_flags, FT_Stream stream, FT_Bitmap *map,
    ...
    switch ((FT_UInt) face->sbit_table_type) {
        ...
        case TT_SBIT_TABLE_TYPE_CBLC: {
            ...
            if (!error) {
                error = tt_sbit_decoder_load_image(
                    decoder, glyph_index, 0, 0, 0, (load_flags & FT_LOAD_BITMAP_METRICS_ONLY) != 0
                );
        ...
        case TT_SBIT_TABLE_TYPE_SBIX:
            error = tt_face_load_sbix_image(
                face, strike_index, glyph_index, stream, map, metrics, (load_flags & FT_LOAD_BITMAP_METRICS_ONLY) != 0
            );
            break;
    ...
    if (!error && !(load_flags & FT_LOAD_COLOR) && !(load_flags & FT_LOAD_BITMAP_METRICS_ONLY) &&
        map->pixel_mode == FT_PIXEL_MODE_BGRA) {
        FT_Bitmap new_map;
