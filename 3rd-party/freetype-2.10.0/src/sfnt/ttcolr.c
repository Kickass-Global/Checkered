...
FT_LOCAL_DEF(FT_Error) tt_face_load_colr(
...
FT_LOCAL_DEF(void) tt_face_free_colr(TT_Face face) {
...
static FT_Bool find_base_glyph_record(
    FT_Byte *base_glyph_begin, FT_Int num_base_glyph, FT_UInt glyph_id, BaseGlyphRecord *record
) {
...
FT_LOCAL_DEF(FT_Bool) tt_face_get_colr_layer(
    ...
    if (!iterator->p) {
        ...
        if (!find_base_glyph_record(
            colr->base_glyphs, colr->num_base_glyphs, base_glyph, &glyph_record
        ))
            return 0;
    ...
    *acolor_index = FT_NEXT_USHORT(iterator->p);
    ...
    if (*aglyph_index >= (FT_UInt) (FT_FACE(face)->num_glyphs) ||
        (*acolor_index != 0xFFFF && *acolor_index >= face->palette_data.num_palette_entries))
        return 0;
...
FT_LOCAL_DEF(FT_Error) tt_face_colr_blend_layer(
    ...
    if (color_index == 0xFFFF) {
        if (face->have_foreground_color) {
            ...
            if (face->palette_data.palette_flags &&
                (face->palette_data.palette_flags[face->palette_index] & FT_PALETTE_FOR_DARK_BACKGROUND)) {
                /* white opaque */
                b = 0xFF;
                g = 0xFF;
                r = 0xFF;
    ...
    dst = dstSlot->bitmap.buffer + dstSlot->bitmap.pitch * (dstSlot->bitmap_top - srcSlot->bitmap_top) +
