...
/* destroy hints table */
static void ps_hint_table_done(
    PS_Hint_Table table, FT_Memory memory
) {
...
static FT_Error ps_hint_table_ensure(
    PS_Hint_Table table, FT_UInt count, FT_Memory memory
) {
...
static FT_Error ps_hint_table_alloc(
    PS_Hint_Table table, FT_Memory memory, PS_Hint *ahint
) {
...
/* destroy mask */
static void ps_mask_done(
    PS_Mask mask, FT_Memory memory
) {
...
static FT_Error ps_mask_ensure(
    PS_Mask mask, FT_UInt count, FT_Memory memory
) {
...
static FT_Int ps_mask_test_bit(
    PS_Mask mask, FT_Int idx
) {
...
/* clear a given bit */
static void ps_mask_clear_bit(
    PS_Mask mask, FT_UInt idx
) {
...
static FT_Error ps_mask_set_bit(
    PS_Mask mask, FT_UInt idx, FT_Memory memory
) {
...
/* destroy mask table */
static void ps_mask_table_done(
    PS_Mask_Table table, FT_Memory memory
) {
...
static FT_Error ps_mask_table_ensure(
    PS_Mask_Table table, FT_UInt count, FT_Memory memory
) {
...
static FT_Error ps_mask_table_alloc(
    PS_Mask_Table table, FT_Memory memory, PS_Mask *amask
) {
...
static FT_Error ps_mask_table_last(
    PS_Mask_Table table, FT_Memory memory, PS_Mask *amask
) {
...
static FT_Error ps_mask_table_set_bits(
...
static FT_Int ps_mask_table_test_intersect(
    PS_Mask_Table table, FT_UInt index1, FT_UInt index2
) {
...
static FT_Error ps_mask_table_merge(
    PS_Mask_Table table, FT_UInt index1, FT_UInt index2, FT_Memory memory
) {
    ...
    if (index1 < index2 && index2 < table->num_masks) {
        ...
        if (delta > 0) {
            ...
            ft_memmove(
                mask2, mask2 + 1, (FT_UInt) delta * sizeof(PS_MaskRec));
...
/*                                                                 */
static FT_Error ps_mask_table_merge_all(
    PS_Mask_Table table, FT_Memory memory
) {
    ...
    for (index1 = (FT_Int) table->num_masks - 1; index1 > 0; index1--) {
        for (index2 = index1 - 1; index2 >= 0; index2--) {
            if (ps_mask_table_test_intersect(
                table, (FT_UInt) index1, (FT_UInt) index2
            )) {
                error = ps_mask_table_merge(
                    table, (FT_UInt) index2, (FT_UInt) index1, memory
                );
                if (error)
                    goto Exit;
...
static void ps_dimension_done(
    PS_Dimension dimension, FT_Memory memory
) {
...
static void ps_dimension_end_mask(
    PS_Dimension dim, FT_UInt end_point
) {
...
static FT_Error ps_dimension_reset_mask(
...
static FT_Error ps_dimension_set_mask_bits(
    PS_Dimension dim, const FT_Byte *source, FT_UInt source_pos, FT_UInt source_bits, FT_UInt end_point,
    FT_Memory memory
) {
    ...
    error = ps_mask_table_set_bits(
        &dim->masks, source, source_pos, source_bits, memory
    );
...
static FT_Error ps_dimension_add_t1stem(
    PS_Dimension dim, FT_Int pos, FT_Int len, FT_Memory memory, FT_Int *aindex
) {
...
static FT_Error ps_dimension_add_counter(
...
static FT_Error ps_dimension_end(
    PS_Dimension dim, FT_UInt end_point, FT_Memory memory
) {
    /* end hint mask table */
    ps_dimension_end_mask(dim, end_point);
...
/* destroy hints */
FT_LOCAL(void) ps_hints_done(PS_Hints hints) {
...
FT_LOCAL(void) ps_hints_init(
    PS_Hints hints, FT_Memory memory
) {
...
static void ps_hints_open(
    PS_Hints hints, PS_Hint_Type hint_type
) {
...
static void ps_hints_stem(
    PS_Hints hints, FT_UInt dimension, FT_Int count, FT_Long *stems
) {
    ...
    for (; count > 0; count--, stems += 2) {
        ...
        error = ps_dimension_add_t1stem(
            dim, (FT_Int) stems[0], (FT_Int) stems[1], memory, NULL);
        if (error) {
...
static void ps_hints_t1stem3(
    PS_Hints hints, FT_UInt dimension, FT_Fixed *stems
) {
    ...
    if (!hints->error) {
        ...
        if (hints->hint_type == PS_HINT_TYPE_1) {
            ...
            for (count = 0; count < 3; count++, stems += 2) {
                error = ps_dimension_add_t1stem(
                    dim, (FT_Int) FIXED_TO_INT(stems[0]), (FT_Int) FIXED_TO_INT(stems[1]), memory, &idx[count]
                );
                if (error)
                    goto Fail;
            }
            ...
            error = ps_dimension_add_counter(
                dim, idx[0], idx[1], idx[2], memory
            );
            if (error)
                goto Fail;
        } else {
...
static void ps_hints_t1reset(
    PS_Hints hints, FT_UInt end_point
) {
    ...
    if (!hints->error) {
        ...
        if (hints->hint_type == PS_HINT_TYPE_1) {
            error = ps_dimension_reset_mask(
                &hints->dimension[0], end_point, memory
            );
            if (error)
                goto Fail;

            error = ps_dimension_reset_mask(
                &hints->dimension[1], end_point, memory
            );
            if (error)
                goto Fail;
        } else {
...
static void ps_hints_t2mask(
    PS_Hints hints, FT_UInt end_point, FT_UInt bit_count, const FT_Byte *bytes
) {
    ...
    if (!hints->error) {
        ...
        error = ps_dimension_set_mask_bits(
            &dim[0], bytes, count2, count1, end_point, memory
        );
        if (error)
            goto Fail;

        error = ps_dimension_set_mask_bits(
            &dim[1], bytes, 0, count2, end_point, memory
        );
        if (error)
            goto Fail;
    }
...
static void ps_hints_t2counter(
    ...
    if (!hints->error) {
        ...
        /* set-up new horizontal and vertical hint mask now */
        error = ps_dimension_set_mask_bits(
            &dim[0], bytes, 0, count1, 0, memory
        );
        if (error)
            goto Fail;

        error = ps_dimension_set_mask_bits(
            &dim[1], bytes, count1, count2, 0, memory
        );
        if (error)
            goto Fail;
    }
...
static FT_Error ps_hints_close(
    PS_Hints hints, FT_UInt end_point
) {
...
static void t1_hints_stem(
    T1_Hints hints, FT_UInt dimension, FT_Fixed *coords
) {
...
static void t2_hints_stems(
    T2_Hints hints, FT_UInt dimension, FT_Int count, FT_Fixed *coords
) {
