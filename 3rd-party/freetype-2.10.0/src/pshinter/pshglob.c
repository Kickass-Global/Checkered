...
static void psh_globals_scale_widths(
    PSH_Globals globals, FT_UInt direction
) {
...
static void psh_blues_set_zones_0(
    PSH_Blues target, FT_Bool is_others, FT_UInt read_count, FT_Short *read, PSH_Blue_Table top_table,
...
static void psh_blues_set_zones(
    PSH_Blues target, FT_UInt count, FT_Short *blues, FT_UInt count_others, FT_Short *other_blues, FT_Int fuzz,
    FT_Int family
) {
    ...
    psh_blues_set_zones_0(
        target, 0, count, blues, top_table, bot_table
    );
    psh_blues_set_zones_0(
        target, 1, count_others, other_blues, top_table, bot_table
    );
...
static void psh_blues_scale_zones(
    PSH_Blues blues, FT_Fixed scale, FT_Pos delta
) {
...
static FT_Short psh_calc_max_height(
    FT_UInt num, const FT_Short *values, FT_Short cur_max
) {
...
FT_LOCAL_DEF(void) psh_blues_snap_stem(
    PSH_Blues blues, FT_Int stem_top, FT_Int stem_bot, PSH_Alignment alignment
) {
...
static FT_Error psh_globals_new(
    FT_Memory memory, T1_Private *priv, PSH_Globals *aglobals
) {
    ...
    if (!FT_NEW(globals)) {
        ...
        /* copy blue zones */
        psh_blues_set_zones(
            &globals->blues, priv->num_blue_values, priv->blue_values, priv->num_other_blues, priv->other_blues,
            priv->blue_fuzz, 0
        );

        psh_blues_set_zones(
            &globals->blues, priv->num_family_blues, priv->family_blues, priv->num_family_other_blues,
        ...
        /* limit the BlueScale value to `1 / max_of_blue_zone_heights' */
        {
            ...
            max_height = psh_calc_max_height(
                priv->num_blue_values, priv->blue_values, max_height
            );
            max_height = psh_calc_max_height(
                priv->num_other_blues, priv->other_blues, max_height
            );
            max_height = psh_calc_max_height(
                priv->num_family_blues, priv->family_blues, max_height
            );
            max_height = psh_calc_max_height(
                priv->num_family_other_blues, priv->family_other_blues, max_height
            );
...
FT_LOCAL_DEF(void) psh_globals_set_scale(
    PSH_Globals globals, FT_Fixed x_scale, FT_Fixed y_scale, FT_Fixed x_delta, FT_Fixed y_delta
) {
