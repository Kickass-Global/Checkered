...
static FT_Int psh_hint_overlap(
    PSH_Hint hint1, PSH_Hint hint2
) {
...
/* destroy hints table */
static void psh_hint_table_done(
    PSH_Hint_Table table, FT_Memory memory
) {
...
static void psh_hint_table_record(
    PSH_Hint_Table table, FT_UInt idx
) {
...
static void psh_hint_table_record_mask(
    PSH_Hint_Table table, PS_Mask hint_mask
) {
...
/* create hints table */
static FT_Error psh_hint_table_init(
    PSH_Hint_Table table, PS_Hint_Table hints, PS_Mask_Table hint_masks, PS_Mask_Table counter_masks, FT_Memory memory
) {
    ...
    /* allocate our tables */
    if (FT_NEW_ARRAY(table->sort, 2 * count) || FT_NEW_ARRAY(table->hints, count) ||
...
static void psh_hint_table_activate_mask(
    PSH_Hint_Table table, PS_Mask hint_mask
) {
...
static FT_Pos psh_dimension_quantize_len(
    PSH_Dimension dim, FT_Pos len, FT_Bool do_snapping
) {
...
static void psh_hint_align(
    PSH_Hint hint, PSH_Globals globals, FT_Int dimension, PSH_Glyph glyph
) {
    ...
    if (!psh_hint_is_fitted(hint)) {
        ...
        if (dimension == 1)
            psh_blues_snap_stem(
                &globals->blues, ADD_INT(hint->org_pos, hint->org_len), hint->org_pos, &align
            );
...
static void psh_hint_table_align_hints(
    PSH_Hint_Table table, PSH_Globals globals, FT_Int dimension, PSH_Glyph glyph
) {
...
static void psh_glyph_load_points(
...
static void psh_glyph_save_points(
    PSH_Glyph glyph, FT_Int dimension
) {
...
static FT_Error psh_glyph_init(
    PSH_Glyph glyph, FT_Outline *outline, PS_Hints ps_hints, PSH_Globals globals
) {
    ...
    error = psh_hint_table_init(
        &glyph->hint_tables[0], &ps_hints->dimension[0].hints, &ps_hints->dimension[0].masks,
    ...
    error = psh_hint_table_init(
        &glyph->hint_tables[1], &ps_hints->dimension[1].hints, &ps_hints->dimension[1].masks,
...
static void psh_hint_table_find_strong_points(
    PSH_Hint_Table table, PSH_Point point, FT_UInt count, FT_Int threshold, FT_Int major_dir
) {
...
static void psh_glyph_find_strong_points(
    PSH_Glyph glyph, FT_Int dimension
) {
        ...
        for (; num_masks > 1; num_masks--, mask++) {
            ...
            if (next > first) {
                ...
                psh_hint_table_find_strong_points(
                    table, point, count, threshold, major_dir
                );
            }
            first = next;
        }
...
static void psh_glyph_find_blue_points(
    PSH_Blues blues, PSH_Glyph glyph
) {
...
static void psh_glyph_interpolate_strong_points(
    PSH_Glyph glyph, FT_Int dimension
) {
        ...
        if (hint) {
            ...
            if (psh_point_is_edge_min(point))
                ...
                if (delta <= 0)
                    ...
                    point->cur_u = hint->cur_pos + FT_MulDiv(
                        delta, hint->cur_len, hint->org_len
                    );
            }
            psh_point_set_fitted(point);
        }
...
static void psh_glyph_interpolate_normal_points(
    PSH_Glyph glyph, FT_Int dimension
) {
    ...
    for (point = points; point < points_end; point++) {
        ...
        /* find best enclosing point coordinates then interpolate */
        {
            ...
            if (nn == 0)  /* point before the first strong point */
            {
                ...
                point->cur_u = after->cur_u + FT_MulFix(
                    point->org_u - after->org_u, scale
                );
            } else {
                before = strongs[nn - 1];
                ...
                if (nn == num_strongs)  /* point is after last strong point */
                {
                    ...
                    point->cur_u = before->cur_u + FT_MulFix(
                        point->org_u - before->org_u, scale
                    );
                } else {
                    FT_Pos u;
                    ...
                    if (u == before->org_u)
                        ...
                        point->cur_u = before->cur_u + FT_MulDiv(
                            u - before->org_u, after->cur_u - before->cur_u, after->org_u - before->org_u
                        );
                }
            }
            psh_point_set_fitted(point);
...
static void psh_glyph_interpolate_other_points(
    PSH_Glyph glyph, FT_Int dimension
) {
...
FT_Error ps_hints_apply(
    PS_Hints ps_hints, FT_Outline *outline, PSH_Globals globals, FT_Render_Mode hint_mode
) {
     ...
     */
    {
        ...
        for (dimension = 0; dimension < 2; dimension++) {
            ...
            psh_hint_table_align_hints(
                &glyph->hint_tables[dimension], glyph->globals, dimension, glyph
            );
            ...
            if (rescale)
                psh_globals_set_scale(
                    glyph->globals, old_x_scale, old_y_scale, 0, 0
                );
        }
    }
