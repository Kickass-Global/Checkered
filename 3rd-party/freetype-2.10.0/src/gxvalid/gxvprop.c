...
static void gxv_prop_zero_advance_validate(
    ...
    error = FT_Load_Glyph(
        face, gid, FT_LOAD_IGNORE_TRANSFORM);
    if (error)
        FT_INVALID_GLYPH_ID;
...
static void gxv_prop_property_validate(
    FT_UShort property, FT_UShort glyph, GXV_Validator gxvalid
) {
...
static void gxv_prop_LookupValue_validate(
...
FT_LOCAL_DEF(void) gxv_prop_validate(
