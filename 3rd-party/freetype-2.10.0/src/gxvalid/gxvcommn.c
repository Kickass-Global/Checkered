...
static int gxv_compare_ushort_offset(
    FT_UShort *a, FT_UShort *b
) {
...
FT_LOCAL_DEF(void) gxv_set_length_by_ushort_offset(
    FT_UShort *offset, FT_UShort **length, FT_UShort *buff, FT_UInt nmemb, FT_UShort limit, GXV_Validator gxvalid
) {
    ...
    ft_qsort(
        buff, (nmemb + 1), sizeof(FT_UShort), (int (*)(const void *, const void *)) gxv_compare_ushort_offset
    );
...
static int gxv_compare_ulong_offset(
    FT_ULong *a, FT_ULong *b
) {
...
FT_LOCAL_DEF(void) gxv_set_length_by_ulong_offset(
    FT_ULong *offset, FT_ULong **length, FT_ULong *buff, FT_UInt nmemb, FT_ULong limit, GXV_Validator gxvalid
) {
    ...
    ft_qsort(
        buff, (nmemb + 1), sizeof(FT_ULong), (int (*)(const void *, const void *)) gxv_compare_ulong_offset
    );
...
FT_LOCAL_DEF(void) gxv_array_getlimits_byte(
    FT_Bytes table, FT_Bytes limit, FT_Byte *min, FT_Byte *max, GXV_Validator gxvalid
) {
...
FT_LOCAL_DEF(void) gxv_array_getlimits_ushort(
    FT_Bytes table, FT_Bytes limit, FT_UShort *min, FT_UShort *max, GXV_Validator gxvalid
) {
...
static void gxv_BinSrchHeader_check_consistency(
    GXV_BinSrchHeader *binSrchHeader, GXV_Validator gxvalid
) {
    ...
    for (searchRange = 1, entrySelector = 1;
         (searchRange * 2) <= binSrchHeader->nUnits && searchRange < 0x8000U; searchRange *= 2, entrySelector++);
    ...
    if (searchRange != binSrchHeader->searchRange || entrySelector != binSrchHeader->entrySelector ||
...
FT_LOCAL_DEF(void) gxv_BinSrchHeader_validate(
    FT_Bytes table, FT_Bytes limit, FT_UShort *unitSize_p, FT_UShort *nUnits_p, GXV_Validator gxvalid
) {
...
static GXV_LookupValueDesc gxv_lookup_value_load(
    FT_Bytes p, GXV_LookupValue_SignSpec signspec
) {
...
static void gxv_LookupTable_fmt0_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
 ...
 *   -- suzuki toshiya <mpsuzuki@hiroshima-u.ac.jp>
 */
static void gxv_LookupTable_fmt2_skip_endmarkers(
    FT_Bytes table, FT_UShort unitSize, GXV_Validator gxvalid
) {
...
static void gxv_LookupTable_fmt2_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
    ...
    for (unit = 0, gid = 0; unit < nUnits; unit++) {
        ...
        if (lastGlyph < firstGlyph) {
            GXV_TRACE(
                ("reverse ordered range specification at unit %d:", " lastGlyph %d < firstGlyph %d ", unit, lastGlyph, firstGlyph));
            GXV_SET_ERR_IF_PARANOID(FT_INVALID_GLYPH_ID);
...
static void gxv_LookupTable_fmt4_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
...
static void gxv_LookupTable_fmt6_skip_endmarkers(
    FT_Bytes table, FT_UShort unitSize, GXV_Validator gxvalid
) {
...
static void gxv_LookupTable_fmt6_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
...
static void gxv_LookupTable_fmt8_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
...
FT_LOCAL_DEF(void) gxv_LookupTable_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
    ...
    GXV_Validate_Func fmt_funcs_table[] = {gxv_LookupTable_fmt0_validate, /* 0 */
                                           NULL,                          /* 1 */
...
FT_LOCAL_DEF(FT_Int) gxv_glyphid_validate(
    FT_UShort gid, GXV_Validator gxvalid
) {
...
FT_LOCAL_DEF(void) gxv_ctlPoint_validate(
    FT_UShort gid, FT_UShort ctl_point, GXV_Validator gxvalid
) {
    ...
    error = FT_Load_Glyph(
        face, gid, FT_LOAD_NO_BITMAP | FT_LOAD_IGNORE_TRANSFORM);
    if (error)
        FT_INVALID_GLYPH_ID;
...
FT_LOCAL_DEF(void) gxv_sfntName_validate(
    FT_UShort name_index, FT_UShort min_index, FT_UShort max_index, GXV_Validator gxvalid
) {
...
static void gxv_ClassTable_validate(
    FT_Bytes table, FT_UShort *length_p, FT_UShort stateSize, FT_Byte *maxClassID_p, GXV_Validator gxvalid
) {
...
static void gxv_StateArray_validate(
    FT_Bytes table, FT_UShort *length_p, FT_Byte maxClassID, FT_UShort stateSize, FT_Byte *maxState_p,
    FT_Byte *maxEntry_p, GXV_Validator gxvalid
) {
...
static void gxv_EntryTable_validate(
    FT_Bytes table, FT_UShort *length_p, FT_Byte maxEntry, FT_UShort stateArray, FT_UShort stateArray_length,
    ...
    for (entry = 0; entry <= maxEntry; entry++) {
        ...
        if (gxvalid->statetable.entry_validate_func)
            gxvalid->statetable.entry_validate_func(
                state, flags, &glyphOffset, statetable_table, statetable_limit, gxvalid
            );
    }
...
FT_LOCAL_DEF(void) gxv_StateTable_subtable_setup(
    FT_UShort table_size, FT_UShort classTable, FT_UShort stateArray, FT_UShort entryTable,
    FT_UShort *classTable_length_p, FT_UShort *stateArray_length_p, FT_UShort *entryTable_length_p,
...
FT_LOCAL_DEF(void) gxv_StateTable_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
    ...
    setup_func((FT_UShort) (limit - table), classTable, stateArray, entryTable, &classTable_length, &stateArray_length,
               &entryTable_length, gxvalid
    );
    ...
    if (classTable != 0)
        gxv_ClassTable_validate(
            table + classTable, &classTable_length, stateSize, &maxClassID, gxvalid
        );
    else
    ...
    if (stateArray != 0)
        gxv_StateArray_validate(
            table + stateArray, &stateArray_length, maxClassID, stateSize, &maxState, &maxEntry, gxvalid
        );
    else {
#if 0
    ...
    if (entryTable != 0)
        gxv_EntryTable_validate(
            table + entryTable, &entryTable_length, maxEntry, stateArray, stateArray_length, maxClassID, table, limit,
            gxvalid
        );
...
FT_LOCAL_DEF(void) gxv_XStateTable_subtable_setup(
    FT_ULong table_size, FT_ULong classTable, FT_ULong stateArray, FT_ULong entryTable, FT_ULong *classTable_length_p,
...
static void gxv_XClassTable_lookupval_validate(
    FT_UShort glyph, GXV_LookupValueCPtr value_p, GXV_Validator gxvalid
) {
...
static void gxv_XStateArray_validate(
    FT_Bytes table, FT_ULong *length_p, FT_UShort maxClassID, FT_ULong stateSize, FT_UShort *maxState_p,
    FT_UShort *maxEntry_p, GXV_Validator gxvalid
) {
...
static void gxv_XEntryTable_validate(
    FT_Bytes table, FT_ULong *length_p, FT_UShort maxEntry, FT_ULong stateArray_length, FT_UShort maxClassID,
    ...
    for (entry = 0; entry <= maxEntry; entry++) {
        ...
        switch (GXV_GLYPHOFFSET_FMT(xstatetable)) {
            ...
            default: GXV_SET_ERR_IF_PARANOID(FT_INVALID_FORMAT);
                goto Exit;
        }

        if (gxvalid->xstatetable.entry_validate_func)
            gxvalid->xstatetable.entry_validate_func(
                state, flags, &glyphOffset, xstatetable_table, xstatetable_limit, gxvalid
            );
    }
...
FT_LOCAL_DEF(void) gxv_XStateTable_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
    /* StateHeader members */
    ...
    setup_func((FT_ULong) (limit - table), classTable, stateArray, entryTable, &classTable_length, &stateArray_length,
               &entryTable_length, gxvalid
    );
    ...
    if (classTable != 0) {
        ...
        gxv_LookupTable_validate(
            table + classTable, table + classTable + classTable_length, gxvalid
        );
#if 0
    ...
    if (stateArray != 0)
        gxv_XStateArray_validate(
            table + stateArray, &stateArray_length, gxvalid->xstatetable.maxClassID, gxvalid->xstatetable.nClasses,
            &maxState, &maxEntry, gxvalid
        );
    else {
#if 0
        maxState = 1; /* 0:start of text, 1:start of line are predefined */
    ...
    if (entryTable != 0)
        gxv_XEntryTable_validate(
            table + entryTable, &entryTable_length, maxEntry, stateArray_length, gxvalid->xstatetable.maxClassID, table,
            limit, gxvalid
        );
...
static int gxv_compare_ranges(
    FT_Bytes table1_start, FT_ULong table1_length, FT_Bytes table2_start, FT_ULong table2_length
) {
...
FT_LOCAL_DEF(void) gxv_odtect_add_range(
    FT_Bytes start, FT_ULong length, const FT_String *name, GXV_odtect_Range odtect
) {
...
FT_LOCAL_DEF(void) gxv_odtect_validate(
    GXV_odtect_Range odtect, GXV_Validator gxvalid
) {
    ...
    for (i = 0; i < odtect->nRanges; i++)
        for (j = 0; j < i; j++)
            if (0 != gxv_compare_ranges(
                odtect->range[i].start, odtect->range[i].length, odtect->range[j].start, odtect->range[j].length
            )) {
