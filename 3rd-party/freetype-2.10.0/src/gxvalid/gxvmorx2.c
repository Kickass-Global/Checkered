...
static void gxv_morx_subtable_type2_opttable_load(
...
static void gxv_morx_subtable_type2_subtable_setup(
...
static void gxv_morx_subtable_type2_ligActionIndex_validate(
    FT_Bytes table, FT_UShort ligActionIndex, GXV_Validator gxvalid
) {
...
static void gxv_morx_subtable_type2_ligatureTable_validate(
    ...
    if (0 != optdata->ligatureTable) {
        ...
        while (p < limit) {
            ...
            if (lig_gid < gxvalid->face->num_glyphs) GXV_SET_ERR_IF_PARANOID(FT_INVALID_GLYPH_ID);
        }
    }
...
FT_LOCAL_DEF(void) gxv_morx_subtable_type2_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
