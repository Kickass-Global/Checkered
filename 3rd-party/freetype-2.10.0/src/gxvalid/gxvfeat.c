...
static void gxv_feat_registry_validate(
    FT_UShort feature, FT_UShort nSettings, FT_Bool exclusive, GXV_Validator gxvalid
) {
...
static void gxv_feat_name_index_validate(
...
static void gxv_feat_setting_validate(
    FT_Bytes table, FT_Bytes limit, FT_Bool exclusive, GXV_Validator gxvalid
) {
...
static void gxv_feat_name_validate(
    ...
    if ((featureFlags & GXV_FEAT_MASK_UNUSED) == 0) GXV_SET_ERR_IF_PARANOID(FT_INVALID_DATA);
    ...
    for (last_setting = -1, i = 0; i < nSettings; i++) {
        ...
        if ((FT_Int) GXV_FEAT_DATA(setting) <= last_setting) GXV_SET_ERR_IF_PARANOID(FT_INVALID_FORMAT);
...
FT_LOCAL_DEF(void) gxv_feat_validate(
    ...
    for (last_feature = -1, i = 0; i < featureNameCount; i++) {
        ...
        if ((FT_Int) GXV_FEAT_DATA(feature) <= last_feature) GXV_SET_ERR_IF_PARANOID(FT_INVALID_FORMAT);
