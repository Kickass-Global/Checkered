...
static void gxv_just_check_max_gid(
    FT_UShort gid, const FT_String *msg_tag, GXV_Validator gxvalid
) {
...
static void gxv_just_wdc_entry_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
...
static void gxv_just_actSubrecord_type0_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
...
static void gxv_just_actSubrecord_type1_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
...
static void gxv_just_actSubrecord_type5_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
...
static void gxv_just_actSubrecord_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
    ...
    if ((actionClass & 0xFF80) != 0) GXV_SET_ERR_IF_PARANOID(FT_INVALID_DATA);
...
static void gxv_just_pcActionRecord_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
...
static void gxv_just_pcTable_LookupValue_entry_validate(
...
static void gxv_just_pcLookupTable_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
...
static void gxv_just_wdcTable_LookupValue_validate(
    FT_UShort glyph, GXV_LookupValueCPtr value_p, GXV_Validator gxvalid
) {
...
static void gxv_just_justData_lookuptable_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
 ...
 */
static void gxv_just_justData_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
    /*
    ...
    gxv_odtect_add_range(
        p, gxvalid->subtable_length, "just_LookupTable", odtect
    );
    ...
    if (wdcTableOffset) {
        ...
        gxv_odtect_add_range(
            gxvalid->root->base + wdcTableOffset, gxvalid->subtable_length, "just_wdcTable", odtect
        );
    }

    if (pcTableOffset) {
        gxv_just_postcompTable_validate(
            gxvalid->root->base + pcTableOffset, limit, gxvalid
        );
        gxv_odtect_add_range(
            gxvalid->root->base + pcTableOffset, gxvalid->subtable_length, "just_pcTable", odtect
        );
    }
    ...
    if (justClassTableOffset) {
        ...
        gxv_odtect_add_range(
            gxvalid->root->base + justClassTableOffset, gxvalid->subtable_length, "just_justClassTable", odtect
        );
    }
...
FT_LOCAL_DEF(void) gxv_just_validate(
    ...
    gxv_odtect_add_range(
        table, (FT_ULong) (p - table), "just header", odtect
    );
    ...
    if (0 < horizOffset) {
        ...
        gxv_odtect_add_range(
            table + horizOffset, gxvalid->subtable_length, "horizJustData", odtect
        );
    }
    ...
    if (0 < vertOffset) {
        ...
        gxv_odtect_add_range(
            table + vertOffset, gxvalid->subtable_length, "vertJustData", odtect
        );
    }
