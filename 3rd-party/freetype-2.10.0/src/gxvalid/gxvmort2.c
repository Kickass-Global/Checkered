...
static void gxv_mort_subtable_type2_opttable_load(
...
static void gxv_mort_subtable_type2_subtable_setup(
...
static void gxv_mort_subtable_type2_ligActionOffset_validate(
    FT_Bytes table, FT_UShort ligActionOffset, GXV_Validator gxvalid
) {
    ...
    if (p < lat_base) {
        ...
        if (offset * 2 < optdata->ligatureTable) {
            ...
            GXV_TRACE(("too long offset 0x%08x:"
                       ...
                       " (%d byte overrun)\n", offset, optdata->ligatureTable + optdata->ligatureTable_length -
                                                       offset * 2));
...
static void gxv_mort_subtable_type2_entry_validate(
    ...
    if (0 < offset)
        gxv_mort_subtable_type2_ligActionOffset_validate(
            table, offset, gxvalid
        );
}


static void gxv_mort_subtable_type2_ligatureTable_validate(
    FT_Bytes table, GXV_Validator gxvalid
) {
    ...
    if (0 != optdata->ligatureTable) {
        ...
        while (p < limit) {
            ...
            if (gxvalid->face->num_glyphs < lig_gid) GXV_SET_ERR_IF_PARANOID(FT_INVALID_GLYPH_ID);
        }
    }
    GXV_EXIT;
}
...
FT_LOCAL_DEF(void) gxv_mort_subtable_type2_validate(
    FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid
) {
