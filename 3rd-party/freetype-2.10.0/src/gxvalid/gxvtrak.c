...
static void gxv_trak_trackTable_validate(
    FT_Bytes table, FT_Bytes limit, FT_UShort nTracks, GXV_Validator gxvalid
) {
...
static void gxv_trak_trackData_validate(
    ...
    gxv_odtect_add_range(
        table, (FT_ULong) (p - table), "trackData header", odtect
    );

    /* validate trackTable */
    gxv_trak_trackTable_validate(p, limit, nTracks, gxvalid);
    gxv_odtect_add_range(
        p, gxvalid->subtable_length, "trackTable", odtect
    );
    ...
    gxv_odtect_add_range(
        gxvalid->root->base + GXV_TRAK_DATA(trackValueOffset_min),
        GXV_TRAK_DATA(trackValueOffset_max) - GXV_TRAK_DATA(trackValueOffset_min) + nSizes * 2, "trackValue array",
        odtect
    );
...
FT_LOCAL_DEF(void) gxv_trak_validate(
