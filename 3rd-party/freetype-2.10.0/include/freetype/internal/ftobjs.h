...
typedef FT_Error
(*FT_CMap_InitFunc)(
    FT_CMap cmap, FT_Pointer init_data
);
...
(*FT_CMap_CharIndexFunc)(
    FT_CMap cmap, FT_UInt32 char_code
);

typedef FT_UInt
(*FT_CMap_CharNextFunc)(
    FT_CMap cmap, FT_UInt32 *achar_code
);

typedef FT_UInt
(*FT_CMap_CharVarIndexFunc)(
    FT_CMap cmap, FT_CMap unicode_cmap, FT_UInt32 char_code, FT_UInt32 variant_selector
);

typedef FT_Int
(*FT_CMap_CharVarIsDefaultFunc)(
    FT_CMap cmap, FT_UInt32 char_code, FT_UInt32 variant_selector
);

typedef FT_UInt32 *(*FT_CMap_VariantListFunc)(
    FT_CMap cmap, FT_Memory mem
);

typedef FT_UInt32 *(*FT_CMap_CharVariantListFunc)(
    FT_CMap cmap, FT_Memory mem, FT_UInt32 char_code
);

typedef FT_UInt32 *(*FT_CMap_VariantCharListFunc)(
    FT_CMap cmap, FT_Memory mem, FT_UInt32 variant_selector
);
...
FT_BASE(FT_Error) FT_CMap_New(
    FT_CMap_Class clazz, FT_Pointer init_data, FT_CharMap charmap, FT_CMap *acmap
);

/* destroy a charmap and remove it from face's list */
FT_BASE(void) FT_CMap_Done(FT_CMap cmap);


/* add LCD padding to CBox */
FT_BASE(void) ft_lcd_padding(
    FT_BBox *cbox, FT_GlyphSlot slot, FT_Render_Mode mode
);
 ...
 */
FT_BASE(const void*)FT_Get_Module_Interface(
    FT_Library library, const char *mod_name
);

FT_BASE(FT_Pointer) ft_module_get_service(
    FT_Module module, const char *service_id, FT_Bool global
);

#ifdef FT_CONFIG_OPTION_ENVIRONMENT_PROPERTIES

FT_BASE(FT_Error) ft_property_string_set(
 ...
 */
FT_BASE(FT_Error) FT_New_GlyphSlot(
...
FT_BASE(void) FT_Select_Metrics(
    FT_Face face, FT_ULong strike_index
);


/* Set the metrics according to a size request. */
FT_BASE(void) FT_Request_Metrics(
    FT_Face face, FT_Size_Request req
);


/* Match a size request against `available_sizes'. */
FT_BASE(FT_Error) FT_Match_Size(
...
FT_BASE(void) ft_synthesize_vertical_metrics(
...
FT_BASE(void) ft_glyphslot_free_bitmap(FT_GlyphSlot slot);
...
FT_BASE(FT_Bool) ft_glyphslot_preset_bitmap(
    FT_GlyphSlot slot, FT_Render_Mode mode, const FT_Vector *origin
);

/* Allocate a new bitmap buffer in a glyph slot. */
FT_BASE(FT_Error) ft_glyphslot_alloc_bitmap(
...
FT_BASE(void) ft_glyphslot_set_bitmap(
...
FT_BASE(FT_Renderer) FT_Lookup_Renderer(
    FT_Library library, FT_Glyph_Format format, FT_ListNode *node
);

FT_BASE(FT_Error) FT_Render_Glyph_Internal(
 ...
 */
FT_BASE(FT_Memory) FT_New_Memory(void);
 ...
 *     A handle to the memory manager.
 */
FT_BASE(void) FT_Done_Memory(FT_Memory memory);
