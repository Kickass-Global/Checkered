...
void Engine::DamageSystem::update(Engine::deltaTime elapsed) {
  ...
  auto models =
      getEngine()->getSubSystem<EngineStore>()->getRoot().getComponentsOfType<Component::Model>();
  ...
  for (auto &model : models) {
    ...
    size_t index = 0;
    for (auto &&part : model->parts) { index_of.emplace(part.region_name, index++); }
    ...
    for (auto damage : damages) {
      ...
      if (model->parts[i].apply_damage_to_health) { model->health -= damage->damage_amount; }
      health_has_changed = true;
    }

    if (model->health <= 0) {
      for (auto &&part : model->parts) { part.current_damage = part.max_damage + 1; }
    }
    ...
    for (auto &&part : model->parts) {
      ...
      auto it = std::find_if(part.variations.begin(), part.variations.end(),
      ...
      if (it != part.variations.end()) {
        ...
        if (variation_changed) { Engine::log("Updating variation#", part.active_variation); }
      }
      if (part.current_damage >
          part.max_damage)// damage to region has exceeded all variation thresholds (region is destroyed).
      {
    ...
    if (health_has_changed) { model->onHealthChanged(model->health); }
  }
}
