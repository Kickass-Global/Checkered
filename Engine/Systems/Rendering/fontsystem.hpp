...
namespace Engine {
	...
	public:
		...
		void update(Engine::deltaTime time) override {
			...
			for (auto& camera : getEngine()->getSubSystem<EngineStore>()->getRoot().getComponentsOfType<Component::Camera>()) {
				...
				glm::mat4 projection = glm::ortho(0.0f, static_cast<GLfloat>(viewport.width), 0.0f, static_cast<GLfloat>(viewport.height));
				program->bind();
				glUniformMatrix4fv(
					glGetUniformLocation(program->programId(), "projection"), 1, GL_FALSE, glm::value_ptr(projection));
				...
				for (auto text : getEngine()->getSubSystem<EngineStore>()->getRoot().getComponentsOfType<Text>()) {
					...
					auto [width, height] = dimensionsOf(text->text, text->font_size / 12.0); // standard compliant?

					auto offset = glm::vec2{ text->offset.x / viewport.width - text->anchor.x + text->align / 2.0 * width,
											text->offset.y / viewport.height - text->anchor.y - height };

					auto anchor = glm::vec2{ (0.5 * text->anchor.x + 0.5) * viewport.width,
											(0.5 * text->anchor.y + 0.5) * viewport.height };



					RenderText(*program, text->text, anchor.x + offset.x, anchor.y + offset.y, text->font_size / 12.0, text->color);
				}
			}
		}
	...
	private:
		...
		struct { float width; float height; } dimensionsOf(std::string text, float scale)
		{
			float width = 0;
			float height = 0;
