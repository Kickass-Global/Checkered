...
public:
    ...
    void initialize() override {
      ...
      // Load font as face
      FT_Face face;
      if (FT_New_Face(ft, "Assets/Fonts/BalooTamma2-Medium.ttf", 0, &face))
      ...
      glm::mat4 projection =
          glm::ortho(0.0f, static_cast<GLfloat>(640), 0.0f, static_cast<GLfloat>(480));
      program->bind();
      glUniformMatrix4fv(glGetUniformLocation(program->programId(), "projection"), 1, GL_FALSE,
    ...
    void update(Engine::deltaTime time) override {
      ...
      for (auto &camera : getEngine()
        ...
        glm::mat4 projection = glm::ortho(0.0f, static_cast<GLfloat>(viewport.width), 0.0f,
        ...
        for (auto text :
             getEngine()->getSubSystem<EngineStore>()->getRoot().getComponentsOfType<Text>()) {

          auto [width, height] = dimensionsOf(text->text, text->font_size / 12);

          auto dst = text->plot.plot(screen.box, text->dst, text->src);

          auto offset =
              glm::vec2{dst.x + text->align / 2.0 * -std::abs(text->plot.width - width),
                        dst.y + text->vertical_align / 2.0 * -std::abs(text->plot.height - height)};


          RenderText(*program, text->text, offset.x, offset.y, text->font_size / 12.0f,
                     text->color);
        }
      }
    }
...
private:
    ...
    dimensions dimensionsOf(std::string text, float scale) {
      ...
      for (c = text.begin(); c != text.end(); c++) {
        ...
        width += std::max(ch.Size.x * scale, (ch.Advance >> 6) * scale);
    ...
    void RenderText(Program &s, std::string text, GLfloat x, GLfloat y, GLfloat scale,
                    glm::vec3 color) {
