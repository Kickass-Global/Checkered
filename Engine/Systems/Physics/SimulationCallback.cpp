		...
		if (pairs[i].events & PxPairFlag::eNOTIFY_TOUCH_FOUND) {
...

			{
				...
				if (a_vehicle) {
					...
					a_vehicle->onHit(Component::CollisionEventArgs{ b_vehicle, total_force.magnitude(), 
						pairs[i].shapes[0]->getName() ? std::string(pairs[i].shapes[0]->getName()) : "" });
				}
				if (b_vehicle) {
					...
					b_vehicle->onHit(Component::CollisionEventArgs{ a_vehicle, total_force.magnitude(),  
						pairs[i].shapes[1]->getName() ? std::string(pairs[i].shapes[1]->getName()) : "" });
				}
			}
		}
	}
