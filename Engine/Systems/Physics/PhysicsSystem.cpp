...
VehicleDesc initVehicleDescription(bool is_player) {
    ...
    const PxVec3 chassisMOI((chassisDims.y * chassisDims.y + chassisDims.z * chassisDims.z) * chassisMass / 12.0f,
                            ...
                            12.0f, (chassisDims.x * chassisDims.x + chassisDims.y * chassisDims.y) * chassisMass / 12.0f
    );
    ...
    if (is_player) {
        vehicleDesc.chassisSimFilterData = PxFilterData(
            Engine::FilterGroup::ePlayerVehicle, Engine::FilterMask::ePlayerColliders, 0, 0
        );
    } else {
        vehicleDesc.chassisSimFilterData = PxFilterData(
            Engine::FilterGroup::eEnemyVehicle, Engine::FilterMask::eEnemyColliders, 0, 0
        );
    }
        ...
        for (PxU32 i = 0; i < count; i++) {
            ...
            if (pairs[i].status == PxPairFlag::eNOTIFY_TOUCH_LOST) {
                ...
                auto[it, not_overlapping] = overlapping_actors.emplace(
                    std::make_pair(reinterpret_cast<size_t>(a), reinterpret_cast<size_t>(b)), 0
                );
                if (it->second <= 1) {
...
PxVehicleDrive4W *Physics::PhysicsSystem::createDrivableVehicle(
    const PxTransform &worldTransform, bool is_player
) {
    ...
    PxTransform startTransform(
        PxVec3(
            0, (vehicleDesc.chassisDims.y * 0.5f + vehicleDesc.wheelRadius + 2.0f), -10
        ), PxQuat(PxIdentity));
...
void Physics::PhysicsSystem::stepPhysics(Engine::deltaTime timestep) {
    ...
    for (auto &meta : active) {
        ...
        PxVehicleDrive4WSmoothAnalogRawInputsAndSetAnalogInputs(
            meta->pxPadSmoothingData, meta->pxSteerVsForwardSpeedTable, meta->pxVehicleInputData, timestep,
            meta->pxIsVehicleInAir, *meta->pxVehicle
        );
    ...
    std::transform(
        active.begin(), active.end(), std::back_inserter(wheels), [](auto meta) { return meta->pxVehicle; }
    );
...
void Physics::PhysicsSystem::onVehicleCreated(const Component::EventArgs<Component::Vehicle *> &args) {
    ...
    auto T = PxTransform(
        vehicleComponent->position.x, vehicleComponent->position.y, vehicleComponent->position.z
    );
...
void Physics::PhysicsSystem::onActorCreated(const Component::EventArgs<Component::PhysicsActor *> &args) {
    ...
    auto rotation = physx::PxQuat{aPhysicsActor->rotation.x, aPhysicsActor->rotation.y, aPhysicsActor->rotation.z,
                                  aPhysicsActor->rotation.w};
    ...
    if (aPhysicsActor->type == PhysicsActor::Type::StaticObject) {
        ...
        PxShape *aConvexShape = PxRigidActorExt::createExclusiveShape(
            *aPhysicsActor->actor, PxConvexMeshGeometry(
                createConvexMesh(aMesh.get())), *cMaterial
        );
    }
    if (aPhysicsActor->type == PhysicsActor::Type::Ground) {
        aPhysicsActor->actor = cPhysics->createRigidStatic(PxTransform(position, rotation));
        PxShape *aConvexShape = PxRigidActorExt::createExclusiveShape(
            *aPhysicsActor->actor, PxTriangleMeshGeometry(
                createTriMesh(aMesh.get())), *cMaterial
        );
    }
...
Component::Passenger *Physics::PhysicsSystem::createPassenger(
    const PxTransform &pickupTrans, const PxTransform &dropOffTrans
) {
