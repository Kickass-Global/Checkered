...
std::map<physx::PxRigidDynamic*, std::shared_ptr<ComponentBase>>
trackedComponents;
...
VehicleDesc initVehicleDescription(bool is_player) {
	...
	const PxVec3 chassisMOI(
		...
		chassisMass / 12.0f);
	const PxVec3 chassisCMOffset(0.0f, -chassisDims.y * 0.5f, -1.25f);
	...
	if (is_player) {
		vehicleDesc.chassisSimFilterData =
			PxFilterData(Engine::FilterGroup::ePlayerVehicle,
				Engine::FilterMask::ePlayerColliders, 0, 0);
	}
	else {
		vehicleDesc.chassisSimFilterData =
			PxFilterData(Engine::FilterGroup::eEnemyVehicle,
	...
	vehicleDesc.wheelSimFilterData = PxFilterData(
		Engine::FilterGroup::eWheel, Engine::FilterMask::eWheelColliders, 0, 0);
...
void Physics::PhysicsSystem::createFoundation() {
	...
	cFoundation =
		PxCreateFoundation(PX_PHYSICS_VERSION, cDefaultAllocator, cErrorCallback);
	if (!cFoundation)
...
void Physics::PhysicsSystem::createPhysicsObject() {
	...
	cPhysics = PxCreatePhysics(PX_PHYSICS_VERSION, *cFoundation,
		PxTolerancesScale(), true, cPVD);
	if (!cPhysics)
...
void Physics::PhysicsSystem::createCooking() {
	...
	cCooking = PxCreateCooking(PX_PHYSICS_VERSION, *cFoundation,
...
void Physics::PhysicsSystem::createGround() {
	...
	cMaterial =
		cPhysics->createMaterial(STATIC_FRICTION, DYNAMIC_FRICTION, RESTITUTION);
...
void Physics::PhysicsSystem::initVehicleSupport() {
	...
	cVehicleSceneQueryData = VehicleSceneQueryData::allocate(
		100, PX_MAX_NB_WHEELS, 1, 100, WheelSceneQueryPreFilterBlocking, nullptr,
		cDefaultAllocator);
	cBatchQuery = VehicleSceneQueryData::setUpBatchedSceneQuery(
		0, *cVehicleSceneQueryData, cScene);
...
Physics::PhysicsSystem::createDrivableVehicle(const PxTransform& worldTransform,
	...
	PxTransform startTransform(PxVec3(0,
		(vehicleDesc.chassisDims.y * 0.5f +
			vehicleDesc.wheelRadius + 2.0f),
		-10),
		PxQuat(PxIdentity));
	...
	tireData.mLongitudinalStiffnessPerUnitGravity = 5.f;
	tireData.mLatStiffX = 2;
	tireData.mLatStiffY = 18;
...
void Physics::PhysicsSystem::stepPhysics(Engine::deltaTime timestep) {
	...
	std::copy_if(
		vehicles.begin(), vehicles.end(), std::back_inserter(active),
	...
	for (auto& meta : active) {
		...
		PxVehicleDrive4WSmoothAnalogRawInputsAndSetAnalogInputs(
			meta->pxPadSmoothingData, meta->pxSteerVsForwardSpeedTable,
			meta->pxVehicleInputData, timestep, meta->pxIsVehicleInAir,
			*meta->pxVehicle);
	}
	...
	if (!wheels.empty()) {
		// raycasts
		PxRaycastQueryResult* raycastResults =
...
void Physics::PhysicsSystem::onKeyPress(
	const Component::EventArgs<int>& args) { /* do nothing */
}

void Physics::PhysicsSystem::onVehicleRegionDestroyed(PxVehicleDrive4W* vehicle, std::string region_name) {
...
void Physics::PhysicsSystem::onVehicleCreated(
	...
	auto pxVehicle = createDrivableVehicle(T, is_player,
...
PxTriangleMesh* Physics::PhysicsSystem::createTriMesh(Mesh* mesh) {
	...
	meshDesc.triangles.count = static_cast<physx::PxU32>(
		mesh->indices.size() / 3); // assumption tri-mesh
...
void Physics::PhysicsSystem::onActorCreated(
	...
	auto position =
		physx::PxVec3{ aPhysicsActor->position.x, aPhysicsActor->position.y,
					  aPhysicsActor->position.z };
	auto rotation =
		physx::PxQuat{ aPhysicsActor->rotation.x, aPhysicsActor->rotation.y,
	...
	case PhysicsActor::Type::StaticObject:
		FilterShader::setupFiltering(aPhysicsActor->actor, FilterGroup::eScenery | FilterGroup::eGround,
...
void Physics::PhysicsSystem::onPassengerCreated(
	...
	for (auto passenger : passengers) {
		...
		if (!passenger->pickup_actor->actor->actor ||
			!passenger->dropoff_actor->actor->actor)
			continue; // todo, not this....

		PxRigidStatic* temp_rigstat_dropoff =
			cPhysics->createRigidStatic(passenger->dropOffTransform);

		passenger->pass_material = cPhysics->createMaterial(100.0f, 100.f, 100.f);

		PxShapeFlags pass_flags =
...
Component::Passenger*
Physics::PhysicsSystem::createPassenger(const PxTransform& pickupTrans,
...
void Physics::PhysicsSystem::link(Vehicle* sceneComponent,
