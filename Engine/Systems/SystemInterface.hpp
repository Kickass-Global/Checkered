...
public:
    ...
    class EngineSystem *getEngine() {
      return enginePtr;
    }

    virtual void update(Engine::deltaTime elapsed) {
      for (auto &subsystem : subsystems) { subsystem.second->early_update(elapsed); }
      for (auto &subsystem : subsystems) { subsystem.second->update(elapsed); }
      for (auto &subsystem : subsystems) { subsystem.second->late_update(elapsed); }
    }
    ...
    std::vector<Engine::SystemInterface *> getSubSystems() {
      ...
      for (auto &[key, system] : subsystems) { result.push_back(system.get()); }
      return result;
    }
  ...
  void setupWheelsSimulationData(const float wheelMass, const float wheelMOI,
                                 const float wheelRadius, const float wheelWidth,
