...
namespace Component {
	...
	struct NodeEntry {
		...
		NodeEntry(bool momentary, int ttl, std::shared_ptr<class ComponentBase> component);
	};
	...
	struct Node {
		...
		std::shared_ptr<T> create(Args&... args) {
			...
			auto it = std::find_if(  // O(n) #todo
				...
				[&uses](NodeEntry& entry) {
					return !entry.component;
				});

			if (it != components[typeid(T)].end()) {
				it->component
					.reset(new T(args...)); // copy-construct a new component using existing allocation...
				return std::static_pointer_cast<T>(it->component);
			}

			auto component = std::make_shared<T>(args...);
			components[typeid(T)].emplace_back(false, 0, component);
			return std::static_pointer_cast<T>(components[typeid(T)].back().component);
		}

		template<typename T>
		void deactivate(T* component)
		{
			auto it = std::find_if(  // O(n) #todo
				components[typeid(T)].begin(),
				components[typeid(T)].end(),
		...
		template<typename T>
		void addComponents(std::shared_ptr<T> component) {
