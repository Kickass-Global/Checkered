...
public:
    ...
    Passenger(glm::vec3 pickup_location, glm::vec3 dropoff_location, std::shared_ptr<Mesh> mesh,
      ...
      dropoff_actor->onEntered +=
          std::bind(&Passenger::onPassengerDroppedOff, this, std::placeholders::_1);
      dropoff_actor->mesh->enabled = false;

      
      auto timer =
          getEngine()->getSubSystem<EngineStore>()->getRoot().getComponentsOfType<Timer>()[0];
      
      

      getEngine()->getSubSystem<EngineStore>()->getRoot().deactivate<Waypoint>(dropoff_actor.get());
...
private:
    void onPassengerPickedUp(PhysicsActor *) {
      ...
      getEngine()->getSubSystem<EngineStore>()->getRoot().deactivate<Waypoint>(pickup_actor.get());
      getEngine()->getSubSystem<EngineStore>()->getRoot().activate<Waypoint>(dropoff_actor.get());
      dropoff_actor->mesh->enabled = true;
      pickup_actor->mesh->enabled = false;
      onPassengerPickedUpDelegate(0);

      auto distance =
          std::abs(glm::distance(dropoff_actor->actor->position, pickup_actor->actor->position));
      std::chrono::seconds duration = std::chrono::seconds((int) distance / 10);
      auto timer = getEngine()->getSubSystem<EngineStore>()->getRoot().getComponentsOfType<Timer>()[0];
