    ...
    std::function<void(const Component::EventArgs<Vehicle *> &)> ai_tick_callback = [player_vehicle](
        ...
        if (meta->path.pathToGoal.size() > 0) {
            ...
            auto player_direction = perpdot(
                glm::normalize(
                    glm::vec3(nextPoint->my_x + 1.5f, 0, nextPoint->my_z + 1.5f) - ai_location
                ), ai_direction
            );
            // check if the point is in front or behind the ai.
            auto heading = [p = glm::normalize(
                glm::vec3(nextPoint->my_x + 1.5f, 0, nextPoint->my_z + 1.5f) - ai_location
            ), b = glm::vec3(ai_direction)]() {
                return glm::dot(p, b);
            };
            ...
            auto heading = [p = glm::normalize(glm::vec3(player_location - ai_location)), b = glm::vec3(
                ai_direction
            )]() {
                return glm::dot(p, b);
            };
