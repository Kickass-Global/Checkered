...
namespace Engine {
  ...
  struct RelativeAnchor {
    ...
    RelativeAnchor(float u, float v, int x, int y) : u(u), v(v), px_u(x), px_v(y) {}

    RelativeAnchor operator+(PixelOffset offset) { return {u, v, offset.x, offset.y}; }
  ...
  struct BoxModel {
    ...
    explicit BoxModel(float x, float y, float width, float height) : box(x, y, width, height) {}
   ...
   */
    ...
    Offset plot(RelativeAnchor anchor) {
      auto [x, y] = anchor(box.width, box.height);
